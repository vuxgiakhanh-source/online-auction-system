 ...
 *
 * <ul>
 *   <li>Không có hậu tố → Client gửi lên Server (request).
 *   <li>Hậu tố {@code _SUCCESS} → Server trả về thành công.
 *   <li>Hậu tố {@code _FAILED} → Server trả về lỗi nghiệp vụ.
 *   <li>Hậu tố {@code _UPDATE} → Server broadcast realtime tới nhiều client.
 *   <li>Hậu tố {@code _NOTIFY} → Server push đơn chiều tới 1 client cụ thể.
 * </ul>
 *
...
public enum PacketType {
  ...
  /**
   * Server xác nhận đăng ký thành công, trả về token + thông tin user. Payload: {@code
   * LoginResponseDTO}.
   */
  REGISTER_SUCCESS,

  /** Server từ chối đăng ký (username đã tồn tại, email trùng, v.v.). Payload: {@code ErrorDTO}. */
  REGISTER_FAILED,
  ...
  /**
   * Server xác nhận đăng nhập thành công, trả về token + thông tin user. Payload: {@code
   * LoginResponseDTO}.
   */
  LOGIN_SUCCESS,

  /** Server từ chối đăng nhập (sai mật khẩu, tài khoản bị ban, v.v.). Payload: {@code ErrorDTO}. */
  LOGIN_FAILED,

  /** Client gửi yêu cầu đăng xuất (giải phóng WebSocket session). Payload: rỗng. */
  LOGOUT,
  ...
  /**
   * Client yêu cầu lấy thông tin hồ sơ người dùng khác theo userId. Payload: {@code String userId}.
   */
  GET_USER_PROFILE,
  ...
  /** Server xác nhận đã nhận đơn yêu cầu, đang chờ hệ thống auto-duyệt. Payload: rỗng. */
  REQUEST_SELLER_ROLE_SUCCESS,

  /**
   * Server từ chối yêu cầu nâng cấp (đã từng bị phạt, đã là Seller, v.v.). Payload: {@code
   * ErrorDTO}.
   */
  REQUEST_SELLER_ROLE_FAILED,

  /**
   * Server push khi hệ thống tự động duyệt (approve) role Seller. Payload: {@code UserDTO} (đã có
   * role SELLER).
   */
  SELLER_ROLE_APPROVED_NOTIFY,

  /**
   * Server push khi hệ thống từ chối tự động duyệt role Seller. Payload: {@code ErrorDTO} với lý
   * do.
   */
  ...
  /**
   * Server xác nhận rút tiền thành công, trả về số dư mới. Payload: {@code
   * WalletBalanceResponseDTO}.
   */
  WITHDRAW_SUCCESS,

  /**
   * Server từ chối rút tiền (số dư không đủ, tài khoản không hợp lệ, v.v.). Payload: {@code
   * ErrorDTO}.
   */
  WITHDRAW_FAILED,
  ...
  /** Server từ chối ban (user không tồn tại, đã bị ban, v.v.). Payload: {@code ErrorDTO}. */
  ...
  /**
   * Server xác nhận nạp tiền thành công, trả về số dư mới. Payload: {@code
   * WalletBalanceResponseDTO}.
   */
  DEPOSIT_SUCCESS,

  /**
   * Server từ chối nạp tiền (số tiền không hợp lệ, tài khoản bị khóa, v.v.). Payload: {@code
   * ErrorDTO}.
   */
  DEPOSIT_FAILED,
  ...
  /**
   * Server trả về số dư ví (balance + lockedDeposit + availableBalance). Payload: {@code
   * WalletBalanceResponseDTO}.
   */
  ...
  /**
   * Server từ chối tạo phiên (seller chưa đủ điều kiện, thông tin không hợp lệ, v.v.). Payload:
   * {@code ErrorDTO}.
   */
  CREATE_AUCTION_FAILED,

  /**
   * Client yêu cầu danh sách tất cả phiên đấu giá (có thể lọc theo status). Payload: {@code
   * AuctionListRequestDTO} (status filter, sort, page).
   */
  GET_AUCTION_LIST,
  ...
  /** Seller yêu cầu cập nhật thông tin phiên (chỉ khi OPEN). Payload: {@code UpdateAuctionDTO}. */
  UPDATE_AUCTION,
  ...
  /**
   * Server từ chối cập nhật (phiên đã RUNNING, không phải chủ, v.v.). Payload: {@code ErrorDTO}.
   */
  ...
  /** Server xác nhận đã nhận đơn yêu cầu hủy. Payload: {@code String auctionId}. */
  CANCEL_AUCTION_REQUEST_SUCCESS,

  /**
   * Server từ chối yêu cầu hủy (phiên không ở OPEN, không phải chủ phiên, v.v.). Payload: {@code
   * ErrorDTO}.
   */
  CANCEL_AUCTION_REQUEST_FAILED,

  /**
   * Server push thông báo cho Staff Admin khi có Seller yêu cầu hủy phiên. Payload: {@code
   ...
   * {@code AdminCancelAuctionDTO} (auctionId + reason).
   */
  ...
  /** Server từ chối hủy (phiên đã FINISHED, không tồn tại, v.v.). Payload: {@code ErrorDTO}. */
  ADMIN_CANCEL_AUCTION_FAILED,

  /**
   * Admin yêu cầu danh sách tất cả phiên đấu giá để quản lý. Payload: {@code
  ...
  /**
   * Server xác nhận đã join thành công, cọc đã bị lock. Payload: {@code JoinAuctionResponseDTO}
   * (AuctionDTO + depositAmount).
   */
  JOIN_AUCTION_SUCCESS,

  /**
   * Server từ chối join (không đủ số dư, đã join, phiên không RUNNING, v.v.). Payload: {@code
   * ErrorDTO}.
   */
  JOIN_AUCTION_FAILED,

  /**
   * Client yêu cầu theo dõi phiên (xem realtime, không đặt cọc). Payload: {@code String auctionId}.
   */
  WATCH_AUCTION,

  /**
   * Server xác nhận đã watch thành công. Payload: {@code AuctionDTO} (trạng thái phiên hiện tại).
   */
  ...
  /** Client ngừng theo dõi phiên (rời khỏi view). Payload: {@code String auctionId}. */
  LEAVE_AUCTION,
  ...
  /**
   * Server từ chối bid (giá thấp hơn hiện tại, phiên đã đóng, chưa join, v.v.). Payload: {@code
   * ErrorDTO}.
   */
  PLACE_BID_FAILED,
  ...
  /** Server xác nhận đăng ký Auto-Bid thành công. Payload: {@code AutoBidRegistrationDTO}. */
  REGISTER_AUTO_BID_SUCCESS,

  /**
   * Server từ chối đăng ký Auto-Bid (maxBid <= giá hiện tại, chưa join, v.v.). Payload: {@code
   * ErrorDTO}.
   */
  REGISTER_AUTO_BID_FAILED,

  /**
   * Client cập nhật maxBid của Auto-Bid đang hoạt động. Payload: {@code AutoBidRequestDTO} (maxBid
   * mới).
   */
  UPDATE_AUTO_BID,
  ...
  /**
   * Server từ chối hủy Auto-Bid (không có auto-bid nào đang chạy, v.v.). Payload: {@code ErrorDTO}.
   */
  CANCEL_AUTO_BID_FAILED,

  /**
   * Client yêu cầu xem trạng thái Auto-Bid hiện tại của mình trong phiên. Payload: {@code String
   * auctionId}.
   */
  GET_AUTO_BID_STATUS,
  ...
  /**
   * Server push thông báo khi hệ thống tự động đặt giá thay client. Payload: {@code
   * AutoBidTriggeredDTO}.
   */
  AUTO_BID_TRIGGERED_NOTIFY,

  /**
   * Server push thông báo khi Auto-Bid của client bị vượt qua bởi đối thủ và đã đạt maxBid (không
  ...
  /**
   * Server broadcast khi có bid hợp lệ nhưng chưa đạt reserve price. Payload: {@code BidUpdateDTO}.
   */
  BID_RESERVE_NOT_MET_UPDATE,

  /**
   * Server broadcast khi phiên chuyển sang trạng thái RUNNING. Payload: {@code AuctionUpdateDTO}.
   */
  AUCTION_STARTED_UPDATE,

  /**
   * Server broadcast khi phiên sắp kết thúc (cảnh báo 5 phút, 1 phút). Payload: {@code
   * AuctionUpcomingEndDTO} (auctionId + remainingSeconds).
   */
  AUCTION_UPCOMING_END_NOTIFY,

  /**
   * Server broadcast khi phiên kết thúc (FINISHED) — có người thắng và đã đạt reserve. Payload:
   * {@code AuctionUpdateDTO} (chứa winner + finalPrice).
   */
  AUCTION_ENDED_UPDATE,

  /**
   * Server push khi phiên kết thúc không có ai đặt giá (CANCELED, no_winner). Payload: {@code
   * AuctionUpdateDTO}.
   */
  AUCTION_NO_WINNER_UPDATE,

  /**
   * Server push khi phiên kết thúc nhưng chưa đạt reserve price (RESERVE_NOT_MET). Payload: {@code
   * AuctionUpdateDTO}.
   */
  AUCTION_RESERVE_NOT_MET_UPDATE,

  /**
   * Server push khi phiên bị hủy (CANCELED) bởi Admin hoặc system. Payload: {@code
   * AuctionUpdateDTO}.
   */
   ...
   * client đang xem phiên đều nhận packet này. Payload: {@code AuctionExtendedDTO} (chứa
   * extendedEndTime mới).
   */
  ...
  /** Server xác nhận thanh toán thành công (trạng thái PAID). Payload: {@code PaymentResultDTO}. */
  PAYMENT_SUCCESS,

  /**
   * Server từ chối thanh toán (không đủ số dư, phiên chưa FINISHED, đã quá hạn, v.v.). Payload:
   * {@code ErrorDTO}.
   */
  PAYMENT_FAILED,

  /**
   * Server push thông báo cho Seller khi người thắng đã thanh toán. Payload: {@code
   * PaymentResultDTO}.
   */
  PAYMENT_COMPLETED_NOTIFY,

  /**
   * Winner xác nhận đã nhận hàng — mở khóa quyền submit Quality Report. Payload: {@code String
   * auctionId}.
   */
  CONFIRM_ITEM_RECEIVED,
  /**
   * Server xác nhận nhận hàng thành công. Payload: {@link
   * com.group13.auction.common.dto.payment.PaymentDTOs.ConfirmItemReceivedResultDTO}.
   */
  CONFIRM_ITEM_RECEIVED_SUCCESS,
  /**
   * Server từ chối — chưa thanh toán, không phải winner, hoặc đã xác nhận rồi. Payload: {@code
   * ErrorDTO}.
   */
  CONFIRM_ITEM_RECEIVED_FAILED,

  /**
   * Server push thông báo cho Winner khi hết hạn thanh toán (24h). Payload: {@code
   * PaymentExpiredDTO} (auctionId + depositForfeited).
   */
   ...
   * (auctionId + refundAmount + newBalance).
   */
  DEPOSIT_REFUND_NOTIFY,

  /**
   * Server push thông báo tịch thu cọc khi Winner không thanh toán đúng hạn. Payload: {@code
  ...
  /** Runner-up chấp nhận Second Chance Offer. Payload: {@code String auctionId}. */
  SECOND_CHANCE_ACCEPT,

  /**
   * Server xác nhận chấp nhận Second Chance thành công. Payload: {@code PaymentResultDTO} (trạng
   * thái PENDING — cần thanh toán tiếp).
   */
  SECOND_CHANCE_ACCEPT_SUCCESS,

  /**
   * Server từ chối chấp nhận (offer đã hết hạn, offer không PENDING, v.v.). Payload: {@code
   * ErrorDTO}.
   */
  SECOND_CHANCE_ACCEPT_FAILED,

  /** Runner-up từ chối Second Chance Offer. Payload: {@code String auctionId}. */
  ...
  /**
   * Broadcast tới tất cả watcher của phiên khi runner-up chấp nhận Second Chance Offer. Phiên có
   * winner mới — client cần cập nhật UI hiển thị winner. Payload: {@link
  ...
  /**
   * Server push một điểm dữ liệu mới lên biểu đồ realtime khi có bid hợp lệ. Client append điểm mới
   * vào chart mà không cần refresh. Payload: {@code BidChartPointDTO} (timestamp + price +
   * bidderName).
   */
  ...
  /** Server xác nhận nhận báo cáo (PENDING). Payload: {@code QualityReportDTO}. */
  SUBMIT_QUALITY_REPORT_SUCCESS,

  /**
   * Server từ chối nhận báo cáo (không phải winner, phiên chưa PAID, v.v.). Payload: {@code
   * ErrorDTO}.
   */
  SUBMIT_QUALITY_REPORT_FAILED,

  /**
   * Admin yêu cầu danh sách báo cáo chất lượng theo bộ lọc trạng thái. Payload: {@code String} —
   * {@code PENDING} | {@code APPROVED} | {@code REJECTED} | {@code ALL}.
   */
  ...
  /**
   * Admin approve báo cáo chất lượng → trừ rating seller + hoàn tiền winner. Payload: {@code String
   * reportId}.
   */
  ADMIN_APPROVE_QUALITY_REPORT,

  /**
   * Server xác nhận đã approve báo cáo, seller bị phạt. Payload: {@code QualityReportResultDTO}.
   */
  ADMIN_APPROVE_QUALITY_REPORT_SUCCESS,

  /** Server từ chối approve (report không PENDING, v.v.). Payload: {@code ErrorDTO}. */
  ...
  /**
   * Server push thông báo cho Winner khi báo cáo được duyệt và tiền đã được hoàn. Payload: {@code
   * QualityReportResultDTO}.
   */
  QUALITY_REPORT_APPROVED_NOTIFY,

  /**
   * Server push thông báo cho Seller khi bị báo cáo chất lượng. Payload: {@code QualityReportDTO}.
   */
  QUALITY_REPORT_RECEIVED_NOTIFY,

  /**
   * Server push thông báo cho Seller khi báo cáo bị reject (tức là Seller không bị phạt). Payload:
   * {@code String reportId}.
   */
  ...
  /**
   * Server từ chối đánh giá (đã đánh giá rồi, phiên chưa PAID, v.v.). Payload: {@code ErrorDTO}.
   */
  RATE_SELLER_FAILED,

  /**
   * Seller đánh giá Bidder sau khi giao dịch hoàn tất. Payload: {@code RateBidderRequestDTO}
   * (bidderId + rating + comment).
   */
  RATE_BIDDER,
  ...
  /** Client yêu cầu lấy lịch sử đánh giá của một user. Payload: {@code String userId}. */
  GET_USER_RATINGS,
  ...
  /**
   * Server push thông báo khi tài khoản bị SUSPEND (rating xuống dưới ngưỡng). Payload: {@code
   * AccountSuspendedDTO} (reason + rating).
   */
  ACCOUNT_SUSPENDED_NOTIFY,

  /**
   * Server push thông báo khi tài khoản bị BAN (vĩnh viễn). Payload: {@code AccountBannedDTO}
   * (reason).
   */
  ...
  /**
   * Server push lỗi hệ thống không thuộc nghiệp vụ cụ thể nào (mất kết nối DB, lỗi nội bộ, v.v.).
   * Payload: {@code ErrorDTO}.
   */
  SYSTEM_ERROR,

  /**
   * Server push thông báo tới toàn bộ client đang kết nối (broadcast toàn hệ thống). Dùng cho bảo
   * trì, thông báo khẩn cấp. Payload: {@code SystemAnnouncementDTO} (message + severity).
   */
  SYSTEM_ANNOUNCEMENT,

  /**
   * Server push thông báo server sắp shutdown/maintenance. Client nên hiển thị cảnh báo và lưu
  ...
  /**
   * Server trả về kết quả tìm kiếm (có phân trang). Payload: {@code
   * SearchDTOs.ItemSearchResponseDTO}.
   */
  ...
  /**
   * Server trả về câu trả lời tìm được từ FAQ. Payload: {@code ChatbotResponse} (faqId + question +
   * answer + category).
   */
  CHATBOT_ANSWER,

  /**
   * Server trả về khi không tìm thấy câu trả lời phù hợp. Payload: {@code ChatbotResponse} (message
   * gợi ý liên hệ admin).
   */
  CHATBOT_NOT_FOUND,

  /**
   * Client yêu cầu lấy toàn bộ danh sách câu hỏi FAQ theo category. Payload: {@code String
   * category} (GENERAL | BIDDING | PAYMENT | RATING | SELLER). Nếu null → trả về tất cả category.
   */
  CHATBOT_GET_FAQ_LIST,

  /**
   * Server trả về danh sách FAQ (dùng để hiển thị menu câu hỏi gợi ý). Payload: {@code
   * ChatbotFaqListResponse} (List&lt;FAQ&gt; theo category).
   */
