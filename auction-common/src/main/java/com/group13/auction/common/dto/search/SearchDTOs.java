 ...
 *
 * <ul>
 *   <li>{@link ItemSearchRequestDTO}  — Client gửi lên kèm packet {@code SEARCH_ITEMS}.</li>
 *   <li>{@link ItemSearchResponseDTO} — Server trả về kèm packet {@code SEARCH_ITEMS_SUCCESS}.</li>
 * </ul>
 */
public final class SearchDTOs {
   ...
   * <p>Các trường hỗ trợ:
   * <ul>
   *   <li>{@code keyword}  — từ khóa tìm theo tên sản phẩm (không phân biệt hoa thường, LIKE).</li>
   *   <li>{@code page}     — trang hiện tại, bắt đầu từ 0 (mặc định 0).</li>
   *   <li>{@code size}     — số kết quả mỗi trang (mặc định 20, tối đa 100).</li>
   *   <li>{@code sortBy}   — cột sắp xếp: {@code currentPrice | endTime | createdAt | itemName}
   *                          (mặc định {@code createdAt}).</li>
   *   <li>{@code sortDir}  — chiều sắp xếp: {@code ASC | DESC} (mặc định {@code DESC}).</li>
   * </ul>
   */
   ...
   *
   * <p>Tái sử dụng {@link AuctionDTOs.AuctionDTO} làm item đơn vị để client
   * không cần mapper mới — cùng ViewModel với danh sách auction thông thường.
   */
  public static class ItemSearchResponseDTO {
    ...
    public ItemSearchResponseDTO(List<AuctionDTOs.AuctionDTO> auctions,
                                 long totalElements,
                                 int currentPage,
                                 int pageSize,
                                 String keyword) {
    ...
    public List<AuctionDTOs.AuctionDTO> getAuctions()                 { return auctions;       }
    public void setAuctions(List<AuctionDTOs.AuctionDTO> auctions)    { this.auctions = auctions; }
