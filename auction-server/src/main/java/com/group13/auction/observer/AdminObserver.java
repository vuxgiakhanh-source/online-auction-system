...
/**
 * Observer dành cho Admin khi join một phiên cụ thể.
 * Nhận notify chi tiết theo phiên như người bình thường.
 *
 * <p>Khi admin joinAuction → dùng observer này để nhận thêm notify
 * chi tiết theo phiên đó và persist vào bảng notifications.
 */
        ...
        switch (event.getEventType()) {
            case AUCTION_STARTED -> {
                log.info("[LOG - THÔNG BÁO tới Admin {}] Phiên bắt đầu: {}",
                    admin.getUsername(), auctionId);
                saveNotification(auctionId,
                    "Phiên đấu giá bắt đầu",
                    "Phiên " + auctionId + " đã chuyển sang trạng thái RUNNING.");
            }
            case AUCTION_ENDED -> {
                String winner = event.getBidder() != null
                    ? event.getBidder().getUsername() : "Không có";
                log.info("[LOG - THÔNG BÁO tới Admin {}] Phiên kết thúc: {} | Winner: {}",
                    admin.getUsername(), auctionId, winner);
                saveNotification(auctionId,
            ...
            case RESERVE_NOT_MET_CLOSED -> {
                log.info("[LOG - THÔNG BÁO tới Admin {}] Phiên {} kết thúc với giá cao nhất {} nhưng CHƯA ĐẠT mức tối thiểu.",
                    admin.getUsername(), auctionId, event.getBidAmount());
                saveNotification(auctionId,
                    "Phiên kết thúc chưa đạt giá sàn",
                    "Phiên " + auctionId + " bị hủy. Giá cao nhất "
                        + event.getBidAmount() + " chưa đạt mức reserve.");
            }
            ...
            case FRAUD_DETECTED -> {
                ...
                log.info("[CẢNH BÁO - THÔNG BÁO tới Admin {}] GIAN LẬN phát hiện tại phiên {}: {}",
                    admin.getUsername(), auctionId, msg);
                saveNotification(auctionId,
                    "⚠ Cảnh báo gian lận",
