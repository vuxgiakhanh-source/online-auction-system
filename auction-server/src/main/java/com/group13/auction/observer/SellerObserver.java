...
public class SellerObserver implements AuctionObserver {
  ...
  public void onBidPlaced(AuctionEvent event) {
    switch (event.getEventType()) {
      case BID_PLACED:
        notifier.notify(seller.getUsername(), "BID_PLACED",
                String.format("Bid mới: %d | Phiên: %s", event.getBidAmount(), event.getAuction().getId()));
        break;
      case BID_RESERVE_NOT_MET:
        notifier.notify(seller.getUsername(), "BID_RESERVE_NOT_MET",
                String.format("Bid %d chưa đạt reserve price (%d).",
  ...
  public void onAuctionEnded(AuctionEvent event) {
    switch (event.getEventType()) {
      case AUCTION_STARTED:
        notifier.notify(seller.getUsername(), "AUCTION_STARTED", "Phiên đấu giá của bạn đã bắt đầu!");
        break;
      case AUCTION_UPCOMING:
        break;
      case AUCTION_ENDED:
        if (event.getBidder() != null) {
          notifier.notify(seller.getUsername(), "AUCTION_ENDED",
                  String.format("Phiên kết thúc. Winner: %s | Giá: %d. Chờ thanh toán.",
                          event.getBidder().getUsername(), event.getBidAmount()));
        }
        break;
      case AUCTION_NO_WINNER:
        notifier.notify(seller.getUsername(), "AUCTION_NO_WINNER",
                "Phiên kết thúc không có ai đặt giá. Phiên đã bị hủy.");
        break;
      case RESERVE_NOT_MET_CLOSED:
        notifier.notify(seller.getUsername(), "RESERVE_NOT_MET_CLOSED",
                String.format("Phiên '%s' kết thúc với mức giá cao nhất là %d nhưng chưa đạt mức giá tối thiểu của bạn. Phiên đã bị hủy.",
                        event.getAuction().getItem().getName(), event.getBidAmount()));
        break;
      case PAYMENT_COMPLETED:
        notifier.notify(seller.getUsername(), "PAYMENT_COMPLETED",
                String.format("Sản phẩm '%s' đã được bán thành công với giá %d! Tiền đã được chuyển vào tài khoản (sau thuế).",
                        event.getAuction().getItem().getName(), event.getBidAmount()));
