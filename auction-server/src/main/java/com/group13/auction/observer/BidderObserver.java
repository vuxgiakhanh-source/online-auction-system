  ...
  public void onAuctionEnded(AuctionEvent event) {
    switch (event.getEventType()) {
      ...
      case AUCTION_EXTENDED:
        String msgExt =
            String.format(
                "Phiên %s được gia hạn. EndTime mới: %s. %s",
                event.getAuction().getId(),
      ...
      case AUCTION_UPCOMING:
        notifier.notify(
            bidder.getUsername(), "AUCTION_UPCOMING", "Phiên sắp bắt đầu - chuẩn bị sẵn sàng.");
        break;
      case AUCTION_ENDED:
        if (event.getBidder() != null
          ...
          notifier.notify(
              ...
              "AUCTION_ENDED_WIN",
              String.format(
                  "Chúc mừng! Bạn thắng phiên với giá %d. Hãy thanh toán trong 24h.",
                  event.getBidAmount()));
        } else {
          notifier.notify(
              bidder.getUsername(),
      ...
      case RESERVE_NOT_MET_CLOSED:
        notifier.notify(
            ...
            String.format(
                "Phiên kết thúc - giá cao nhất %d chưa đạt reserve. Cọc sẽ được hoàn trả.",
                event.getBidAmount()));
        break;
