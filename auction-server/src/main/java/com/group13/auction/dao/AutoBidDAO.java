...
/**
 * DAO persist AutoBid entries xuống DB. Giải quyết vấn đề: auto-bid registry chỉ in-memory → mất
 * khi server restart.
 *
...
public class AutoBidDAO {
  ...
  public boolean upsert(String userId, String auctionId, long maxBid, LocalDateTime registeredAt) {
    String sql =
        "INSERT INTO auto_bids (user_id, auction_id, max_bid, registered_at) VALUES (?, ?, ?, ?) ON"
            + " DUPLICATE KEY UPDATE max_bid = VALUES(max_bid), registered_at ="
    ...
    try (Connection conn = DatabaseConnection.getInstance().getConnection();
      ...
      log.debug(
          "upsert auto_bid: userId={} auctionId={} maxBid={} ok={}", userId, auctionId, maxBid, ok);
      return ok;
      ...
      log.warn(
          "Lỗi upsert auto_bid: userId={} auctionId={} — {}", userId, auctionId, e.getMessage());
      return false;
    }
  }
