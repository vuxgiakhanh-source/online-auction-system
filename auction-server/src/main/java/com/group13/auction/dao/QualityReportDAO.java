...
public class QualityReportDAO {
  ...
  private QualityReport mapReportRow(java.sql.ResultSet rs) throws java.sql.SQLException {
    ...
    return QualityReport.reconstitute(
        ...
        status,
        null,
        rs.getBoolean("refund_completed"));
  }
  ...
  private com.group13.auction.model.user.User.AccountStatus parseStatus(String s) {
    ...
    try {
      return com.group13.auction.model.user.User.AccountStatus.valueOf(s);
    } catch (Exception e) {
      return com.group13.auction.model.user.User.AccountStatus.ACTIVE;
    }
  }
  ...
  /**
   * Kiểm tra reporter đã từng gửi báo cáo cho phiên này chưa (không phân biệt trạng thái — chỉ được
   * gửi 1 lần duy nhất).
   *
  ...
  public boolean existsByAuctionAndReporter(String auctionId, String reporterId) {
    ...
    try (Connection conn = DatabaseConnection.getInstance().getConnection();
      ...
      log.error(
          "Lỗi existsByAuctionAndReporter: auctionId={}, reporterId={}", auctionId, reporterId, e);
      return false;
    }
  }

  /**
   * Lưu báo cáo chất lượng mới vào Database. Bao gồm description và image_urls (bắt buộc NOT NULL
   * trong schema).
   */
  /**
   * Tìm QualityReport theo reportId — dùng cho approve/reject. JOIN users để load reporter cùng lúc
   * (tránh N+1).
   */
  ...
  /**
   * Cập nhật trạng thái và cờ refund_completed của báo cáo. Được gọi ngay sau khi approve/reject để
   * persist status xuống DB.
   */
