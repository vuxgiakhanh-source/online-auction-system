...
public class UserDAO {
  ...
  public boolean saveUserAuctionActivity(String userId, String auctionId, String activityType) {
    ...
    String sql =
        "INSERT INTO user_auction_activity (user_id, auction_id, activity_type) "
            + "VALUES (?, ?, ?) "
  ...
  public boolean markUserLeftAuction(String userId, String auctionId) {
    ...
    try (Connection conn = DatabaseConnection.getInstance().getConnection();
      ...
      log.error(
          "Lỗi xóa hoạt động JOINED khi rời phiên: userId={}, auctionId={}", userId, auctionId, e);
      return false;
    }
  }
  ...
  /**
   * Lấy danh sách userId đang JOINED một phiên đấu giá cụ thể. Dùng bởi
   * ServerBroadcastNotifier.notifyJoinedParticipants() để push notification tới đúng những user
   * đang tham gia phiên.
   *
  ...
  private static boolean getBooleanOrDefault(ResultSet rs, String columnName, boolean defaultValue)
      throws SQLException {
