 ...
 *
 * <h3>Mục đích:</h3>
 * <p>Khi hệ thống có 5000 người dùng đồng thời, một user duy nhất có thể gửi
 * hàng trăm bid/giây (flood attack hoặc bug client), gây nghẽn lock và DB.
 * Rate limiter này chặn request quá nhanh ở tầng sớm nhất có thể.
 *
 * <h3>Thuật toán: Token Bucket đơn giản hóa (Fixed Window)</h3>
 * <ul>
 *   <li>Mỗi user có một bucket với {@code MAX_BIDS_PER_WINDOW} token.</li>
 *   <li>Mỗi lần bid tiêu tốn 1 token.</li>
 *   <li>Bucket reset mỗi {@code WINDOW_MS} milliseconds.</li>
 *   <li>Thread-safe: dùng AtomicLong cho count và lastReset.</li>
 * </ul>
 *
 * <h3>Giá trị mặc định:</h3>
 * <ul>
 *   <li>5 bid / 1 giây / user — đủ cho normal usage và đủ hạn chế flood.</li>
 *   <li>ConcurrentHashMap — không global lock khi check rate.</li>
 *   <li>Tự cleanup sau khi user idle {@code IDLE_CLEANUP_MS} (15 phút).</li>
 * </ul>
 */
public final class BidRateLimiter {
    ...
    public static BidRateLimiter getInstance() { return INSTANCE; }
    ...
    public boolean tryConsume(String userId) {
        ...
        if (!allowed) {
            log.warn("Rate limit exceeded: userId={} (max {}/{}ms)",
                    userId, MAX_BIDS_PER_WINDOW, WINDOW_MS);
        }
        return allowed;
    }
    ...
    /**
     * Cleanup các bucket đã idle quá lâu.
     * Gọi định kỳ từ AuctionTimerService (mỗi 5 phút là đủ).
     */
