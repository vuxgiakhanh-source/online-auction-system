 ...
 *
 * <p>hệ thống chỉ bid mức TỐI THIỂU cần thiết để vượt người đang dẫn đầu.
 *
 * <p><b>Cơ chế vượt leader:</b> Khi user đặt maxBid thấp hơn giá hiện tại (currentPrice), hệ thống
 * tự động tính mức bid cần thiết để vượt người dẫn đầu: {@code nextBid = currentPrice +
 * minIncrement}. Nếu nextBid > maxBid thì không tự bid nữa (trả về -1).
 *
...
public class AutoBidStrategy implements BidStrategy {
  ...
  public boolean isValidBid(Auction auction, long amount) {
    ...
    if (!valid) {
      log.warn(
          "Auto bid rejected: auctionId={}, amount={}, maxBid={}, currentPrice={}, minIncrement={}",
          auction.getId(),
          amount,
          maxBid,
      ...
      log.debug(
          "Auto bid accepted by strategy: auctionId={}, amount={}, maxBid={}, currentPrice={},"
              + " minIncrement={}",
          auction.getId(),
          amount,
          maxBid,
   ...
   *
   * <p>Hệ thống tính dựa trên {@code currentPrice} (giá người đang dẫn đầu), không phải giá user đã
   * nhập.
   *
  ...
  public long calculateNextBid(Auction auction) {
    ...
    if (next > maxBid) {
      log.debug(
          "Auto bid cannot continue because next bid exceeds maxBid: auctionId={}, nextBid={},"
              + " maxBid={}",
          auction.getId(),
          next,
          maxBid);
    ...
    log.debug(
        "Calculated next auto bid: auctionId={}, currentPrice={}, minIncrement={}, nextBid={},"
            + " maxBid={}",
        auction.getId(),
        auction.getCurrentPrice(),
        increment,
  ...
  public String describe() {
    return String.format(
        "Auto: Tự bid vượt người dẫn đầu (bước giá tự động theo ngưỡng" + "), tối đa %d.", maxBid);
  }
