...
/**
 * Registry lưu một ReentrantLock riêng biệt cho mỗi phiên đấu giá. Lưu một {@link ReentrantLock}
 * riêng biệt cho mỗi phiên đấu giá.
 *
 * <h3>Tại sao cần lock per-auction?</h3>
 *
 * <p>Khi nhiều client đặt giá cùng lúc (concurrent bidding), nếu không có lock per-auction thì xảy
 * ra race condition:
 *
 * <ul>
 *   <li>Thread A đọc {@code currentPrice = 1_000_000}
 *   <li>Thread B đọc {@code currentPrice = 1_000_000} (cùng lúc)
 *   <li>Cả hai đặt bid → cả hai pass validate → lost update
 * </ul>
 *
 * <h3>⚠️ Giới hạn: SINGLE INSTANCE ONLY</h3>
 *
 * <p>Lock này là in-memory {@link ReentrantLock} — chỉ hoạt động đúng trong <b>một JVM process duy
 * nhất</b>. Nếu deploy nhiều instance (Docker Swarm, Kubernetes horizontal scaling), mỗi instance
 * có lock registry riêng:
 *
 * <pre>
 ...
 *
 * <p>Thay thế bằng Redis distributed lock (Redisson RLock hoặc SET NX PX pattern). Business logic
 * trong BidService và AuctionTimerService không cần thay đổi — chỉ cần swap implementation ở
 * ServerMain.
 *
...
public final class AuctionLockRegistry {
  ...
  /**
   * Lấy lock của một phiên, tạo mới nếu chưa có. Thread-safe nhờ {@link
   * ConcurrentHashMap#computeIfAbsent}.
   *
   * <p><b>FIX PERFORMANCE:</b> dùng {@code fair=false} (unfair lock). Fair lock (FIFO queue) dưới
   * high contention cho throughput thấp hơn ~4× vì phải context-switch giữ thứ tự. Unfair lock tận
   * dụng cache hot path. Starvation không xảy ra trong thực tế vì mỗi bid đến qua network với độ
   * trễ ms.
   */
  ...
  public boolean tryLock(String auctionId, long timeout, TimeUnit unit) {
    ...
    try {
      ...
      if (!acquired) {
        log.warn(
            "tryLock TIMEOUT: auctionId={} timeout={}{}",
            auctionId,
            timeout,
  ...
  /**
   * Xóa lock khi phiên đấu giá đã kết thúc (tránh memory leak). Gọi từ AuctionTimerService sau khi
   * closeAuction.
   */
  ...
  /**
   * Xóa toàn bộ lock khỏi registry. <b>CHỈ dùng trong unit/integration test để reset state giữa các
   * test.</b> Không bao giờ gọi trong production code.
   */
