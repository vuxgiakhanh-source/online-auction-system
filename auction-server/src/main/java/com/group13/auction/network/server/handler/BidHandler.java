...
/**
 * Xử lý các packet liên quan đến đấu giá trực tiếp: JOIN_AUCTION, WATCH_AUCTION, LEAVE_AUCTION,
 * PLACE_BID, REGISTER_AUTO_BID, UPDATE_AUTO_BID, CANCEL_AUTO_BID, GET_AUTO_BID_STATUS,
 * GET_BID_HISTORY.
 *
 * <h3>Concurrent Bidding — cơ chế lock:</h3>
 *
 * <p>Mỗi phiên đấu giá có một {@link ReentrantLock} riêng biệt từ {@link AuctionLockRegistry}. Toàn
 * bộ chuỗi validate → update price → trigger auto-bid phải nằm trong cùng 1 critical section để
 * tránh:
 *
 * <ul>
 *   <li>Lost update (hai thread cùng pass validate với cùng currentPrice)
 *   <li>Hai người cùng thắng
 *   <li>Giá bị rollback do race condition
 * </ul>
 *
 * <h3>Auto-Bid trigger chain:</h3>
 *
 * <p>Sau mỗi {@code placeBid()} thành công, {@link AutoBidProcessor} được gọi bên trong lock để
 * trigger counter-bid cho những người bị vượt qua. Chuỗi tiếp tục cho đến khi không còn ai có thể
 * counter hoặc chỉ còn 1 leader.
 *
 * <h3>Anti-sniping:</h3>
 *
 * <p>Xử lý trong {@link BidService#placeBid()} — nếu có bid trong X giây cuối thì phiên được gia
 * hạn thêm Y giây. {@code newEndTime} được nhúng vào {@code BidUpdateDTO} broadcast ngay.
 *
 * <p>FIX Bug #1: Xóa field bidTransactionDAO vì BidService đã tự persist qua
 * bidTransactionDAO.saveTransaction(tx) bên trong placeBid(). Handler không cần và không nên truy
 * cập DAO layer trực tiếp.
 */
public class BidHandler implements PacketHandler {
   ...
   */
  public BidHandler(
      BidService bidService, IRatingService ratingService, SessionManager sessionManager) {
  ...
  public void handle(
    ...
    if (!session.isAuthenticated()) {
      log.warn(
          "Reject bid packet from unauthenticated session: type={}, requestId={}", type, requestId);
      session.send(
          Packet.of(
  ...
  private void handleJoin(ClientSession session, JsonElement payload, String requestId) {
    try {
      ...
      log.info(
          "Join auction handled: auctionId={}, bidderId={}, username={}, requestId={}",
          auctionId,
          bidder.getId(),
          bidder.getUsername(),
          requestId);
      ...
      AuctionDTOs.AuctionDTO auctionDto =
          DTOMapper.toAuctionDTO(auction, bidder); // viewerCount từ auction.getViewerCount()
      ...
      log.warn(
          "Join auction rejected: username={}, requestId={}, reason={}",
          session.getUsername(),
          requestId,
          e.getReason());
      session.send(
      ...
      log.error(
          "Join auction failed: username={}, requestId={}", session.getUsername(), requestId, e);
      session.send(
          Packet.of(
              PacketType.JOIN_AUCTION_FAILED,
  ...
  private void handleWatch(ClientSession session, JsonElement payload, String requestId) {
    try {
      ...
      sessionManager.addAuctionWatcher(session.getConnection(), auctionId);
      log.info(
          "Watch auction handled: auctionId={}, userId={}, username={}, requestId={}",
          auctionId,
          user.getId(),
          user.getUsername(),
          requestId);
      ...
      log.error(
          "Watch auction failed: username={}, requestId={}", session.getUsername(), requestId, e);
      session.send(
          Packet.of(
  ...
  private void handleLeave(ClientSession session, JsonElement payload, String requestId) {
    ...
    if (bidder != null) {
      ...
      log.info(
          "Leave auction handled: auctionId={}, username={}, bidderId={}, leaderChanged={},"
              + " requestId={}",
          auctionId,
          session.getUsername(),
          bidder.getId(),
      ...
      if (result.extendedForAntiSniping && auction != null && auction.getEndTime() != null) {
        ...
        log.info(
            "Anti-sniping on leader leave — extension broadcast: auctionId={}, requestId={}",
            auctionId,
            requestId);
      }
      ...
      log.info(
          "Leave auction handled (non-normal user): auctionId={}, username={}, requestId={}",
          auctionId,
          session.getUsername(),
          requestId);
  ...
  private void handlePlaceBid(ClientSession session, JsonElement payload, String requestId) {
    ...
    try {
      ...
      log.warn(
          "Invalid place bid payload: username={}, requestId={}",
          session.getUsername(),
          requestId,
          e);
      session.send(
          Packet.of(
              PacketType.PLACE_BID_FAILED,
    ...
    if (req.getAmount() <= 0 || req.getAmount() < 1_000) {
      session.send(
          Packet.of(
              PacketType.PLACE_BID_FAILED,
              ErrorDTO.of(ErrorDTO.VALIDATION_ERROR, "Số tiền đặt giá không hợp lệ.", requestId),
              requestId));
      return;
    }
      ...
      session.send(
          Packet.of(
              ...
              ErrorDTO.of(
                  ...
                  "Số tiền đặt giá vượt quá giới hạn cho phép.",
                  requestId),
              requestId));
      return;
    }
    ...
    if (rateLimitUserId != null && !rateLimiter.tryConsume(rateLimitUserId)) {
      session.send(
          Packet.of(
              ...
              ErrorDTO.of(
                  "RATE_LIMIT_EXCEEDED",
                  "Bạn đang đặt giá quá nhanh. Vui lòng thử lại sau 1 giây.",
                  requestId),
              requestId));
      return;
    }
    ...
    if (rejectIfNotLiveParticipation(session, auction, requestId, PacketType.PLACE_BID_FAILED)) {
      return;
    }
    ...
    try {
      ...
      log.info(
          "Place bid handled: auctionId={}, bidderId={}, username={}, amount={}, requestId={}",
          req.getAuctionId(),
          bidder.getId(),
          bidder.getUsername(),
          req.getAmount(),
      ...
      log.warn(
          "Place bid rejected — auction closed: auctionId={}, username={}, requestId={}",
          req.getAuctionId(),
          session.getUsername(),
          requestId);
      session.send(
      ...
      log.warn(
          "Place bid rejected — invalid amount: auctionId={}, username={}, amount={}, requestId={}",
          req.getAuctionId(),
          session.getUsername(),
          req.getAmount(),
          requestId);
      ...
      log.warn(
          "Place bid rejected — business rule: auctionId={}, username={}, amount={}, requestId={},"
              + " reason={}",
          req.getAuctionId(),
          session.getUsername(),
          req.getAmount(),
      ...
      log.error(
          "Place bid failed: auctionId={}, username={}, amount={}, requestId={}",
          req.getAuctionId(),
          session.getUsername(),
          req.getAmount(),
          requestId,
    ...
    if (extended) {
      ...
      extDto.setExtendedBySeconds(60);
      sessionManager.broadcastToAuctionAsync(
          req.getAuctionId(), Packet.of(PacketType.AUCTION_EXTENDED_NOTIFY, extDto));
      log.info(
          "Auction extension broadcast queued: auctionId={}, requestId={}",
          req.getAuctionId(),
          requestId);
    }
    ...
    sessionManager.broadcastToAuctionAsync(
        ...
        Packet.of(
            ...
            DTOMapper.toBidChartPoint(
                req.getAuctionId(), confirmedAmount, bidder.getUsername(), false)));
  ...
  private void handleRegisterAutoBid(ClientSession session, JsonElement payload, String requestId) {
    ...
    try {
      ...
      log.warn(
          "Invalid register auto-bid payload: username={}, requestId={}",
          session.getUsername(),
          requestId,
          e);
      session.send(
    ...
    try {
      ...
      log.info(
          "Auto-bid registered: auctionId={}, bidderId={}, username={}, maxBid={}, firstBid={}",
          req.getAuctionId(),
          bidder.getId(),
          bidder.getUsername(),
          req.getMaxBid(),
      ...
      if (isAlreadyLeader) {
        log.info(
            "Auto-bid registered without initial bid (user is already leader): "
      ...
      log.warn(
          "Register auto-bid rejected by business rule: auctionId={}, username={}, requestId={},"
              + " reason={}",
          req.getAuctionId(),
          session.getUsername(),
          requestId,
      ...
      log.error(
          "Register auto-bid failed: auctionId={}, username={}, requestId={}",
          req.getAuctionId(),
          session.getUsername(),
          requestId,
          e);
  ...
  private void handleUpdateAutoBid(ClientSession session, JsonElement payload, String requestId) {
    ...
    try {
      ...
      log.warn(
          "Invalid update auto-bid payload: username={}, requestId={}",
          session.getUsername(),
          requestId,
          e);
      session.send(
    ...
    try {
      ...
      log.info(
          "Auto-bid updated: auctionId={}, bidderId={}, username={}, oldMaxBid={}, newMaxBid={}",
          req.getAuctionId(),
          bidder.getId(),
          bidder.getUsername(),
          oldMaxBid,
      ...
      log.error(
          "Update auto-bid failed: auctionId={}, username={}, requestId={}",
          req.getAuctionId(),
          session.getUsername(),
          requestId,
          e);
  ...
  private void handleCancelAutoBid(ClientSession session, JsonElement payload, String requestId) {
    try {
      ...
      try {
        ...
        if (!cancelled) {
          log.warn(
              "Cancel auto-bid requested but entry does not exist: auctionId={}, bidderId={},"
                  + " username={}",
              auctionId,
              bidder.getId(),
              bidder.getUsername());
        ...
        log.info(
            "Auto-bid cancel handled: auctionId={}, bidderId={}, username={}, cancelled={}",
            auctionId,
            bidder.getId(),
            bidder.getUsername(),
            cancelled);
      ...
      log.error(
          "Cancel auto-bid failed: username={}, requestId={}", session.getUsername(), requestId, e);
      session.send(
          Packet.of(
  ...
  private void handleGetAutoBidStatus(
    ...
    try {
      ...
      if (entry != null) {
        ...
        boolean isStale = auction != null && entry.calculateNextBid(currentPrice) < 0;
        ...
        if (isStale) {
          ...
          log.info(
              "auto-bid stale detected on status load: userId={} auctionId={} maxBid={}"
                  + " currentPrice={}",
              bidder.getId(),
              auctionId,
              entry.getMaxBid(),
  ...
  private Auction requireAuction(ClientSession session, String auctionId, String requestId) {
    ...
    if (auction == null) {
      log.warn(
          "Auction not found while handling bid request: auctionId={}, requestId={}",
          auctionId,
          requestId);
      session.send(
          Packet.of(
              PacketType.SYSTEM_ERROR,
              ErrorDTO.of(ErrorDTO.AUCTION_NOT_FOUND, "Không tìm thấy phiên đấu giá.", requestId),
              requestId));
    }
    return auction;
  }
  ...
  private boolean rejectIfNotLiveParticipation(
      ClientSession session, Auction auction, String requestId, PacketType failureType) {
  ...
  private NormalUser requireNormalUser(ClientSession session, String requestId) {
    ...
    if (!(user instanceof NormalUser)) {
      log.warn(
          "NormalUser required but session user is invalid: username={}, requestId={}",
          session.getUsername(),
          requestId);
      session.send(
          Packet.of(
