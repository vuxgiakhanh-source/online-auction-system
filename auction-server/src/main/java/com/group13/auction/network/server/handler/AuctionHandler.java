...
public class AuctionHandler implements PacketHandler {
  ...
  public void handle(
    ...
    if (!session.isAuthenticated()) {
      log.warn(
          "Reject auction packet from unauthenticated session: type={}, requestId={}",
          type,
          requestId);
      session.send(
          Packet.of(
    ...
    switch (type) {
      ...
      case SEARCH_ITEMS -> handleSearchItems(session, payload, requestId);
      default -> {}
    }
  }
  ...
  private void handleCreate(ClientSession session, JsonElement payload, String requestId) {
    ...
    try {
      AuctionDTOs.CreateAuctionRequestDTO req =
          PacketCodec.fromElement(payload, AuctionDTOs.CreateAuctionRequestDTO.class);
        ...
        session.send(
            Packet.of(
                ...
                ErrorDTO.of(
                    ...
                    "Tài khoản chưa được phê duyệt làm người bán.",
                    requestId),
                requestId));
        return;
      }
      ...
      for (String url : imageUrls) {
        if (url == null
          ...
          session.send(
              Packet.of(
                  PacketType.CREATE_AUCTION_FAILED,
                  ErrorDTO.of(ErrorDTO.VALIDATION_ERROR, "URL ảnh không hợp lệ: " + url, requestId),
                  requestId));
          return;
        }
      }
      ...
      Auction auction =
          auctionService.createAuction(
              seller, item, req.getStartTime(), req.getEndTime(), (long) req.getReservePrice());

      session.send(
          Packet.of(PacketType.CREATE_AUCTION_SUCCESS, DTOMapper.toAuctionDTO(auction), requestId));
      log.info(
          "Create auction: auctionId={}, sellerId={}, images={}, requestId={}",
          auction.getId(),
          seller.getId(),
          imageUrls.size(),
          requestId);
      ...
      log.warn(
          "Create auction rejected: username={}, requestId={}, reason={}",
          session.getUsername(),
          requestId,
          e.getMessage());
      session.send(
      ...
      rollbackOrphanItem(savedItemId);
      log.error(
          "Create auction failed: username={}, requestId={}", session.getUsername(), requestId, e);
      session.send(
          Packet.of(
              PacketType.CREATE_AUCTION_FAILED,
  ...
  private void handleGetList(ClientSession session, JsonElement payload, String requestId) {
    try {
      ...
      if ("ALL".equals(scope) || resolvedUser == null) {
        ...
        int pageSize = (req.getPageSize() <= 0 || req.getPageSize() > 100) ? 20 : req.getPageSize();

        String userId = resolvedUser.getId();
        String sellerId = userId; // OWNED dùng chính userId làm sellerId

        List<Auction> auctions =
            auctionDAO.findByScope(userId, sellerId, scope, req.getStatusFilter(), page, pageSize);
      ...
      session.send(
          Packet.of(
              ...
              new AuctionDTOs.AuctionListDTO(dtos, dtos.size()),
              requestId));
      log.debug(
          "Auction list returned: username={}, scope={}, count={}, requestId={}",
          session.getUsername(),
          scope,
          dtos.size(),
          requestId);
      ...
      log.error(
          "Get auction list failed: username={}, requestId={}",
          session.getUsername(),
          requestId,
          e);
      session.send(
    ...
    try {
      ...
      session.send(
          Packet.of(
              ...
              DTOMapper.toAuctionDTO(auction, requestingUser),
              requestId));
    } catch (Exception e) {
      log.error(
          "Get auction detail failed: username={}, requestId={}",
          session.getUsername(),
          requestId,
          e);
      session.send(
  ...
  private void handleUpdate(ClientSession session, JsonElement payload, String requestId) {
    try {
      AuctionDTOs.UpdateAuctionDTO req =
          PacketCodec.fromElement(payload, AuctionDTOs.UpdateAuctionDTO.class);
      NormalUser seller = requireNormalUser(session, requestId);
      if (seller == null) {
        return;
      }
      ...
      if (auction == null || auction.getStatus() != Auction.AuctionStatus.OPEN) {
        session.send(
            Packet.of(
                ...
                ErrorDTO.of(
                    ErrorDTO.VALIDATION_ERROR, "Phiên không tồn tại hoặc đã bắt đầu.", requestId),
                requestId));
        return;
      }
      ...
      session.send(
          Packet.of(PacketType.UPDATE_AUCTION_SUCCESS, DTOMapper.toAuctionDTO(auction), requestId));
      log.info(
          "Update auction: auctionId={}, sellerId={}, requestId={}",
          auction.getId(),
          seller.getId(),
          requestId);
    } catch (Exception e) {
      log.error(
          "Update auction failed: username={}, requestId={}", session.getUsername(), requestId, e);
      session.send(
          Packet.of(
              PacketType.UPDATE_AUCTION_FAILED,
              ErrorDTO.of(ErrorDTO.INTERNAL_ERROR, e.getMessage(), requestId),
  ...
  private void handleCancelRequest(ClientSession session, JsonElement payload, String requestId) {
    try {
      AuctionDTOs.CancelAuctionRequestDTO req =
          PacketCodec.fromElement(payload, AuctionDTOs.CancelAuctionRequestDTO.class);
      NormalUser seller = requireNormalUser(session, requestId);
      if (seller == null) {
        return;
      }
      ...
      session.send(
          Packet.of(PacketType.CANCEL_AUCTION_REQUEST_SUCCESS, req.getAuctionId(), requestId));
      log.info(
          "Cancel request: auctionId={}, sellerId={}, requestId={}",
          req.getAuctionId(),
          seller.getId(),
          requestId);
    } catch (Exception e) {
      log.error(
          "Cancel auction request failed: username={}, requestId={}",
          session.getUsername(),
          requestId,
          e);
      session.send(
  ...
  private void handleAdminCancel(ClientSession session, JsonElement payload, String requestId) {
    if (!session.isAdmin()) {
      log.warn(
          "Reject admin cancel from non-admin: username={}, requestId={}",
          session.getUsername(),
          requestId);
      return;
    }
    try {
      AuctionDTOs.AdminCancelAuctionDTO req =
          PacketCodec.fromElement(payload, AuctionDTOs.AdminCancelAuctionDTO.class);
      Auction auction = AuctionManager.getInstance().findAuctionById(req.getAuctionId());
      ...
      session.send(
          Packet.of(
              PacketType.ADMIN_CANCEL_AUCTION_SUCCESS, DTOMapper.toAuctionDTO(auction), requestId));
      log.info(
          "Admin cancel: auctionId={}, adminId={}, reason={}, requestId={}",
          req.getAuctionId(),
          admin.getId(),
          req.getReason(),
          requestId);
      ...
      log.error(
          "Admin cancel failed: username={}, requestId={}", session.getUsername(), requestId, e);
      session.send(
          Packet.of(
  ...
  private void handleAdminGetAll(ClientSession session, JsonElement payload, String requestId) {
    if (!session.isAdmin()) {
      log.warn(
          "Reject admin get all from non-admin: username={}, requestId={}",
          session.getUsername(),
          requestId);
      return;
    }
   ...
   *
   * <ol>
   *   <li>Parse {@link SearchDTOs.ItemSearchRequestDTO} từ payload.
   *   <li>Validate keyword không rỗng.
   *   <li>Query DB qua {@code auctionDAO.searchByItemName()} (LIKE không phân biệt hoa thường, phân
   *       trang).
   *   <li>Đếm tổng bản ghi qua {@code auctionDAO.countByItemName()} để tính totalPages.
   *   <li>Trả về {@link SearchDTOs.ItemSearchResponseDTO} kèm packet {@code SEARCH_ITEMS_SUCCESS}.
   * </ol>
   *
   * <p>Không yêu cầu quyền đặc biệt — mọi user đã đăng nhập đều tìm kiếm được.
   */
  private void handleSearchItems(ClientSession session, JsonElement payload, String requestId) {
    try {
      SearchDTOs.ItemSearchRequestDTO req =
          PacketCodec.fromElement(payload, SearchDTOs.ItemSearchRequestDTO.class);
      ...
      if (req.getKeyword() == null || req.getKeyword().isBlank()) {
        session.send(
            Packet.of(
                ...
                ErrorDTO.of(
                    ErrorDTO.VALIDATION_ERROR, "Từ khóa tìm kiếm không được để trống.", requestId),
                requestId));
        return;
      }
      ...
      if ("ALL".equals(scope) || userId == null) {
        ...
        auctions =
            auctionDAO.searchByItemName(keyword, page, size, req.getSortBy(), req.getSortDir());
        totalElements = auctionDAO.countByItemName(keyword);
      } else {
        // Lọc kết hợp keyword + scope
        auctions =
            auctionDAO.searchByItemNameAndScope(
                keyword, userId, sellerId, scope, page, size, req.getSortBy(), req.getSortDir());
        totalElements = auctionDAO.countByItemNameAndScope(keyword, userId, sellerId, scope);
      }
      ...
      SearchDTOs.ItemSearchResponseDTO response =
          new SearchDTOs.ItemSearchResponseDTO(dtos, totalElements, page, size, keyword);

      session.send(Packet.of(PacketType.SEARCH_ITEMS_SUCCESS, response, requestId));
      log.info(
          "Search items: username={}, keyword='{}', scope={}, page={}, size={}, found={}, total={}",
          session.getUsername(),
          keyword,
          scope,
          page,
      ...
      log.error(
          "Search items failed: username={}, requestId={}", session.getUsername(), requestId, e);
      session.send(
          Packet.of(
              PacketType.SEARCH_ITEMS_FAILED,
              ErrorDTO.of(
                  ErrorDTO.INTERNAL_ERROR, "Tìm kiếm thất bại, vui lòng thử lại.", requestId),
              requestId));
    }
  }
