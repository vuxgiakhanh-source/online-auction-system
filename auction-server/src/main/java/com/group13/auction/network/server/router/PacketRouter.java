 ...
 *
 * <p>Mỗi message đến đều được xử lý đồng bộ trong thread của java-websocket. Handler nặng nên dùng
 * ExecutorService riêng nếu cần async — nhưng với đấu giá realtime, xử lý tuần tự per-session là
 * đủ.
 */
  ...
  public void route(ClientSession session, String message) {
    ...
    try {
      ...
      com.google.gson.JsonObject obj =
          com.google.gson.JsonParser.parseString(message).getAsJsonObject();
      requestId =
      ...
      log.warn(
          "Malformed packet: username={}, messageLength={}",
          session.getUsername(),
      ...
      session.send(
          Packet.of(
              ...
              ErrorDTO.of(ErrorDTO.VALIDATION_ERROR, "Packet không hợp lệ: " + e.getMessage())));
      return;
    }
      ...
      if (handler.supports(type)) {
        ...
        try {
          if (payload != null && payload.isJsonObject()) {
            ...
            String auctionId =
                p.has("auctionId") && !p.get("auctionId").isJsonNull()
                    ? p.get("auctionId").getAsString()
                    : null;
            if (auctionId != null) {
          ...
          /* payload không phải object — không cần auctionId */
        }
        try {
          ...
          log.error(
              "Unhandled exception while routing packet: type={}, username={}, requestId={}",
              type,
              session.getUsername(),
              requestId,
              e);
          session.send(
              Packet.of(
                  PacketType.SYSTEM_ERROR,
                  ErrorDTO.of(ErrorDTO.INTERNAL_ERROR, "Lỗi hệ thống khi xử lý " + type, requestId),
                  requestId));
        } finally {
    ...
    log.warn(
        "No handler registered for packet type: type={}, username={}, requestId={}",
        type,
        session.getUsername(),
        requestId);
    session.send(
        Packet.of(
            PacketType.SYSTEM_ERROR,
            ErrorDTO.of(
                ErrorDTO.VALIDATION_ERROR, "PacketType không được hỗ trợ: " + type, requestId),
            requestId));
  }
}
