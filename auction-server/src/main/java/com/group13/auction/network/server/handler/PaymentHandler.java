...
/**
 * Xử lý các packet liên quan đến thanh toán: DEPOSIT, WITHDRAW, GET_WALLET_BALANCE,
...
public class PaymentHandler implements PacketHandler {
  ...
  public void handle(
    ...
    if (!session.isAuthenticated()) {
      log.warn(
          "Reject payment packet from unauthenticated session: type={}, requestId={}",
          type,
          requestId);
      session.send(
          Packet.of(
  ...
  private void handleDeposit(ClientSession session, JsonElement payload, String requestId) {
    try {
      PaymentDTOs.DepositRequestDTO req =
          PacketCodec.fromElement(payload, PaymentDTOs.DepositRequestDTO.class);
      ...
      log.info(
          "Deposit handled: userId={}, username={}, amount={}, requestId={}",
          user.getId(),
          user.getUsername(),
          req.getAmount(),
          requestId);
      ...
      PaymentDTOs.WalletBalanceResponseDTO resp =
          new PaymentDTOs.WalletBalanceResponseDTO(
      ...
      log.warn(
          "Deposit rejected: username={}, requestId={}, reason={}",
          session.getUsername(),
          requestId,
          e.getMessage());
      session.send(
          Packet.of(
              PacketType.DEPOSIT_FAILED,
              ErrorDTO.of(ErrorDTO.INVALID_AMOUNT, e.getMessage(), requestId),
              requestId));
    } catch (Exception e) {
      log.error("Deposit failed: username={}, requestId={}", session.getUsername(), requestId, e);
      session.send(
          Packet.of(
              PacketType.DEPOSIT_FAILED,
  ...
  private void handleWithdraw(ClientSession session, JsonElement payload, String requestId) {
    try {
      PaymentDTOs.WithdrawRequestDTO req =
          PacketCodec.fromElement(payload, PaymentDTOs.WithdrawRequestDTO.class);
      ...
      log.info(
          "Withdraw handled: userId={}, username={}, amount={}, requestId={}",
          user.getId(),
          user.getUsername(),
          req.getAmount(),
          requestId);
      ...
      log.warn(
          "Withdraw rejected: username={}, requestId={}, reason={}",
          session.getUsername(),
          requestId,
          e.getMessage());
      session.send(
          Packet.of(
              PacketType.WITHDRAW_FAILED,
              ErrorDTO.of(ErrorDTO.INSUFFICIENT_BALANCE, e.getMessage(), requestId),
              requestId));
    } catch (Exception e) {
      log.error("Withdraw failed: username={}, requestId={}", session.getUsername(), requestId, e);
      session.send(
          Packet.of(
              PacketType.WITHDRAW_FAILED,
              ErrorDTO.of(ErrorDTO.INTERNAL_ERROR, e.getMessage(), requestId),
  ...
  private void handleGetBalance(ClientSession session, String requestId) {
    try {
      ...
      log.debug(
          "Wallet balance returned: userId={}, username={}, requestId={}",
          user.getId(),
          user.getUsername(),
          requestId);
    } catch (Exception e) {
      log.error(
          "Get wallet balance failed: username={}, requestId={}",
          session.getUsername(),
          requestId,
          e);
      session.send(
  ...
  private void handleConfirmItemReceived(
    ...
    try {
      ...
      com.group13.auction.model.user.NormalUser winner = requireNormalUser(session, requestId);
      if (winner == null) {
        return;
      }
      ...
      if (auctionWinner.getPaymentStatus()
          != com.group13.auction.model.auction.AuctionWinner.PaymentStatus.FUNDS_HELD) {
        session.send(
            Packet.of(
                PacketType.CONFIRM_ITEM_RECEIVED_FAILED,
                com.group13.auction.common.dto.core.ErrorDTO.of(
      ...
      log.info(
          "Item received confirmed: auctionId={}, winnerId={}, requestId={}",
          auctionId,
          winner.getId(),
          requestId);
      ...
      log.error(
          "Confirm item received failed: username={}, requestId={}",
          session.getUsername(),
          requestId,
          e);
      session.send(
  ...
  private void handlePayment(ClientSession session, JsonElement payload, String requestId) {
    try {
      PaymentDTOs.PaymentRequestDTO req =
          PacketCodec.fromElement(payload, PaymentDTOs.PaymentRequestDTO.class);

      Auction auction = AuctionManager.getInstance().findAuctionById(req.getAuctionId());
      if (auction == null) {
        log.warn(
            "Payment rejected because auction was not found: auctionId={}, username={},"
                + " requestId={}",
            req.getAuctionId(),
            session.getUsername(),
            requestId);
      ...
      if (auctionWinner == null) {
        log.warn(
            "Payment rejected because auction has no winner: auctionId={}, username={},"
                + " requestId={}",
            req.getAuctionId(),
            session.getUsername(),
            requestId);
      ...
      if (!caller.getId().equals(auctionWinner.getWinner().getId())) {
        log.warn(
            "Payment rejected because caller is not winner: auctionId={}, callerId={}, winnerId={},"
                + " requestId={}",
            req.getAuctionId(),
            caller.getId(),
            auctionWinner.getWinner().getId(),
            requestId);
        session.send(
            Packet.of(
                PacketType.PAYMENT_FAILED,
                ErrorDTO.of(
                    ErrorDTO.UNAUTHORIZED, "Chỉ winner của phiên mới được thanh toán.", requestId),
                requestId));
        return;
      }

      boolean locked =
          lockRegistry.tryLock(auction.getId(), AUCTION_LOCK_TIMEOUT_SECONDS, TimeUnit.SECONDS);
      if (!locked) {
        ...
        session.send(
            Packet.of(
                ...
                ErrorDTO.of(
                    ...
                    "Hệ thống đang xử lý phiên này, vui lòng thử lại.",
                    requestId),
                requestId));
        return;
      }
      try {
        ...
        log.info(
            "Payment request handled: auctionId={}, winnerId={}, username={}, finalPrice={},"
                + " requestId={}",
            req.getAuctionId(),
            caller.getId(),
            caller.getUsername(),
        ...
        sessionManager.broadcastToAuction(
            req.getAuctionId(), Packet.of(PacketType.AUCTION_ENDED_UPDATE, update));
      } finally {
      ...
      log.warn(
          "Payment rejected: username={}, requestId={}, reason={}",
          session.getUsername(),
          requestId,
          e.getReason());
      session.send(
      ...
      log.error("Payment failed: username={}, requestId={}", session.getUsername(), requestId, e);
      session.send(
          Packet.of(
              PacketType.PAYMENT_FAILED,
              ErrorDTO.of(ErrorDTO.INTERNAL_ERROR, e.getMessage(), requestId),
  ...
  private void handleSecondChanceAccept(
    ...
    try {
      ...
      if (auction == null) {
        log.warn(
            "Second chance accept rejected because auction was not found: auctionId={},"
                + " username={}, requestId={}",
            auctionId,
            session.getUsername(),
            requestId);
      ...
      if (offer == null) {
        log.warn(
            "Second chance accept rejected because offer was not found: auctionId={}, username={},"
                + " requestId={}",
            auctionId,
            session.getUsername(),
            requestId);
        session.send(
            Packet.of(
                ...
                ErrorDTO.of(
                    ...
                    "Không tìm thấy Second Chance Offer PENDING cho phiên này.",
                    requestId),
                requestId));
        return;
      }
      ...
      if (!caller.getId().equals(offer.getRunnerUp().getId())) {
        log.warn(
            "Second chance accept rejected because caller is not runner-up: auctionId={},"
                + " callerId={}, runnerUpId={}, requestId={}",
            auctionId,
            caller.getId(),
            offer.getRunnerUp().getId(),
            requestId);
        session.send(
            Packet.of(
                PacketType.SECOND_CHANCE_ACCEPT_FAILED,
                ErrorDTO.of(
                    ErrorDTO.UNAUTHORIZED, "Bạn không phải runner-up của offer này.", requestId),
                requestId));
        return;
      }

      boolean locked =
          lockRegistry.tryLock(auctionId, AUCTION_LOCK_TIMEOUT_SECONDS, TimeUnit.SECONDS);
      if (!locked) {
        session.send(
            Packet.of(
                PacketType.SECOND_CHANCE_ACCEPT_FAILED,
      ...
      try {
        ...
        log.info(
            "Second chance accept handled: auctionId={}, offerId={}, runnerUpId={}, requestId={}",
            auctionId,
            offer.getId(),
            caller.getId(),
            requestId);
      ...
      log.error(
          "Second chance accept failed: username={}, requestId={}",
          session.getUsername(),
          requestId,
          e);
      session.send(
  ...
  private void handleSecondChanceDecline(
    ...
    try {
      ...
      if (auction == null) {
        log.warn(
            "Second chance decline rejected because auction was not found: auctionId={},"
                + " username={}, requestId={}",
            auctionId,
            session.getUsername(),
            requestId);
      ...
      if (offer == null) {
        log.warn(
            "Second chance decline rejected because offer was not found: auctionId={}, username={},"
                + " requestId={}",
            auctionId,
            session.getUsername(),
            requestId);
        session.send(
            Packet.of(
                ...
                ErrorDTO.of(
                    ErrorDTO.VALIDATION_ERROR,
                    "Không tìm thấy Second Chance Offer PENDING.",
                    requestId),
                requestId));
        return;
      }
      ...
      if (!caller.getId().equals(offer.getRunnerUp().getId())) {
        log.warn(
            "Second chance decline rejected because caller is not runner-up: auctionId={},"
                + " callerId={}, runnerUpId={}, requestId={}",
            auctionId,
            caller.getId(),
            offer.getRunnerUp().getId(),
      ...
      try {
        ...
        log.info(
            "Second chance decline handled: auctionId={}, offerId={}, runnerUpId={}, requestId={}",
            auctionId,
            offer.getId(),
            caller.getId(),
            requestId);
      ...
      log.error(
          "Second chance decline failed: username={}, requestId={}",
          session.getUsername(),
          requestId,
          e);
      session.send(
  ...
  /**
   * Lấy AuctionWinner từ in-memory. Nếu null (server vừa restart), thử lazy-restore từ DB rồi gán
   * lại vào auction để các request sau không cần query DB nữa.
   *
  ...
  private AuctionWinner resolveWinner(Auction auction) {
    ...
    log.warn(
        "Winner null in-memory, attempting lazy restore from DB: auctionId={}, status={}",
        auction.getId(),
        auction.getStatus());
    try {
      ...
      if (winner != null) {
        ...
        log.info(
            "Lazy winner restore success: auctionId={}, winnerId={}",
            auction.getId(),
            winner.getWinner().getId());
      } else {
        log.error(
            "No winner found in DB either: auctionId={} — data inconsistency", auction.getId());
      }
    } catch (Exception e) {
