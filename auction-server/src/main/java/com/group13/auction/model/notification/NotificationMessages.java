...
public final class NotificationMessages {
    ...
    public static String outbidBody(Auction auction, String newBidderName,
                                    long newAmount, long yourAmount) {
        return String.format(
            "Phiên \"%s\" (mã %s): %s đặt giá %s, cao hơn mức %s của bạn. "
                + "Hãy đặt giá cao hơn nếu bạn vẫn muốn thắng.",
            itemName(auction), auction.getId(), newBidderName,
    ...
    public static String auctionEndingSoonBody(Auction auction, int minutesLeft) {
        String endText = auction.getEndTime() != null
            ? auction.getEndTime().format(DEADLINE_FMT)
            : "—";
        return String.format(
            "Phiên \"%s\" (mã %s) sẽ kết thúc sau khoảng %d phút (dự kiến lúc %s). "
                + "Giá hiện tại: %s. Hãy kiểm tra và đặt giá kịp thời nếu cần.",
            itemName(auction), auction.getId(), minutesLeft, endText,
    ...
    public static String scoReceivedBody(Auction auction, long offerPrice, long depositRequired,
        ...
        return String.format(
            ...
            formatVnd(depositRequired), deadline.format(DEADLINE_FMT));
    }
    ...
    public static String scoSentToSellerBody(Auction auction, String runnerUpName,
                                             long offerPrice, LocalDateTime deadline) {
        return String.format(
            "Winner không thanh toán. Hệ thống đã gửi đề nghị mua thứ cấp cho %s — sản phẩm \"%s\" "
                + "(mã %s), giá %s, hạn phản hồi %s. Bạn sẽ được thông báo khi họ chấp nhận hoặc từ chối.",
            runnerUpName, itemName(auction), auction.getId(),
            formatVnd(offerPrice), deadline.format(DEADLINE_FMT));
    }
    ...
    public static String scoAcceptedSellerBody(Auction auction, String runnerUpName, long offerPrice) {
        return String.format(
    ...
    public static String paymentSuccessWinnerBody(Auction auction, long finalPrice, long depositPaid) {
        ...
        return String.format(
            ...
            itemName(auction), auction.getId(), formatVnd(finalPrice),
            formatVnd(depositPaid), formatVnd(remaining));
    }
    ...
    public static String paymentSuccessSellerBody(Auction auction, String buyerName, long finalPrice) {
        return String.format(
    ...
    public static String sellerMessageBody(Auction auction, String sellerName, String messagePreview) {
    ...
    public static String formatDeadline(SecondChanceOffer offer) {
        return offer.getDeadline() != null
            ? offer.getDeadline().format(DEADLINE_FMT)
            : "—";
    }
