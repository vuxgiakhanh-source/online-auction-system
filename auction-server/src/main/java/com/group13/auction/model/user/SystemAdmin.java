 ...
 *
 * <ul>
 *   <li>Logging chuẩn SLF4J (xóa System.out/err.println).
 *   <li>printInfo() dùng log.info thay vì System.out.
 * </ul>
 *
 * <h3>Fix v3:</h3>
 *
 * <ul>
 *   <li>bootstrap() kiểm tra sự tồn tại của SYSTEM trong bảng {@code admins} (thay vì bảng {@code
 *       users}) để tránh Duplicate entry khi restart.
 * </ul>
 *
 * <h3>Fix v4 (Qodana):</h3>
 *
 * <ul>
 *   <li>[Non-distinguishable logging] Hai log.error trong autoBanIfNeeded() và banUserByStaff() có
 *       message giống nhau, Qodana không phân biệt được nguồn. Đã thêm context "source=autoBan" /
 *       "source=staffBan" để phân biệt.
 * </ul>
 */
public class SystemAdmin extends Admin {
    ...
    if (user.getRating() < MIN_ELIGIBLE_RATING) {
      ...
      if (userDAO != null) {
        ...
        if (!updated) {
          ...
          log.error(
              "Không thể persist ban cho user: username={} source=autoBan", user.getUsername());
        } else if (accountBanDAO != null) {
          accountBanDAO.insertBan(user.getId(), null, "SYSTEM", BanReason.SYSTEM_AUTO.name(), null);
        }
      }
  ...
  public void banUserByStaff(Admin staff, User user, Admin.BanReason reason) {
    ...
    String staffLog =
        String.format(
            "[STAFF BAN] %s ban %s | Lý do: %s", staff.getUsername(), user.getUsername(), reason);
    staff.addActionLog(staffLog);
    log.info(
        "STAFF BAN: staff={} target={} reason={}", staff.getUsername(), user.getUsername(), reason);

    String auditLog =
        String.format(
            "[AUDIT] Staff %s ban %s | Lý do: %s", staff.getUsername(), user.getUsername(), reason);
    ...
    if (userDAO != null) {
      ...
      if (!updated) {
        ...
        log.error(
            "Không thể persist ban cho user: username={} source=staffBan staff={}",
            user.getUsername(),
            staff.getUsername());
      } else if (accountBanDAO != null) {
        accountBanDAO.insertBan(
            user.getId(), staff.getId(), staff.getUsername(), reason.name(), null);
      }
    }
  }
  ...
  public void printInfo() {
    log.info(
        "=== SYSTEM ADMIN INFO === username={} email={} level={} actionCount={}",
        getUsername(),
        getEmail(),
        getAdminLevel(),
        getActionLog().size());
