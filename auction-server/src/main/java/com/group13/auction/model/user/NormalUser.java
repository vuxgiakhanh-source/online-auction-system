 ...
 *
 * <p>Khi mới tạo, mặc định có role BIDDER. Muốn trở thành Seller, user phải gửi yêu cầu và được hệ
 * thống phê duyệt qua {@link com.group13.auction.service.AccountService}.
 *
 * <p>Lớp này hỗ trợ dual-role theo yêu cầu nghiệp vụ. BIDDER có state riêng SELLER có state riêng.
 */
public class NormalUser extends User {
  ...
  /**
   * Số lần tài khoản được auto-restore sau khi bị SUSPENDED. Hiển thị trên profile. Tăng mỗi lần
   * restore thành công.
   */
   ...
   *
   * <p>Sau khi gọi reconstitute(), DAO phải inject thêm dữ liệu lịch sử bằng các setter tương ứng.
   * Tất cả các hàm DAO đã được triển khai đầy đủ:
   *
   * <pre>
  ...
  /**
   * Constructor reconstitute: các list lịch sử được khởi tạo rỗng. DAO chịu trách nhiệm nạp dữ liệu
   */
  private NormalUser(
      String id,
      LocalDateTime createdAt,
  ...
  /**
   * Thêm role cho user — chỉ AccountService gọi sau khi phê duyệt. Admin không được addRole thêm
   * (guard ở AccountService).
   */
  @Override
  ...
  /**
   * Kiểm tra tài khoản đã từng được auto-restore chưa. Backward-compatible với RatingService guard.
   */
  ...
  /**
   * Khóa một khoản cọc - gọi khi joinAuction thành công. <= số dư khả dụng. Không được tự ý gọi,
   * chỉ được gọi trong WalletService.
   *
  ...
  /**
   * Giải phóng khoản cọc - gọi khi phiên kết thúc. Không được tự ý gọi, chỉ được gọi trong
   * WalletService.
   *
  ...
  /**
   * Đánh dấu user đã từng bị phạt rating. Chỉ RatingService gọi — dùng để kiểm tra điều kiện duyệt
   * role Seller.
   */
  ...
  /**
   * Tăng số lần restore của tài khoản. Chỉ {@link com.group13.auction.service.RatingService} gọi.
   */
  ...
  /**
   * Inject lịch sử bid từ DB — chỉ UserDAO gọi sau reconstitute(). Đã thực hiện TODO:
   * UserDAO.findBidHistoryByUserId(id) đã được triển khai. Lưu ý: UserDAO.findNormalUserById()
   * không tự inject để tránh đệ quy; caller cần gọi thủ công nếu cần đầy đủ lịch sử bid.
   */
  public void setBidHistory(List<BidTransaction> bidHistory) {
    this.bidHistory =
        bidHistory != null ? new CopyOnWriteArrayList<>(bidHistory) : new CopyOnWriteArrayList<>();
  }
  ...
  /**
   * Inject danh sách auctionId của seller từ DB — chỉ DAO gọi sau reconstitute(). Thiếu setter này
   * khiến allAuctionIds luôn rỗng sau server restart.
   */
  public void setAllAuctionIds(java.util.List<String> auctionIds) {
    this.allAuctionIds =
        auctionIds != null ? new CopyOnWriteArrayList<>(auctionIds) : new CopyOnWriteArrayList<>();
    markUpdated();
  }
   ...
   *
   * <p>Dữ liệu được filter từ {@code allAuctionIds} đã được AuctionManager load vào memory. Trong
   * môi trường thực tế, nên ưu tiên query thẳng DB để tránh load toàn bộ phiên.
   *
   * <p>Đã thực hiện TODO: Hàm {@code AuctionDAO.findUnfinishedAuctionIdsBySellerId(id)} đã được
   * thêm vào AuctionDAO để query trực tiếp DB (WHERE item_id IN (SELECT id FROM items WHERE
   * seller_id = ?) AND status IN ('OPEN','RUNNING')). Nên gọi hàm đó thay vì filter in-memory khi
   * allAuctionIds chưa được inject đủ.
   *
  ...
  public List<String> getUnfinishedAuctionIds(
      java.util.function.Function<String, com.group13.auction.model.auction.Auction>
          auctionLookup) {
