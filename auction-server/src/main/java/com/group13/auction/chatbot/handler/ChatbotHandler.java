 ...
 *
 * <p>Implement {@link PacketHandler} để tích hợp liền mạch vào kiến trúc
 * WebSocket hiện có — được đăng ký vào {@code PacketRouter} trong {@code AuctionWebSocketServer}.
 *
 * <h3>Các PacketType được xử lý:</h3>
 * <ul>
 *   <li>{@code CHATBOT_ASK} — Người dùng hỏi tự do hoặc chọn FAQ theo ID.</li>
 *   <li>{@code CHATBOT_GET_FAQ_LIST} — Lấy danh sách câu hỏi theo category.</li>
 * </ul>
 *
 * <h3>Thiết kế tách biệt nghiệp vụ:</h3>
 * <p>Handler này không chứa logic tìm kiếm — toàn bộ logic tìm kiếm nằm trong
 * {@link ChatbotProvider} (Singleton). Handler chỉ thực hiện:
 * <ol>
 *   <li>Parse payload JSON từ client.</li>
 *   <li>Gọi đúng phương thức của {@code ChatbotProvider}.</li>
 *   <li>Serialize kết quả và gửi về qua {@code ClientSession}.</li>
 * </ol>
 *
 * <p>Chatbot hoạt động được ngay cả khi client chưa đăng nhập (unauthenticated)
 * — phù hợp với vai trò "hỗ trợ khách hàng" không yêu cầu xác thực.
 *
...
public class ChatbotHandler implements PacketHandler {
    ...
    /**
     * Tập PacketType mà handler này chịu trách nhiệm xử lý.
     * Dùng EnumSet để kiểm tra {@code supports()} trong O(1).
     */
    private static final Set<PacketType> SUPPORTED_PACKET_TYPES = EnumSet.of(
            PacketType.CHATBOT_ASK,
            PacketType.CHATBOT_GET_FAQ_LIST
    );

    /**
     * Provider chatbot — truy xuất qua Singleton để tái sử dụng dữ liệu FAQ
     * đã nạp vào bộ nhớ từ lúc server khởi động.
     */
     ...
     *
     * <p>Không nhận tham số nào từ ngoài — chatbot module hoạt động hoàn toàn
     * độc lập (không cần AccountService, AuctionService, v.v.).
     * Đây là điểm thể hiện tính "Microservice style" của module.
     */
    public ChatbotHandler() {
        ...
        this.gson            = new Gson();
        log.info("[ChatbotHandler] Khởi tạo — ChatbotProvider đang giữ {} FAQ.",
     ...
     * <ol>
     *   <li>Nếu payload có trường {@code "faqId"} (VD: "FAQ_001") → tra cứu trực tiếp theo ID
     *       (người dùng chọn từ menu gợi ý).</li>
     *   <li>Nếu payload có trường {@code "query"} → tìm kiếm theo từ khóa tự do.</li>
     *   <li>Nếu payload không hợp lệ → trả về NOT_FOUND với thông báo lỗi.</li>
     * </ol>
     *
     ...
     */
    private void handleChatbotAsk(ClientSession session,
                                  JsonElement payload, String requestId) {
        ...
        PacketType responseType = response.isSuccess()
                ? PacketType.CHATBOT_ANSWER
                : PacketType.CHATBOT_NOT_FOUND;
     ...
     *
     * <p>Frontend dùng endpoint này để hiển thị menu câu hỏi nhanh theo từng chủ đề,
     * giúp người dùng không cần gõ câu hỏi mà chỉ cần click.
     *
     * <h3>Payload JSON từ client:</h3>
     * <pre>{@code
     ...
     * @param requestId ID request để echo
     */
    private void handleGetFaqList(ClientSession session,
                                  JsonElement payload, String requestId) {
        ...
        log.debug("[ChatbotHandler] Trả về {} FAQ thuộc category='{}'",
                faqs.size(), category == null ? "TẤT CẢ" : category);

        // Gửi packet thành công
        @SuppressWarnings("unchecked")
        Packet<JsonObject> responsePacket = new Packet<>(
     ...
     */
    private void sendNotFoundResponse(ClientSession session,
     ...
     */
    private void sendResponse(ClientSession session, PacketType packetType,
                              ChatbotResponse response, String requestId) {
     ...
     *
     * <p>Tránh gửi toàn bộ nội dung answer khi chỉ cần hiển thị menu gợi ý,
     * giúp giảm kích thước packet WebSocket đáng kể.
     *
