 ...
 *
 * <p>Lớp này được serialize thành JSON qua Gson và gửi qua WebSocket trong packet
 * {@code CHATBOT_ANSWER} hoặc {@code CHATBOT_NOT_FOUND}.
 *
 * <p>Sử dụng <strong>Static Factory Methods</strong> thay vì constructor trực tiếp
 * để tạo ra các loại phản hồi khác nhau một cách rõ ràng và dễ đọc:
 * <ul>
 *   <li>{@link #ofSuccess(FAQ)} — tìm thấy câu trả lời</li>
 *   <li>{@link #ofNotFound(String)} — không tìm thấy câu trả lời phù hợp</li>
 *   <li>{@link #ofFaqList(String)} — tiêu đề khi trả danh sách FAQ</li>
 * </ul>
 *
...
public class ChatbotResponse {
     ...
     * <ul>
     *   <li>{@code SUCCESS} — tìm thấy FAQ phù hợp và trả về câu trả lời.</li>
     *   <li>{@code NOT_FOUND} — không tìm thấy FAQ phù hợp với câu hỏi.</li>
     *   <li>{@code FAQ_LIST} — trả về danh sách câu hỏi theo category (không kèm answer).</li>
     * </ul>
     */
    public enum ResponseStatus { SUCCESS, NOT_FOUND, FAQ_LIST }
    ...
    /**
     * Thông điệp phụ dành cho trường hợp NOT_FOUND —
     * gợi ý người dùng liên hệ Admin hoặc thử từ khóa khác.
     */
     ...
     */
    private ChatbotResponse(String faqId, String question, String answer,
                            String category, ResponseStatus status, String fallbackMessage) {
        this.faqId           = faqId;
     ...
     *
     * <p>Kèm thông điệp gợi ý để người dùng không bị bỏ lại một mình —
     * hướng họ liên hệ Admin hoặc thử từ khóa khác.
     *
    ...
    /**
     * Tạo phản hồi tiêu đề khi trả về danh sách FAQ theo category.
     * Thường đi kèm với payload list FAQ riêng trong packet {@code CHATBOT_FAQ_LIST_SUCCESS}.
     *
    ...
    public static ChatbotResponse ofFaqList(String category) {
        ...
        String answer   = "Dưới đây là các câu hỏi phổ biến trong nhóm "
                + cat + ". Chọn mã FAQ (VD: FAQ_001) để xem câu trả lời chi tiết.";
        return new ChatbotResponse(
                null,
                question,
                answer,
                category,
                ResponseStatus.FAQ_LIST,
                null
        );
    }
    ...
    public String toString() {
        return "ChatbotResponse{status=" + status
                + ", faqId='" + faqId + "'"
                + ", category='" + category + "'}";
    }
}