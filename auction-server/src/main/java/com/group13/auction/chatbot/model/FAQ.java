 ...
 *
 * <p>Dữ liệu được đọc từ {@code faq_data.json} thông qua {@link com.group13.auction.chatbot.provider.ChatbotProvider}.
 * Lớp này là immutable sau khi khởi tạo để đảm bảo thread-safety khi nhiều
 * client truy vấn chatbot đồng thời.
 *
 * <p>Mỗi FAQ thuộc một {@code category} (nhóm nghiệp vụ) và có danh sách
 * {@code keywords} giúp thuật toán tìm kiếm so khớp câu hỏi tự do của người dùng.
 *
...
public class FAQ {
    ...
    /**
     * Mã định danh duy nhất của câu hỏi (ví dụ: "FAQ_001").
     * Dùng để tra cứu trực tiếp qua {@code getAnswerByQuestionId()}.
     */
    private String id;

    /**
     * Nhóm nghiệp vụ của câu hỏi.
     * Các giá trị hợp lệ: GENERAL | BIDDING | PAYMENT | RATING | SELLER.
     */
    private String category;

    /**
     * Danh sách từ khóa liên quan, dùng để tìm kiếm câu hỏi phù hợp
     * khi người dùng nhập câu hỏi tự do (free-text query).
     */
    ...
    /**
     * Constructor mặc định không tham số — bắt buộc để Gson deserialize từ JSON.
     * Không dùng trực tiếp trong code ứng dụng.
     */
    public FAQ() {}
     ...
     */
    public FAQ(String id, String category, List<String> keywords,
               String question, String answer) {
        this.id       = id;
        this.category = category;
        this.keywords = keywords != null ? Collections.unmodifiableList(keywords)
     ...
     *
     * <p>Dùng nội bộ trong {@link com.group13.auction.chatbot.provider.ChatbotProvider}
     * để tính điểm phù hợp (relevance score) khi tìm kiếm free-text.
     *
    ...
    public String toString() {
        return "FAQ{id='" + id + "', category='" + category
                + "', question='" + question + "'}";
    }
