 ...
 *
 * <p>Đọc cấu hình từ env vars (Docker/CI) hoặc data.properties (local dev), khởi tạo toàn bộ
 * dependency, start server và AuctionTimerService.
 *
 * <h3>Env vars:</h3>
 *
 * <ul>
 *   <li>{@code SERVER_PORT} — cổng WebSocket (mặc định 8080)
 *   <li>{@code IMAGE_SERVER_PORT} — cổng HTTP upload ảnh (mặc định 8081)
 *   <li>{@code IMAGE_UPLOAD_DIR} — thư mục lưu ảnh (mặc định "uploads/items")
 *   <li>{@code DB_URL} — JDBC URL (bắt buộc trong Docker)
 *   <li>{@code DB_USERNAME} — username DB
 *   <li>{@code DB_PASSWORD} — password DB
 * </ul>
 */
public class ServerMain {
  ...
  public static void main(String[] args) throws Exception {
    ...
    if (imagePortEnv != null && !imagePortEnv.isBlank()) {
      try {
        imagePort = Integer.parseInt(imagePortEnv.trim());
      } catch (NumberFormatException e) {
    ...
    server.start();
    log.info("=== WebSocket server on port {}, Image HTTP server on port {} ===", port, imagePort);
  }
}
