...
/**
 * Quản lý toàn bộ logic rating — đảm bảo chỉ hệ thống mới thay đổi rating. Không phụ thuộc vào
 * service nào khác.
 *
 * <p>Rating KHÔNG bao giờ được thay đổi trực tiếp từ bên ngoài service này. {@link
 * User#adjustRating(double)} chỉ được gọi từ đây.
 *
 * <p>Cơ chế phục hồi SUSPENDED: Sau 3 tháng được 1 lần, 1 tài khoản được 1 lần.
 */
public class RatingService implements IRatingService {
  ...
  /**
   * Tài khoản BANNED/SUSPENDED vẫn được nạp/rút/xem ví (restricted login). Không ảnh hưởng {@link
   * #isEligible(User)} dùng cho bid/auction/seller.
   */
  ...
  public void rewardBidder(NormalUser bidder) {
    ...
    log.info(
        "Rating reward bidder: username={}, delta=+{}, newRating={}",
        bidder.getUsername(),
        REWARD_BIDDER_PAYMENT,
        bidder.getRating());
    ...
    saveNotification(
        ...
        "Điểm uy tín tăng",
        String.format(
            "Bạn đã thanh toán đúng hạn. Điểm uy tín tăng +%.1f → %.2f.",
  ...
  public void rewardSeller(User seller) {
    ...
    log.info(
        "Rating reward seller: username={}, delta=+{}, newRating={}",
        seller.getUsername(),
        REWARD_SELLER_SALE,
        seller.getRating());
    ...
    saveNotification(
        ...
        "Điểm uy tín tăng",
        String.format(
            "Phiên đấu giá của bạn hoàn tất thành công. Điểm uy tín tăng +%.1f → %.2f.",
  ...
  public void penalizeLatePayment(NormalUser bidder) {
    ...
    log.info(
        "Rating penalty late-payment: username={}, delta=-{}, newRating={}",
        bidder.getUsername(),
    ...
    userDAO.updateRatingAndPenalty(
        bidder.getId(), bidder.getRating(), bidder.isHasEverBeenPenalized());
    ...
    saveNotification(
        ...
        String.format(
            "Bạn không thanh toán đúng hạn. Điểm uy tín trừ -%.1f → %.2f.%s",
            PENALTY_LATE_PAYMENT,
            bidder.getRating(),
  ...
  public void penalizeSeller(User seller) {
    ...
    log.info(
        "Rating penalty seller-quality: username={}, delta=-{}, newRating={}",
        seller.getUsername(),
        PENALTY_SELLER_QUALITY,
        seller.getRating());
    ...
    saveNotification(
        ...
        "Cảnh báo: điểm uy tín bị trừ",
        String.format(
            "Báo cáo chất lượng sản phẩm được duyệt. Điểm uy tín trừ -%.1f → %.2f.%s",
            PENALTY_SELLER_QUALITY,
            seller.getRating(),
            seller.getAccountStatus() == AccountStatus.SUSPENDED ? " Tài khoản bị tạm khóa." : ""));
  }
  ...
  public void penalizeEarlyLeave(NormalUser bidder) {
    ...
    bidder.markPenalized();
    log.warn(
        "Rating penalty early-leave: username={}, delta=-{}, newRating={}",
        bidder.getUsername(),
        PENALTY_EARLY_LEAVE,
        bidder.getRating());
    ...
    saveNotification(
        ...
        "Cảnh báo: điểm uy tín bị trừ",
        String.format(
            "Bạn đã rời phiên đấu giá khi đang dẫn đầu. Điểm uy tín trừ -%.1f → %.2f.%s",
            PENALTY_EARLY_LEAVE,
            bidder.getRating(),
            bidder.getAccountStatus() == AccountStatus.SUSPENDED ? " Tài khoản bị tạm khóa." : ""));
  }
  ...
  public void checkAndRestoreSuspended(User user, LocalDateTime currentTime) {
    ...
    if (!currentTime.isAfter(threshold)) {
      return;
    }
    ...
    if (user.getRating() > AUTO_SUSPEND_THRESHOLD) {
      ...
      log.info(
          "Rating auto-restored to ACTIVE: username={}, newRating={}",
          user.getUsername(),
          user.getRating());
    } else {
      log.info(
          "Rating restored but still suspended: username={}, newRating={}",
          user.getUsername(),
          user.getRating());
    }
    ...
    saveNotification(
        ...
        String.format(
            "Tài khoản của bạn đã được xem xét lại. Điểm uy tín hiện tại: %.2f. Trạng thái: %s.",
   ...
   *
   * <p>Công thức: delta = (score - 3.0) × 0.1 → range [-0.2, +0.2]. Score 1–2: trừ nhẹ; Score 4–5:
   * cộng nhẹ; Score 3: không thay đổi.
   *
  ...
  public void applyUserRating(User target, double score, String auctionId, String raterName) {
    if (score < 1.0 || score > 5.0) {
      throw new IllegalArgumentException(
          "Rating score phải trong khoảng 1.0 – 5.0, nhận được: " + score);
    }
    ...
    log.info(
        "applyUserRating: target={}, rater={}, score={}, delta={}, newRating={}",
        target.getUsername(),
        raterName,
        score,
        delta,
    ...
    saveNotification(
        ...
        String.format(
            "%s đánh giá bạn %.1f/5. Điểm uy tín %s%.2f → %.2f.",
  ...
  private void autoSuspendIfNeeded(User user, String auctionId) {
    if (user.getAccountStatus() == AccountStatus.ACTIVE
      ...
      log.info(
          "Account auto-suspended: username={}, rating={}, threshold={}",
          user.getUsername(),
          user.getRating(),
      ...
      saveNotification(
          ...
          String.format(
              "Điểm uy tín của bạn (%.2f) xuống dưới ngưỡng cho phép. "
                  + "Tài khoản bị tạm khóa và sẽ được xem xét sau 3 tháng.",
              user.getRating()));
