 ...
 *
 * <p>Mọi thao tác tiền tệ trong hệ thống đấu giá đều đi qua interface này. Tuân thủ DIP: các class
 * phụ thuộc (BidService, PaymentService) nhận {@code IWalletService} qua constructor, không new
 * cứng {@code WalletService}.
 *
 * <p>Tuân thủ OCP: muốn thêm cơ chế thanh toán mới (escrow, crypto, trả góp...) chỉ cần tạo
 * implementation mới --- không sửa code hiện có.
 *
...
public interface IWalletService {
   ...
   *
   * <p>Chỉ rút được phần availableBalance (không rút tiền đang khóa cọc). Bắt buộc rút hết trước
   * khi xóa tài khoản.
   *
  ...
  /**
   * Khóa cọc khi bidder tham gia phiên đấu giá (joinAuction). Trừ trực tiếp khỏi balance và tăng
   * lockedDeposit. Ngăn dùng cùng một số tiền cọc cho nhiều phiên vượt khả năng chi trả.
   *
   * @param bidder bidder tham gia
   * @param depositAmount số tiền cọc (30% giá khởi điểm)
   * @param auctionId id phiên
   * @throws com.group13.auction.exception.AuctionBusinessException nếu số dư không đủ để khóa cọc
   */
  void lockDeposit(NormalUser bidder, long depositAmount, String auctionId);

  /**
   * Hoàn cọc cho bidder không thắng khi phiên kết thúc. Giải phóng lockedDeposit và trả lại
   * balance.
   *
  ...
  /**
   * Tịch thu cọc của winner không thanh toán đúng hạn. Cọc bị chuyển vào {@link
   * com.group13.auction.bank.SystemBank}. Theo luật đấu giá: winner không thanh toán mất toàn bộ
   * cọc.
   *
   ...
   *
   * <p>Tịch thu {@code penaltyAmount} vào SystemBank, hoàn trả phần còn lại ({@code depositAmount -
   * penaltyAmount}) về available balance. Toàn bộ thực hiện trong một lock để tránh race condition.
   *
   ...
   * @param auctionId id phiên
   */
  void partialForfeitDeposit(
      NormalUser bidder, long depositAmount, long penaltyAmount, String auctionId);

  /**
   * Chuyển tiền từ Winner -> SystemBank (FUNDS_HELD). Seller CHƯA nhận tiền - chỉ nhận qua {@link
   * PaymentService#releaseToSeller}.
   *
   * <p>Logic:
   *
   * <ol>
   *   <li>Trừ phần {@code remaining = finalPrice - depositPaid} từ balance winner.
   *   <li>Giải phóng cọc (lockedDeposit) rồi trừ luôn khỏi balance - cọc chuyển vào bank.
   *   <li>Bank ghi nhận toàn bộ {@code finalPrice}.
   * </ol>
   *
   ...
   * @param auctionId id phiên
   */
  void executePaymentToBank(NormalUser winner, long finalPrice, long depositPaid, String auctionId);
}
