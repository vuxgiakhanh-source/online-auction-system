...
/**
 * Xử lý bất đồng bộ khi Seller bị {@link AccountStatus#BANNED} hoặc {@link AccountStatus#SUSPENDED}:
 * hủy phiên OPEN/RUNNING, hoàn cọc, giải phóng quality report PENDING, hủy Second Chance PENDING.
 *
 * <p>Tách khỏi {@link com.group13.auction.service.AccountService} để tránh tight coupling —
 * các điểm đổi trạng thái chỉ gọi {@link #onAccountSanctioned(User, AccountStatus)}.
 */
public class SellerSanctionCoordinator {
    ...
    public static void initialize(
            ...
            QualityReportDAO qualityReportDAO,
            SecondChanceOfferDAO secondChanceOfferDAO) {
        instance = new SellerSanctionCoordinator(
    ...
    /**
     * Kích hoạt workflow nếu user là Seller và vừa chuyển sang BANNED/SUSPENDED.
     * Fire-and-forget — không block luồng ban/suspend.
     */
    ...
    private void runSanctionWorkflow(NormalUser seller, AccountStatus status) {
        log.info("Seller sanction workflow started: userId={}, username={}, status={}",
                seller.getId(), seller.getUsername(), status);
        try {
            ...
            log.error("Seller sanction workflow failed: userId={}, status={}",
                    seller.getId(), status, e);
        }
        log.info("Seller sanction workflow finished: userId={}, status={}",
                seller.getId(), status);
    }
    ...
    private void cancelOpenAuctions(NormalUser seller) {
        ...
        for (String auctionId : auctionIds) {
            ...
            try {
                ...
                log.info("Sanction cancel auction: auctionId={}, seller={}", auctionId, seller.getUsername());
            } catch (Exception e) {
                log.warn("Sanction cancel auction failed: auctionId={}, seller={}",
                        auctionId, seller.getUsername(), e);
            }
        }
    }
    ...
    private void dismissPendingQualityReports(NormalUser seller) {
        for (QualityReport report : qualityReportDAO.findBySellerId(seller.getId())) {
            ...
            try {
                ...
                log.info("Sanction auto-resolved quality report: reportId={}, auctionId={}",
                        report.getId(), report.getAuctionId());
            } catch (Exception e) {
    ...
    private void expirePendingSecondChanceOffers(NormalUser seller) {
        List<SecondChanceOffer> offers = secondChanceOfferDAO.findPendingOffersBySellerId(seller.getId());
        for (SecondChanceOffer offer : offers) {
            ...
            try {
                ...
                log.info("Sanction expired second-chance offer: offerId={}, auctionId={}",
                        offer.getId(), offer.getAuctionId());
            } catch (Exception e) {
