 ...
 *
 * <ol>
 *   <li>Winner gọi {@link #submitReport} → report ở PENDING, notify winner xác nhận.
 *   <li>Admin gọi {@link #approveReport} → trừ rating Seller, hoàn tiền Winner, notify cả hai bên.
 *   <li>Admin gọi {@link #rejectReport} → notify winner biết kết quả.
 * </ol>
 */
public class QualityReportService implements IQualityReportService {
  ...
  public QualityReport submitReport(QualityReport report) {
    ...
    if (auction != null && auction.getWinner() != null) {
      ...
      String winnerId =
          auctionWinner.getWinner() != null ? auctionWinner.getWinner().getId() : null;
      if (reporterId == null || !reporterId.equals(winnerId)) {
        log.warn(
            "Quality report rejected — reporter is not the auction winner: auctionId={},"
                + " reporterId={}, winnerId={}",
            report.getAuctionId(),
            reporterId,
            winnerId);
      ...
      if (status != com.group13.auction.model.auction.AuctionWinner.PaymentStatus.ITEM_RECEIVED) {
        log.warn(
            "Quality report rejected — winner has not confirmed item received: auctionId={},"
                + " status={}",
            report.getAuctionId(),
            status);
        throw new IllegalStateException(
    ...
    if (qualityReportDAO.existsByAuctionAndReporter(
      ...
      log.warn(
          "Quality report rejected — duplicate: auctionId={}, reporterId={}",
          report.getAuctionId(),
    ...
    log.info(
        "Quality report submitted: reportId={}, auctionId={}, reporterId={}, username={}",
        report.getId(),
        report.getAuctionId(),
        report.getReporter().getId(),
  ...
  public void approveReport(Admin admin, QualityReport report, Auction auction) {
    ...
    synchronized (lock) {
      if (report.getStatus() != QualityReport.ReportStatus.PENDING) {
        log.warn(
            "Approve quality report rejected because status is not PENDING: reportId={}, status={}",
            report.getId(),
            report.getStatus());
        throw new IllegalStateException("Report không ở trạng thái PENDING: " + report.getStatus());
      }
      ...
      log.info(
          "Quality report approved: reportId={}, auctionId={}, adminId={}, sellerId={},"
              + " winnerId={}, refundAmount={}",
          report.getId(),
          report.getAuctionId(),
          admin.getId(),
          seller.getId(),
      ...
      saveNotification(
          ...
          String.format(
              "Báo cáo của bạn đã được chấp thuận. " + "Số tiền %d đã được hoàn vào tài khoản.",
              finalPrice));
      ...
      saveNotification(
          ...
          String.format(
              "Báo cáo chất lượng sản phẩm của bạn tại phiên %s đã được quản trị viên xác nhận. "
  ...
  public void rejectReport(Admin admin, QualityReport report) {
    ...
    synchronized (lock) {
      if (report.getStatus() != QualityReport.ReportStatus.PENDING) {
        log.warn(
            "Reject quality report rejected because status is not PENDING: reportId={}, status={}",
            report.getId(),
            report.getStatus());
        throw new IllegalStateException("Report không ở trạng thái PENDING: " + report.getStatus());
      }
      ...
      admin.addActionLog(entry);
      log.info(
          "Quality report rejected: reportId={}, auctionId={}, adminId={}, reporterId={}",
          report.getId(),
          report.getAuctionId(),
          admin.getId(),
          report.getReporter().getId());
      ...
      saveNotification(
          ...
          "Sau khi xem xét, quản trị viên quyết định không chấp thuận báo cáo chất lượng của bạn"
              + " tại phiên "
              + report.getAuctionId()
              + ".");
    }
