 ...
 * <p><b>Strategy:</b>
 * <ul>
 *   <li>Constructor validation — fail-fast on bad args</li>
 *   <li>scheduleAtFixedRate — execution timing verified with CountDownLatch (no sleep())</li>
 *   <li>Error isolation — task exception must NOT kill the scheduler</li>
 *   <li>shutdownNow — executor terminates; subsequent tasks don't run</li>
 *   <li>Thread naming — daemon flag set correctly</li>
 * </ul>
 *
 * <p><b>Flaky-test mitigations:</b>
 * <ul>
 *   <li>All timing assertions use CountDownLatch.await() with a generous but finite timeout.</li>
 *   <li>No Thread.sleep() calls.</li>
 *   <li>Tests that verify "no extra invocation" wait for at least one cycle first,
 *       then assert count with AtomicInteger — never race against wall clock.</li>
 * </ul>
 */
...
class TaskSchedulerTest {
    ...
    class ScheduleAtFixedRate {
        ...
        void should_executeTask_when_scheduledWithZeroInitialDelay() throws InterruptedException {
            ...
            assertThat(executed)
                    .as("Task should execute at least once within timeout")
                    .isTrue();
        }
        ...
        void should_respectInitialDelay_when_initialDelayIsNonZero() throws InterruptedException {
            ...
            sut.scheduleAtFixedRate(() -> {
                ...
                started.countDown();
            }, 200, 1_000, TimeUnit.MILLISECONDS);
    ...
    class ErrorIsolation {
        ...
        void should_continueScheduling_when_taskThrowsRuntimeException() throws InterruptedException {
            ...
            assertThat(recovered)
                    .as("Scheduler must continue running after task exception")
                    .isTrue();
        ...
        void should_captureThrownException_without_propagatingToScheduler() throws InterruptedException {
    ...
    class ThreadProperties {
        ...
        void should_useDaemonThread_so_jvmCanExitCleanly() throws InterruptedException {
            ...
            assertThat(isDaemon.get())
                    .as("Scheduler thread must be a daemon thread")
                    .isTrue();
        }
