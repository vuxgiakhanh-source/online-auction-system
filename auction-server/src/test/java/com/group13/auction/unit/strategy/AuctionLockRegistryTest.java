 ...
 *
 * <ul>
 *   <li>Object identity: cùng auctionId → cùng lock instance (==)
 *   <li>Isolation: khác auctionId → khác lock instance
 *   <li>Repeated retrieval: nhiều lần getLock cùng id → deterministic
 *   <li>Release & recreate: sau release, getLock tạo lock mới
 *   <li>Registry integrity: size() phản ánh đúng số lock đang tồn tại
 *   <li>Edge case: null, empty string, whitespace-only, special char, very long id
 *   <li>Lock usability: lock trả về có thể lock/unlock bình thường
 *   <li>Concurrency contract: nhiều thread cùng getLock cùng id → cùng instance
 * </ul>
 *
 * <p>FIRST compliant: Fast, Independent, Repeatable, Self-validating, Timely. Không DB, không
 * network, không filesystem. Thread test chỉ kiểm tra contract (object identity) — không dùng
 * sleep/timing.
 */
...
class AuctionLockRegistryTest {
   ...
   *
   * <p>Dùng {@link AuctionLockRegistry#clearAll()} thay vì release từng id cố định vì Singleton này
   * được chia sẻ với tất cả test khác trong cùng JVM (concurrency tests, load tests…). Các test đó
   * có thể đã tạo lock với UUID ngẫu nhiên mà không cleanup → {@code size()} khác 0 khi chạy tuần
   * tự.
   */
  @BeforeEach
  void setUp() {
  ...
  class LockUsability {
    ...
    void getLock_sameInstance_lockStateConsistent() {
      ...
      assertTrue(lockAgain.isLocked(), "Cùng lock instance phải phản ánh trạng thái lock hiện tại");
  ...
  class ReleaseContract {
    ...
    void release_existingId_decreasesSize() {
      ...
      // Assert
      assertEquals(
          sizeBefore - 1, registry.size(), "size() phải giảm 1 sau khi release lock tồn tại");
    }
    ...
    void release_nonExistentId_doesNotThrowAndSizeUnchanged() {
      ...
      assertEquals(
          sizeBefore, registry.size(), "size() không được thay đổi khi release id không tồn tại");
    }
    ...
    void release_thenGetLock_returnsNewInstance() {
      ...
      // Assert
      assertNotSame(
          oldLock, newLock, "Sau khi release và getLock lại, phải nhận được lock instance mới");
    ...
    void release_specificId_doesNotAffectOtherLocks() {
      ...
      assertSame(lockB, lockBAfter, "release(idA) không được ảnh hưởng đến lock của idB");
  ...
  class EdgeCases {
    ...
    void getLock_emptyVsWhitespace_differentInstances() {
      ...
      // Assert
      assertNotSame(
          emptyLock, whitespaceLock, "\"\" và \" \" là hai key khác nhau trong ConcurrentHashMap");
  ...
  class DeterminismAndSideEffect {
    ...
    void afterRelease_newLock_isUnlocked() {
      ...
      assertFalse(newLock.isLocked(), "Lock mới sau release phải ở trạng thái unlocked");
      assertEquals(0, newLock.getHoldCount(), "Lock mới không được có holdCount > 0");
    ...
    void getLock_oneId_doesNotAffectAnotherId() {
      ...
      try {
        ...
        assertFalse(
            lockB.isLocked(), "lockB phải ở trạng thái unlocked, không bị ảnh hưởng bởi lockA");
        assertTrue(lockB.tryLock(), "tryLock() trên lockB phải thành công dù lockA đang bị giữ");
        lockB.unlock();
      } finally {
        lockA.unlock();
      }
  ...
  class ConcurrencyContract {
    ...
    void getLock_concurrentAccess_sameIdAlwaysSameInstance() throws Exception {
      ...
      for (int i = 1; i < results.size(); i++) {
        assertSame(
            expected, results.get(i), "Thread " + i + " phải nhận cùng lock instance với thread 0");
      }
