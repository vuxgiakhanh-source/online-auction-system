...
class ObserverNotificationSmokeTest {
    ...
    void onBidPlaced_nullBidder_noNpe(String role, AuctionObserver observer) {
        AuctionEvent event = new AuctionEvent(
                AuctionEventType.BID_PLACED, runningAuction, null, 1_500_000L);
        assertDoesNotThrow(() -> observer.onBidPlaced(event));
    }
    ...
    void onAuctionEnded_nullWinner_noNpe(String role, AuctionObserver observer) {
        AuctionEvent event = new AuctionEvent(
                AuctionEventType.AUCTION_ENDED, runningAuction, null, 0L);
    ...
    static Stream<org.junit.jupiter.params.provider.Arguments> roleObservers() throws Exception {
        ...
        Admin staff = Admin.reconstitute(
                ...
                "staffObs01", User.hashPassword("adminpass1"), "staff@test.com",
                User.AccountStatus.ACTIVE, 5.0, Admin.LEVEL_STAFF, null);
        Admin master = Admin.reconstitute(
                UUID.randomUUID().toString(), LocalDateTime.now(), LocalDateTime.now(),
                "masterObs1", User.hashPassword("adminpass1"), "master@test.com",
                User.AccountStatus.ACTIVE, 5.0, Admin.LEVEL_MASTER, null);

        return Stream.of(
                org.junit.jupiter.params.provider.Arguments.of("Bidder", new BidderObserver(b, TestFixture.ratingServiceAllowAll())),
                org.junit.jupiter.params.provider.Arguments.of("Seller", new SellerObserver(s, TestFixture.ratingServiceAllowAll())),
                ...
                org.junit.jupiter.params.provider.Arguments.of("SystemAdmin", new SystemAdminObserver(SystemAdmin.getInstance()))
        );
    }
}
