...
class PaymentServiceTest {
  ...
  void completePayment_happyPath() {
    ...
    order.verify(walletService).executePaymentToBank(winner, FINAL_PRICE, DEPOSIT, auction.getId());
  ...
  void completePayment_expiredWinner_throws() {
    ...
    auction.setWinner(
        TestFixture.expiredPendingWinner(winner, auction.getId(), FINAL_PRICE, DEPOSIT));
  ...
  void expirePayment_forfeitsAndOffersSecondChance() {
    ...
    when(bidTransactionDAO.findHighestValidBidExcept(auction.getId(), winner.getId()))
        .thenReturn(
            BidTransaction.create(runnerUp, auction.getId(), 2_500_000L, BidResult.ACCEPTED));
    ...
    verify(auctionService)
        .notify(
            eq(auction),
            eq(AuctionEventType.SECOND_CHANCE_OFFERED),
            eq(runnerUp),
            eq(2_500_000L),
            anyString());
  }
  ...
  private Auction finishedAuctionWithPendingWinner() {
    Auction auction = TestFixture.finishedAuction(seller, winner, STARTING_PRICE, FINAL_PRICE);
    AuctionWinner aw = TestFixture.pendingWinner(winner, auction.getId(), FINAL_PRICE, DEPOSIT);
