...
class AccountServiceTest {
    ...
    void setUp() throws Exception {
        ...
        sut = new AccountService(
                ratingService, userDAO, sellerDAO, adminDAO,
                auctionDAO, auctionWinnerDAO, notificationDAO, accountBanDAO);
        staffAdmin = Admin.reconstitute(
                UUID.randomUUID().toString(), LocalDateTime.now(), LocalDateTime.now(),
                "staffAdmin", User.hashPassword("adminPass1"), "staff@test.com",
                AccountStatus.ACTIVE, 5.0, Admin.LEVEL_STAFF, null);
    }
    ...
    void banUser_setsBannedAndPersists() {
        ...
        verify(accountBanDAO).insertBan(eq(target.getId()), eq(staffAdmin.getId()),
                eq(staffAdmin.getUsername()), eq("LOW_RATING"), isNull());
    }
