...
class AuctionTimerServiceTest {
  ...
  @Mock private IScheduler scheduler;
  ...
  class ExpirePendingWinnerPayments {
    ...
    void pendingWinner_notExpired_skipsExpirePayment() throws Exception {
      Auction auction =
          reconstitutedAuction(
              NOW.minusHours(2), NOW.minusHours(1), Auction.AuctionStatus.FINISHED);
  ...
  class CloseExpiredAuctions {
    ...
    void runningAuction_nullEndTime_ignored() throws Exception {
      Auction auction =
          reconstitutedAuction(NOW.minusHours(1), null, Auction.AuctionStatus.RUNNING);
      register(auction);
    ...
    void closeException_noBroadcastNoRefund() throws Exception {
      Auction auction = runningAuction(NOW.minusHours(1), NOW.minusSeconds(1));
      register(auction);
      doThrow(new IllegalStateException("close failed")).when(auctionService).closeAuction(auction);
  ...
  class StateConsistencyAndCleanup {
    ...
    void successfulClose_cleansAutoBidAndLockRegistries() throws Exception {
      Auction auction = runningAuction(NOW.minusHours(1), NOW.minusSeconds(1));
      register(auction);
      AutoBidRegistry.getInstance()
          .register(normalBidder("auto-1").getId(), auction.getId(), 3_000_000L);
      AutoBidRegistry.getInstance()
    ...
    void failedClose_keepsLockForRetry() throws Exception {
      Auction auction = runningAuction(NOW.minusHours(1), NOW.minusSeconds(1));
      register(auction);
      doThrow(new RuntimeException("temporary failure")).when(auctionService).closeAuction(auction);
  ...
  private void invokeStartPending(LocalDateTime now) throws Exception {
    Method method =
        AuctionTimerService.class.getDeclaredMethod("startPendingAuctions", LocalDateTime.class);
  ...
  private void invokeCloseExpired(LocalDateTime now) throws Exception {
    Method method =
        AuctionTimerService.class.getDeclaredMethod("closeExpiredAuctions", LocalDateTime.class);
    method.setAccessible(true);
    method.invoke(sut, now);
  }
  ...
  private void invokeNotifyUpcomingAuctionEnds(LocalDateTime now) throws Exception {
    Method method =
        AuctionTimerService.class.getDeclaredMethod(
  ...
  private static Auction reconstitutedAuction(
