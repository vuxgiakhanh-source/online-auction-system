...
class AuctionStateMachineTest {
  ...
  class OpenStateTest {
    ...
    static Stream<IllegalTransition> illegalFromOpen() {
      AuctionState open = OpenState.INSTANCE;
      return Stream.of(() -> open.close(true), () -> open.close(false), open::markPaid);
    }
  }
  ...
  class LifecycleTest {
    ...
    void paths_toCanceled() {
      assertEquals(
          Auction.AuctionStatus.CANCELED, OpenState.INSTANCE.start().close(false).getStatus());
      assertEquals(Auction.AuctionStatus.CANCELED, OpenState.INSTANCE.cancel().getStatus());
    }
