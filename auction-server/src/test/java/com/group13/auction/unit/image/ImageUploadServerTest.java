 ...
 *
 * <p>Khởi động server thật trên cổng ngẫu nhiên, không mock network. Không cần DB, không cần
 * Docker.
 */
...
class ImageUploadServerTest {
  ...
  class UploadHappyPath {
    ...
    void uploadedFile_canBeServedViaGet() throws Exception {
      ...
      HttpURLConnection getConn =
          (HttpURLConnection)
              URI.create("http://localhost:" + TEST_PORT + url).toURL().openConnection();
  ...
  class UploadErrorPaths {
    ...
    void getOnUploadEndpoint_returns405() throws Exception {
      HttpURLConnection conn =
          (HttpURLConnection)
              URI.create("http://localhost:" + TEST_PORT + "/upload").toURL().openConnection();
  ...
  private static HttpURLConnection openPost(String path, String contentType) throws Exception {
    HttpURLConnection conn =
        (HttpURLConnection)
            URI.create("http://localhost:" + TEST_PORT + path).toURL().openConnection();
    conn.setRequestMethod("POST");
    conn.setDoOutput(true);
