...
class ChatbotProviderTest {
    ...
    class SingletonTests {
        ...
        void testSingletonInstance() {
            ...
            assertSame(instance1, instance2, "Hai lần gọi getInstance phải trả về cùng một vùng nhớ reference");
        }

        @Test
        @DisplayName("[DATA-LOAD] Phải nạp thành công bộ dữ liệu FAQ từ file tài nguyên json")
        void testFaqDataLoaded() {
            assertTrue(provider.getTotalFaqCount() > 0, "Tổng số câu hỏi FAQ nạp vào bộ nhớ phải lớn hơn 0");
    ...
    class GetAnswerByIdTests {
        ...
        @Test
        @DisplayName("[ID-SUCCESS] Tìm kiếm thành công với mã ID viết thường hoặc viết hoa có khoảng trắng")
        void testGetAnswerById_Success() {
            ...
            ChatbotResponse responseWithSpaces = provider.getAnswerByQuestionId("  " + validId.toUpperCase() + "  ");

            assertTrue(responseLower.isSuccess(), "Phải tìm thấy FAQ khi ID viết thường");
            assertTrue(responseWithSpaces.isSuccess(), "Phải tìm thấy FAQ khi ID bị dính khoảng trắng dư thừa");
        ...
        void testGetAnswerById_EmptyOrNull(String invalidId) {
            ...
            assertTrue(response.getFallbackMessage().contains("[mã câu hỏi trống]"),
        ...
        void testGetAnswerById_NotFound() {
            ...
            assertTrue(response.getFallbackMessage().contains(nonExistentId),
    ...
    class SearchByQueryTests {
        ...
        void testSearchByQuery_ExactPhraseMatch() {
            // Danh sách các câu ngắn thực tế hay lỗi ở bản cũ, hệ thống mới phải xử lý tốt nhờ Khớp nguyên cụm
        ...
        @Test
        @DisplayName("[QUERY-SCORE] Kiểm tra tính toán phân mảnh từ theo công thức mới (matchedWords / totalUserWords)")
        void testSearchByQuery_KeywordScoreMatching() {
            // Lấy từ khóa thực tế từ FAQ đầu tiên để kiểm tra thuật toán phân rã Set từ độc lập dữ liệu cứng
            ...
            String query = sampleQuestion.length() > 10 ? sampleQuestion.substring(0, 10) : sampleQuestion;
            ChatbotResponse response = provider.searchByQuery(query);

            assertNotNull(response);
            assertTrue(response.isSuccess(), "Thuật toán score mới bắt buộc phải khớp thành công cụm từ trích xuất");
        }
        ...
        void testSearchByQuery_NoMatchFallback() {
            ...
            // ĐÃ FIX: Thay vì so bằng tuyệt đối, kiểm tra fallbackMessage hệ thống sinh ra có bọc chuỗi vô nghĩa đó không
            assertTrue(response.getFallbackMessage().contains(nonsenseQuery),
    ...
    class CategoryTests {
        ...
        void testGetFaqsByCategory_EmptyOrNull(String emptyCategory) {
            ...
            assertEquals(provider.getTotalFaqCount(), allFaqs.size(),
    ...
    class ConcurrentTests {
        ...
        @Test
        @DisplayName("[THREAD-SAFE] Đảm bảo nhiều thread gọi getInstance đồng thời thu về cùng một instance")
