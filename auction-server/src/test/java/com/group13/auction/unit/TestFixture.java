 ...
 *
 * <ul>
 *   <li>Fake {@link IRatingService} cho phép kiểm soát kết quả trả về.
 *   <li>Builder nhanh để tạo {@link NormalUser} giả không cần DB.
 *   <li>Builder nhanh để tạo {@link Auction} và {@link Item} con.
 *   <li>Builder nhanh để tạo {@link AuctionWinner} với các trạng thái khác nhau.
 *   <li>Builder nhanh để tạo {@link SecondChanceOffer} với các trạng thái khác nhau.
 *   <li>Builder nhanh để tạo {@link QualityReport} với các trạng thái khác nhau.
 *   <li>Utility reset {@link SystemBank} balance cho test isolation.
 *   <li>Factory helpers không cần DB.
 * </ul>
 *
...
public final class TestFixture {
  ...
  /**
   * Fake IRatingService: mọi Seller đều bị từ chối tạo phiên đấu giá. Dùng để test trường hợp
   * Seller bị khoá.
   */
  ...
  /**
   * Tạo NormalUser có đầy đủ cả role BIDDER lẫn SELLER. Có sẵn balance 10_000_000 để tham gia đấu
   * giá.
   */
  ...
  /**
   * Tạo NormalUser với rating tùy chỉnh và role BIDDER. Dùng khi test các ngưỡng rating
   * (suspend/ban/eligible).
   */
  ...
  /**
   * Tạo NormalUser đã từng bị penalize (hasEverBeenPenalized = true). Dùng để test điều kiện tự
   * động duyệt role Seller.
   */
  ...
  /**
   * Tạo Auction FINISHED: có winner và reserve đã đạt. currentPrice = winningPrice, currentLeader =
   * winner.
   */
  public static Auction finishedAuction(
      NormalUser seller, NormalUser winner, long startingPrice, long winningPrice) {
  ...
  /**
   * Tạo Auction ở bất kỳ trạng thái nào qua reconstitute. Dùng khi cần kiểm soát chính xác
   * currentPrice và status.
   */
  public static Auction auctionWithStatus(
      NormalUser seller, long startingPrice, long currentPrice, Auction.AuctionStatus status) {
    Item item = art("Tranh Test", startingPrice, seller);
   ...
   */
  public static AuctionWinner pendingWinner(
      NormalUser winner, String auctionId, long finalPrice, long depositPaid) {
    return AuctionWinner.create(winner, auctionId, finalPrice, depositPaid, false);
  }

  /**
   * Tạo AuctionWinner PENDING đã quá hạn thanh toán (paymentDeadline đã qua). Dùng để test {@code
   * isExpired() == true}.
   */
  public static AuctionWinner expiredPendingWinner(
      NormalUser winner, String auctionId, long finalPrice, long depositPaid) {
  ...
  public static AuctionWinner expiredSecondOfferWinner(
      NormalUser runnerUp, String auctionId, long finalPrice, long depositPaid) {
    return AuctionWinner.reconstitute(
        UUID.randomUUID().toString(),
        LocalDateTime.now(),
        LocalDateTime.now(),
  ...
  /**
   * Tạo AuctionWinner FUNDS_HELD với confirmReceiptDeadline còn trong tương lai. Dùng để test
   * luồng: thanh toán xong, chờ winner xác nhận nhận hàng.
   */
  public static AuctionWinner fundsHeldWinner(
      NormalUser winner, String auctionId, long finalPrice, long depositPaid) {
    return AuctionWinner.reconstitute(
        UUID.randomUUID().toString(),
        LocalDateTime.now(),
  ...
  /**
   * Tạo AuctionWinner FUNDS_HELD đã quá hạn confirmReceipt. Dùng để test {@code
   * isConfirmReceiptOverdue() == true}.
   */
  public static AuctionWinner overdueConfirmReceiptWinner(
      NormalUser winner, String auctionId, long finalPrice, long depositPaid) {
    return AuctionWinner.reconstitute(
        UUID.randomUUID().toString(),
        LocalDateTime.now(),
  ...
  /**
   * Tạo AuctionWinner COMPLETED (đã hoàn tất thanh toán). Dùng để test các điều kiện terminal
   * state.
   */
  public static AuctionWinner completedWinner(
      NormalUser winner, String auctionId, long finalPrice, long depositPaid) {
    return AuctionWinner.reconstitute(
        UUID.randomUUID().toString(),
        LocalDateTime.now(),
  ...
  public static AuctionWinner secondOfferWinner(
   ...
   */
  public static SecondChanceOffer pendingOffer(
      NormalUser runnerUp, String auctionId, long offerPrice, long depositPaid) {
    return SecondChanceOffer.create(runnerUp, auctionId, offerPrice, depositPaid);
  }

  /**
   * Tạo SecondChanceOffer PENDING đã hết hạn (deadline đã qua). Dùng để test {@code isExpired() ==
   * true}.
   */
  public static SecondChanceOffer expiredPendingOffer(
      NormalUser runnerUp, String auctionId, long offerPrice, long depositPaid) {
  ...
  /**
   * Tạo SecondChanceOffer đã ACCEPTED. Dùng để test transition invalid hoặc idempotent behavior.
   */
  public static SecondChanceOffer acceptedOffer(
      NormalUser runnerUp, String auctionId, long offerPrice, long depositPaid) {
    return SecondChanceOffer.reconstitute(
        UUID.randomUUID().toString(),
        LocalDateTime.now(),
  ...
  public static SecondChanceOffer declinedOffer(
      NormalUser runnerUp, String auctionId, long offerPrice, long depositPaid) {
    return SecondChanceOffer.reconstitute(
        UUID.randomUUID().toString(),
        LocalDateTime.now(),
  ...
  public static QualityReport pendingReport(NormalUser reporter, String auctionId) {
    return QualityReport.create(
        reporter, auctionId, "Hàng không đúng mô tả", List.of("http://evidence1.jpg"));
  }

  /** Tạo QualityReport PENDING với nhiều ảnh minh chứng. */
  public static QualityReport pendingReportWithImages(
      NormalUser reporter, String auctionId, List<String> imageUrls) {
    return QualityReport.create(reporter, auctionId, "Hàng không đúng mô tả", imageUrls);
  }

  /**
   * Tạo QualityReport đã APPROVED với sellerRefundDeadline còn trong tương lai (24h). Dùng để test
   * {@code isSellerRefundOverdue() == false}.
   */
  ...
  /**
   * Tạo QualityReport APPROVED đã quá hạn refund (sellerRefundDeadline đã qua). Dùng để test {@code
   * isSellerRefundOverdue() == true}.
   */
  ...
  /**
   * Tạo QualityReport APPROVED đã quá hạn nhưng refund đã hoàn tất. Dùng để test: overdue +
   * refundCompleted → không trigger ban seller.
   */
  public static QualityReport overdueButRefundCompletedReport(
      NormalUser reporter, String auctionId) {
    return QualityReport.reconstitute(
        UUID.randomUUID().toString(),
        LocalDateTime.now(),
        LocalDateTime.now(),
   ...
   *
   * <p>Tạo instance qua private constructor bằng reflection rồi inject thẳng vào field {@code
   * INSTANCE}. Phải gọi trong {@code @BeforeEach} của mọi test class mà SUT gọi {@code
   * SystemAdmin.getInstance()} (ví dụ: {@code expirePayment}).
   *
  ...
  /**
   * Reset SystemAdmin Singleton về null sau mỗi test. Cần thiết để tránh state rò rỉ giữa các test.
   *
   * @throws Exception nếu reflection thất bại
   */
