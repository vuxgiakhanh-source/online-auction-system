...
/** Unit test cho {@link DTOMapper} — đảm bảo mapping đúng và null-safe. Không DB, không network. */
...
class DTOMapperTest {
  ...
  class ToItemDTO {
    ...
    void item_withImages_dtoImageUrlsMapped() {
      ...
      assertThat(dto.getImageUrls()).hasSize(2).containsExactlyElementsOf(imgs);
  ...
  class ToAuctionUpdateDTO {
    ...
    void finishedAuction_withWinner_winnerFieldsMapped() {
      ...
      AuctionWinner aw =
          AuctionWinner.create(bidder, finished.getId(), 2_000_000L, 300_000L, false);
  ...
  class ToBidChartPoint {
    ...
    void chartPoint_allFieldsMapped() {
      BidDTOs.BidChartPointDTO dto =
          DTOMapper.toBidChartPoint("auc-test-1", 2_500_000L, "alice_user", false);
    ...
    void chartPoint_autoBid_flagTrue() {
      BidDTOs.BidChartPointDTO dto =
          DTOMapper.toBidChartPoint("auc-test-2", 3_000_000L, "bot_user", true);
