...
/**
 * ============================================================================
 * LOAD TEST — SessionManagerLoadTest (unit, không cần Docker)
 * ============================================================================
 *
 * Kiểm tra SessionManager Singleton dưới tải cao đồng thời:
 *   - register / unregister / authenticate song song nhiều thread
 *   - broadcastToAuction / broadcastAll dưới 200+ connections
 *   - addAuctionWatcher / removeAuctionWatcher race condition
 *   - getUserIdsWatchingAuction consistency dưới concurrent mutation
 *
 * Không cần Docker — SessionManager không chạm DB.
 */
...
class SessionManagerLoadTest {
    ...
    void tearDown() {
        ...
        for (WebSocket ws : managedSockets) {
            try { sm.unregister(ws); } catch (Exception ignored) {}
        }
    ...
    class RegisterAuthLoadTest {
        ...
        @Test
        @Timeout(value = 30)
        @DisplayName("L-SM1: 50 thread đồng thời register + authenticate — không NPE, connectedCount đúng")
        void concurrent_registerAndAuthenticate_noNPE() throws Exception {
            ...
            for (int i = 0; i < threads; i++) sockets.add(openSocket());
            ...
            long onlineCount = sockets.stream()
                    .mapToInt(ws -> {
        ...
        @Test
        @Timeout(value = 30)
        @DisplayName("L-SM2: 32 thread register rồi unregister ngay — không memory leak (connectedCount không tăng mãi)")
    ...
    class BroadcastLoadTest {
        ...
        @Test
        @Timeout(value = 30)
        @DisplayName("L-SM3: 100 client watch cùng 1 auction, 4 thread broadcast đồng thời — không exception")
        ...
        @Test
        @Timeout(value = 30)
        @DisplayName("L-SM4: addAuctionWatcher / removeAuctionWatcher / broadcastToAuction đồng thời — không deadlock")
    ...
    class WatcherListConsistencyTest {
        ...
        @Test
        @Timeout(value = 30)
        @DisplayName("L-SM8: 20 thread add/remove watcher + 10 thread query — không NPE, list không bao giờ null")
                ...
                new Thread(() -> {
                    try {
                        ...
                        for (int op = 0; op < ops * 2; op++) {
                            ...
                            if (ids == null) failures.incrementAndGet();
                        }
                    } catch (Exception e) {
                        failures.incrementAndGet();
                    } finally {
