...
/**
 * ============================================================================ LOAD TEST —
 * AutoBidProcessorLoadIT (DB thật)
 * ============================================================================
 *
 * <p>Kiểm tra AutoBidProcessor dưới tải với MySQL thật:
 *
 * <p>Group 1 — Chain auto-bid nhiều auto-bidder dưới tải: Nhiều auto-bidder + manual bidder cùng
 * auction — chain xử lý đúng, giá không vượt maxBid, không deadlock dưới lock.
 *
 * <p>Group 2 — Auto-bid chain qua nhiều phiên đồng thời: N phiên chạy song song, mỗi phiên có
 * auto-bid chain riêng. Verify isolation giữa các phiên.
 *
 * <p>Group 3 — AutoBidProcessor.process() gọi đồng thời từ nhiều thread (trong lock): Stress test
 * luồng: manual bid → trigger process() trong lock — không NPE, không chain vô tận.
 *
 * <p>Group 4 — AutoBidProcessor fallback DB lookup dưới tải: User không có trong AuctionManager
 * in-memory → processor fallback tìm DB. Stress test đường DB fallback.
 */
@RequiresDocker
@Testcontainers
...
class AutoBidProcessorLoadIT extends IntegrationTestBase {
  ...
  class AutoBidChainLoadTest {
    ...
    @Timeout(value = 90)
    @DisplayName(
        "L-ABP1: 6 auto-bidder + 4 manual bidder cùng phiên — giá không vượt maxBid, không"
            + " deadlock")
    ...
    @Timeout(value = 60)
    @DisplayName(
        "L-ABP2: Auto-bid chain khi auto-bidder cạn maxBid — bị xóa khỏi registry, giá dừng đúng")
    void autoBidChain_exhaustedMaxBid_removedFromRegistry() throws Exception {
      ...
      long autoBidMax =
          manualBid + BidIncrementCalculator.calculate(manualBid); // vừa đủ counter 1 lần
  ...
  class MultiAuctionAutoBidLoadTest {
    ...
    @Timeout(value = 120)
    @DisplayName(
        "L-ABP3: 5 phiên × 2 auto-bidder × 3 manual bid song song — isolation đúng, giá không chéo"
            + " phiên")
  ...
  class ProcessUnderHighLoadTest {
    ...
    @Timeout(value = 60)
    @DisplayName(
        "L-ABP4: 8 thread × 5 lần (placeBid + process) trong lock — giá tăng đơn điệu, không"
            + " exception")
  ...
  class AutoBidProcessorDBFallbackTest {
    ...
    @Timeout(value = 90)
    @DisplayName(
        "L-ABP6: auto-bidder KHÔNG có trong AuctionManager — processor fallback DB, không NPE")
    ...
    void concurrent_dbFallback_6Auctions_noDeadlock() throws Exception {
      ...
      assertThat(failures.get()).as("DB fallback đồng thời không được throw exception").isZero();
  ...
  /**
   * Dùng reflection để shutdown tất cả executor của AutoBidProcessor và xóa static state. Gọi trước
   * cleanupDB() để tránh chain vẫn đang chạy khi auction bị xóa khỏi DB.
   */
  ...
  /**
   * Poll chainRunning via reflection cho đến khi chain kết thúc hoặc hết timeout. chainRunning được
   * set false trong finally block của chain task, SAU KHI tất cả DB writes (từ bidService.placeBid)
   * đã hoàn thành → an toàn để assert DB sau đây.
   */
