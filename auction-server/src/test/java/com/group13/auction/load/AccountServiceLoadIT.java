...
/**
 * ============================================================================ LOAD TEST —
 * AccountServiceLoadIT (DB thật)
 * ============================================================================
 *
 * <p>Kiểm tra AccountService dưới tải đồng thời với MySQL thật:
 *
 * <p>Group 1 — banUser() song song: Nhiều admin ban nhiều user cùng lúc — DB status khớp RAM, không
 * lost update.
 *
 * <p>Group 2 — autoApproveSellerRole() song song: Nhiều user request seller role đồng thời — không
 * double-approve, DB sellers record nhất quán.
 *
 * <p>Group 3 — createStaffAdmin() song song: Nhiều request tạo admin STAFF cùng lúc — unique
 * constraint enforcement, không NPE trong AuctionManager.registerUser().
 *
 * <p>Group 4 — deposit/withdraw + banUser xen kẽ: Mixed workload: tài chính và quản lý tài khoản
 * đồng thời.
 *
 * <p>Group 5 — requestCancelAuction() song song: Nhiều seller gửi yêu cầu hủy phiên đồng thời —
 * AuctionManager observer không bị race condition.
 */
@RequiresDocker
@Testcontainers
...
class AccountServiceLoadIT extends IntegrationTestBase {
  ...
  class BanUserLoadTest {
    ...
    @Timeout(value = 60)
    @DisplayName(
        "L-ACC1: 20 user × ban đồng thời bởi 1 admin — DB status = BANNED, không lost update")
    void concurrent_banUsers_20Targets_allBanned() throws Exception {
      ...
      assertThat(failure.get()).as("banUser không được throw exception dưới tải").isZero();
  ...
  class AutoApproveSellerLoadTest {
    ...
    @Timeout(value = 60)
    @DisplayName(
        "L-ACC3: 16 user đủ điều kiện — autoApproveSellerRole đồng thời, không double-approve")
    ...
    @Timeout(value = 60)
    @DisplayName(
        "L-ACC4: 1 user bị gọi autoApproveSellerRole từ 10 thread — idempotent, không exception")
  ...
  class CreateStaffAdminLoadTest {
    ...
    @Timeout(value = 60)
    @DisplayName(
        "L-ACC5: 10 thread tạo 10 admin STAFF khác nhau — tất cả thành công, DB unique đảm bảo")
    ...
    @Timeout(value = 60)
    @DisplayName(
        "L-ACC6: 8 thread tạo admin STAFF với cùng username — chỉ 1 thành công (unique constraint)")
  ...
  class MixedAccountWorkloadTest {
    ...
    @Timeout(value = 90)
    @DisplayName(
        "L-ACC7: 8 thread deposit + 4 thread banUser cùng pool users — không deadlock, không NPE")
    void mixed_depositAndBan_noDeadlockNoNPE() throws Exception {
      ...
      assertThat(failures.get()).as("Mixed deposit+ban không được throw exception").isZero();
  ...
  class RequestCancelAuctionLoadTest {
    ...
    @Timeout(value = 90)
    @DisplayName(
        "L-ACC8: 8 seller × requestCancelAuction đồng thời — không NPE trong observer dispatch")
    ...
    @Timeout(value = 60)
    @DisplayName(
        "L-ACC9: 16 thread notifyStaffObservers đồng thời — CopyOnWriteArrayList không bị"
                      ...
                      try {
                        ...
                        com.group13.auction.manager.AuctionManager.getInstance()
                            .notifyStaffObservers(
                                new com.group13.auction.observer.AuctionEvent(
                                    com.group13.auction.observer.AuctionEvent.AuctionEventType
