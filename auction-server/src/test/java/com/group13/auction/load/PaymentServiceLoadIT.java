...
/**
 * ============================================================================ LOAD TEST —
 * PaymentServiceLoadIT (DB thật)
 * ============================================================================
 *
 * <p>Kiểm tra PaymentService + WalletService dưới tải đồng thời với MySQL thật: - refundDeposits()
 * đồng thời nhiều phiên — không deadlock, không lost update - lockDeposit song song nhiều user trên
 * cùng phiên — balance nhất quán - completePayment song song nhiều phiên — không double-charge -
...
class PaymentServiceLoadIT extends IntegrationTestBase {
  ...
  class LockDepositLoadTest {
    ...
    @Timeout(value = 90)
    @DisplayName(
        "L-PAY1: 20 user đồng thời lockDeposit vào cùng 1 phiên — balance DB khớp RAM, không lost"
            + " update")
    void concurrent_lockDeposit_20Users_balanceConsistent() throws Exception {
      ...
      assertThat(failure.get()).as("lockDeposit không được throw exception dưới tải").isZero();
  ...
  class RefundDepositsLoadTest {
    ...
    @Timeout(value = 120)
    @DisplayName(
        "L-PAY2: 6 phiên × 4 bidder, refundDeposits chạy song song — không deadlock, balance đúng")
  ...
  class WalletDepositWithdrawLoadTest {
    ...
    @Timeout(value = 60)
    @DisplayName(
        "L-PAY3: 12 thread deposit/withdraw xen kẽ trên cùng user — balance không bao giờ âm")
    ...
    @Timeout(value = 60)
    @DisplayName(
        "L-PAY4: 8 phiên song song, mỗi phiên 1 bidder đặt cọc rồi hoàn — không lost update")
    void parallel_lockAndUnlock_8Auctions_noLostUpdate() throws Exception {
      ...
      for (NormalUser user : users) {
        ...
        assertThat(fromDB.getBalance())
            ...
            .isEqualTo(initialBalance);
        assertThat(fromDB.getLockedDeposit()).as("lockedDeposit phải = 0 sau unlock").isZero();
      }
    }
  }
  ...
  class CompletePaymentLoadTest {
    ...
    void concurrent_completePayment_5Auctions_noDoubleCharge() throws Exception {
      ...
      for (int i = 0; i < auctionCount; i++) {
        ...
        NormalUser winner =
            buildUserWithBalance("pay5_win" + i, bidAmount + deposit + 1_000_000L, userDAO);
