...
/**
 * ============================================================================
 * LOAD TEST — PacketRouterLoadTest (unit, không cần Docker)
 * ============================================================================
 *
 * Kiểm tra PacketRouter dưới tải đồng thời:
 *   - route() song song 32 thread — không exception, không deadlock
 *   - Nhiều loại packet xen kẽ — dispatch đúng handler
 *   - Packet lỗi (malformed JSON) song song — không crash router
 *   - register() handler song song nhiều thread
 */
...
class PacketRouterLoadTest {
    ...
    void setUp() {
        ...
        lenient().when(bidHandler.supports(any())).thenAnswer(inv -> {
            PacketType t = inv.getArgument(0);
            return t == PacketType.PLACE_BID || t == PacketType.JOIN_AUCTION
                    || t == PacketType.WATCH_AUCTION || t == PacketType.LEAVE_AUCTION;
        });
        ...
        lenient().doNothing().when(authHandler).handle(any(), any(), any(JsonElement.class), anyString());
        lenient().doNothing().when(bidHandler).handle(any(), any(), any(JsonElement.class), anyString());
        lenient().doNothing().when(paymentHandler).handle(any(), any(), any(JsonElement.class), anyString());
    ...
    class RouteConcurrentTest {
        ...
        @Test
        @Timeout(value = 30)
        @DisplayName("L-PR1: 32 thread × 200 lần route() với nhiều PacketType — không exception, không deadlock")
        void concurrent_route_mixedPacketTypes_noException() throws Exception {
            int threads      = 32;
            int opsPerThread = 200;

            // Chuẩn bị các packet JSON hợp lệ
            String loginJson = PacketCodec.encode(Packet.of(PacketType.LOGIN,
                    new com.group13.auction.common.dto.auth.LoginRequestDTO("user", "pass"), "r1"));
            String joinJson  = "{\"type\":\"JOIN_AUCTION\",\"requestId\":\"r2\",\"payload\":\"auc-1\"}";
            String bidJson   = "{\"type\":\"PLACE_BID\",\"requestId\":\"r3\",\"payload\":{\"auctionId\":\"A\",\"amount\":1000000}}";
            String depJson   = "{\"type\":\"DEPOSIT\",\"requestId\":\"r4\",\"payload\":{\"amount\":500000}}";
        ...
        @Test
        @Timeout(value = 30)
        @DisplayName("L-PR2: 16 thread route() xen kẽ packet hợp lệ và malformed JSON — router không crash")
        void concurrent_route_mixedValidAndMalformed_routerDoesNotCrash() throws Exception {
            int threads      = 16;
            int opsPerThread = 100;

            String validJson    = PacketCodec.encode(Packet.of(PacketType.LOGIN,
                    new com.group13.auction.common.dto.auth.LoginRequestDTO("u", "p"), "valid"));
            ...
            assertThat(crashes.get())
                    .as("Router không được bị JVM crash dưới bất kỳ input nào")
                    .isZero();
        }
        ...
        void routeThroughput_sustainedLoad_meetsThreshold() throws Exception {
            ...
            String json = PacketCodec.encode(Packet.of(PacketType.LOGIN,
                    new com.group13.auction.common.dto.auth.LoginRequestDTO("u", "p"), "tp"));
            ...
            assertThat(totalRouted.get())
                    .as("Phải có ít nhất 1 route thực hiện được")
                    .isPositive();
    ...
    class PacketCodecLoadTest {
            ...
            for (int t = 0; t < threads; t++) {
                ...
                new Thread(() -> {
                    try {
                        ...
                        for (int op = 0; op < opsPerThread; op++) {
                            try {
                                String json = PacketCodec.encode(
        ...
        void concurrent_peekType_noException_correctType() throws Exception {
            ...
            String loginJson = PacketCodec.encode(Packet.of(PacketType.LOGIN,
                    new com.group13.auction.common.dto.auth.LoginRequestDTO("u", "p"), "r"));
                    ...
                    try {
                        ...
                        for (int op = 0; op < opsPerThread; op++) {
                            try {
                                ...
                                if (type == null) failures.incrementAndGet();
                                if (seed % 2 == 0 && type != PacketType.LOGIN) failures.incrementAndGet();
                            } catch (Exception e) {
                                failures.incrementAndGet();
                            }
                        }
