 ...
 *
 * <p>THAY ĐỔI SO VỚI BẢN CŨ: P1: Trước đây chỉ assert upper-bound (không sinh tiền). Sau đây
 * ENFORCE: chỉ đúng 1 lần releaseToSeller() được chạy thực sự. FIX CẦN: bọc synchronized(seller)
 * trong PaymentService.releaseToSeller() quanh đoạn read-modify-write: setBalance(getBalance() +
 * payout).
 *
 * <p>P2: Trước đây không enforce — chỉ check <= 2×finalPrice. Sau đây ENFORCE: balance ==
 * finalPrice (đúng 1 lần refund). FIX CẦN: bọc synchronized(winner) trong refundToWinnerFromBank()
 * quanh đoạn setBalance(getBalance() + finalPrice).
 *
 * <p>P3: Không thay đổi — WalletService.executePaymentToBank đã synchronized OK. P4: Không thay đổi
 * — bounds check hợp lệ.
 * ============================================================================
 */
...
class PaymentServiceConcurrencyTest extends ConcurrencyTestBase {
  ...
  void releaseToSeller_concurrent_exactlyOnePayout() throws InterruptedException {
    ...
    Auction auction =
        buildRunningAuction(finalPrice, finalPrice, java.time.LocalDateTime.now().minusMinutes(1));
    AuctionWinner aw =
        AuctionWinner.create(winner, auction.getId(), finalPrice, finalPrice * 3 / 10, false);
    aw.markFundsHeld();
    ...
    log.info(
        "[P1] seller.balance={}, expectedPerCall={}, successes={}, failures={}",
        actualBalance,
  ...
  @Order(2)
  @DisplayName(
      "P2: refundToWinnerFromBank() concurrent 2 lần — balance phải = đúng 1 lần [ENFORCE]")
  @Timeout(value = 5)
  void refundToWinnerFromBank_concurrent_exactlyOneRefund() throws InterruptedException {
    ...
    log.info(
        "[P2] winner.balance={}, finalPrice={}, successes={}, failures={}",
        actualBalance,
        finalPrice,
        successes.get(),
        failures.get());
  ...
  void completePayment_concurrent_onlyOneSucceeds() throws InterruptedException {
    ...
    Auction auction =
        buildRunningAuction(finalPrice, finalPrice, java.time.LocalDateTime.now().minusMinutes(1));
    AuctionWinner aw =
        AuctionWinner.create(winner, auction.getId(), finalPrice, depositPaid, false);
    auction.setWinner(aw);
    ...
    log.info(
        "[P3] successes={} failures={} winnerBalance={}",
        successes.get(),
        failures.get(),
        winner.getBalance());
  ...
  @Order(4)
  @DisplayName(
      "P4: releaseToSeller() + refundToWinnerFromBank() concurrent — balance bounds hợp lệ")
  @Timeout(value = 5)
  void releaseAndRefund_concurrent_balanceBoundsValid() throws InterruptedException {
    ...
    assertThat(seller.getBalance()).as("Seller balance không được âm").isGreaterThanOrEqualTo(0L);
    assertThat(seller.getBalance())
        .as("Seller balance không vượt 1 payout")
        .isLessThanOrEqualTo(payout);
    assertThat(winner.getBalance()).as("Winner balance không được âm").isGreaterThanOrEqualTo(0L);
    assertThat(winner.getBalance())
        .as("Winner balance không vượt finalPrice")
  ...
  void completePayment_vsExpirePayment_onlyOneOutcome() throws InterruptedException {
    ...
    Auction auction =
        buildRunningAuction(finalPrice, finalPrice, java.time.LocalDateTime.now().minusHours(2));
    ...
    boolean paid =
        auction.getStatus() == com.group13.auction.model.auction.Auction.AuctionStatus.PAID;
