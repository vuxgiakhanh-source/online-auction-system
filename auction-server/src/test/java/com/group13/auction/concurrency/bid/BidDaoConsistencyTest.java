...
/**
 * ============================================================================
 * BidDaoConsistencyTest — Group C (SANDWICH) Tầng giữa (BidService + AuctionLockRegistry) với tầng
 * dưới (DAO mock). Điểm giao nhau concurrency + persistence.
 * ============================================================================
 */
...
class BidDaoConsistencyTest extends ConcurrencyTestBase {
  ...
  void eachSuccessfulBidPersistsExactlyOnce() {
    ...
    verify(mockBidTransactionDAO, times(1))
  ...
  @Order(2)
  @DisplayName(
      "C2: Bid bị reject (giá thấp) → throw exception, KHÔNG gọi saveTransaction (FIX #2), KHÔNG"
          + " gọi updateHighestPrice")
  void invalidBid_savesRejectedTransaction_noAuctionUpdate() {
    ...
    verify(mockBidTransactionDAO, never())
  ...
  void antiSniping_bidInLastWindow_extendsEndTime() {
    ...
    Auction sniping =
        buildRunningAuction(STARTING_PRICE, RESERVE_PRICE, LocalDateTime.now().plusSeconds(20));
  ...
  void concurrentBids_totalSaveTransactionCallsMatchTotalAttempts() throws InterruptedException {
    ...
    verify(mockBidTransactionDAO, atMost(attempts.get()))
        .saveTransactionAndUpdatePrice(any(), anyString(), anyLong(), anyString());
  }
}
