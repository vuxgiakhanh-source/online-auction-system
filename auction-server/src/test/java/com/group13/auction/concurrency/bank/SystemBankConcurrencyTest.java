 ...
 *
 * SystemBank dùng đồng thời cả synchronized method lẫn AtomicLong.addAndGet()
 * bên trong — hai cơ chế chồng chéo. Bộ test này verify:
 *
 *   B1: N threads receive() đồng thời → totalBalance chính xác (không lost update).
 *   B2: receive() + payoutToSeller() concurrent → balance không âm sai.
 *   B3: receive() + refundToWinner() + receiveForfeittedDeposit() hỗn hợp
 *       → net balance khớp với tổng đại số các operations.
 *   B4: getTotalBalance() thread-safe — không bao giờ trả về giá trị ngoài
 *       khoảng [min_possible, max_possible].
 * ============================================================================
 */
...
class SystemBankConcurrencyTest extends ConcurrencyTestBase {
    ...
    void concurrentReceive_balanceIsExact() throws InterruptedException {
        ...
        for (int i = 0; i < threadCount; i++) {
            new Thread(() -> {
                try { gate.await(); bank.receive(amount); }
    ...
    @Test
    @Order(2)
    @DisplayName("B2: receive() + payoutToSeller() concurrent — balance không âm, net value chính xác")
    @Timeout(value = 5)
    void concurrentReceiveAndPayout_balanceNeverNegative() throws InterruptedException {
        ...
        assertThat(bank.getTotalBalance())
                .as("totalBalance phải = baseline(%d) + netDelta(%d) = %d",
                        baseline, netDelta.get(), expectedFinal)
                .isEqualTo(expectedFinal);

        assertThat(bank.getTotalBalance())
                .as("totalBalance không được âm")
    ...
    @Test
    @Order(3)
    @DisplayName("B3: receive() + refundToWinner() + receiveForfeittedDeposit() hỗn hợp — net balance khớp")
    @Timeout(value = 8)
    void mixedConcurrentOps_netBalanceCorrect() throws InterruptedException {
        ...
        long expectedNet = (long) nReceive  * receiveAmt
                - (long) nRefund   * refundAmt
                + (long) nForfeit  * forfeitAmt;
        ...
        for (int i = 0; i < nReceive; i++) {
            new Thread(() -> {
                try { gate.await(); bank.receive(receiveAmt); }
                catch (InterruptedException e) { Thread.currentThread().interrupt(); }
                finally { done.countDown(); }
            }).start();
        }
        for (int i = 0; i < nRefund; i++) {
            new Thread(() -> {
                try { gate.await(); bank.refundToWinner(refundAmt); }
                catch (InterruptedException e) { Thread.currentThread().interrupt(); }
                finally { done.countDown(); }
            }).start();
        }
        for (int i = 0; i < nForfeit; i++) {
            new Thread(() -> {
                try { gate.await(); bank.receiveForfeittedDeposit(forfeitAmt); }
                catch (InterruptedException e) { Thread.currentThread().interrupt(); }
                finally { done.countDown(); }
            }).start();
        }
    ...
    @Test
    @Order(4)
    @DisplayName("B4: getTotalBalance() trong khi receive() đang chạy — giá trị luôn trong khoảng hợp lệ")
    @Timeout(value = 8)
