 ...
 *
 * GAP 2: executePaymentToBank() concurrent double-payment
 *   → synchronized(winner) bảo vệ, verify chỉ 1 payment đi qua.
 *
 * GAP 5: getAvailableBalance() non-atomic read
 *   → 2 AtomicLong reads riêng biệt → snapshot thoáng âm có thể xảy ra.
 *   → Verify: balance thực tế không bao giờ âm sau concurrent ops.
 * ============================================================================
 */
...
class WalletConcurrencyTest extends ConcurrencyTestBase {
    ...
    @Test
    @Order(1)
    @DisplayName("G2-1: 2 threads executePaymentToBank() cùng winner — synchronized(winner) ngăn double-payment")
    @Timeout(value = 5)
    ...
    @Test
    @Order(2)
    @DisplayName("G2-2: 5 threads concurrent deposit() — synchronized(user) atomic, không lost update")
    @Timeout(value = 5)
    ...
    @Test
    @Order(4)
    @DisplayName("G5-1: concurrent getAvailableBalance() + lockDeposit() — balance thực tế không bao giờ âm")
    @Timeout(value = 8)
    void concurrentBalanceRead_neverActuallyNegative() throws InterruptedException {
        ...
        for (int i = 0; i < writerCount; i++) {
            ...
            new Thread(() -> {
                try {
                    gate.await();
                    try { walletService.lockDeposit(user, lockAmt, aId); }
                    catch (Exception ignored) {}
                } catch (InterruptedException e) {
                    Thread.currentThread().interrupt();
                } finally {
