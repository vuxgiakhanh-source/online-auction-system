 ...
 *
 * SessionManager Singleton dùng ConcurrentHashMap → thread-safe theo Javadoc.
 * Test này verify không có state corruption khi:
 *   - register/unregister/authenticate chạy đồng thời (sandwich real vs mock)
 *   - broadcast khi session đang bị remove đồng thời (lost update check)
 *   - watcher list đọc/ghi đồng thời không NPE
 *
 * GAP-SESSION-1: register + authenticate đồng thời → byUserId nhất quán
 * GAP-SESSION-2: unregister giữa chừng broadcast → không NPE/exception
 * GAP-SESSION-3: addAuctionWatcher + removeAuctionWatcher đồng thời → không ConcurrentModificationException
 * ============================================================================
 */
...
class SessionManagerConcurrencyTest extends ConcurrencyTestBase {
    ...
    class RegisterAuthenticateConcurrencyTest {
        ...
        void registerThenAuthenticate_concurrent_connectedCountCorrect() throws Exception {
            ...
            for (int i = 0; i < THREAD_COUNT; i++) wsList.add(mockWs("c1a-" + i));
            ...
            for (int i = 0; i < THREAD_COUNT; i++) {
                ...
                futures.add(pool.submit(() -> {
                    ...
                    try {
                        ...
                        sessionManager.authenticate(wsList.get(idx),
                                "userId-c1a-" + idx, "user-c1a-" + idx, "NORMAL_USER");
                    } catch (Exception e) {
            ...
            startGate.countDown();
            for (Future<?> f : futures) f.get(TIMEOUT_SECONDS, TimeUnit.SECONDS);
            pool.shutdown();

            assertThat(errors.get()).as("Không có exception khi register+authenticate đồng thời").isZero();
        ...
        void authenticateDeauthenticate_concurrent_noNPE() throws Exception {
            ...
            for (int i = 0; i < THREAD_COUNT; i++) {
                ...
                futures.add(pool.submit(() -> {
                    ...
                    try {
                        sessionManager.authenticate(wsList.get(idx),
                                "userId-c1b-" + idx, "user-c1b-" + idx, "NORMAL_USER");
            ...
            assertThat(sessionManager.getAuthenticatedCount())
                    .as("Tất cả đã deauthenticate → byUserId = 0").isZero();
        }
    }
    ...
    class UnregisterDuringOperationTest {
        ...
        void registerUnregister_concurrent_byConnectionConsistent() throws Exception {
            List<WebSocket> wsList = new ArrayList<>();
            for (int i = 0; i < THREAD_COUNT; i++) wsList.add(mockWs("c2a-" + i));
            ...
            assertThat(sessionManager.getConnectedCount())
                    .as("Tất cả đã unregister → byConnection = 0").isZero();
        }
    }
    ...
    class WatcherConcurrencyTest {
        ...
        @Order(1)
        @Timeout(TIMEOUT_SECONDS)
        @DisplayName("C3a: nhiều session cùng watch/unwatch một auction — không ConcurrentModificationException")
        void watchUnwatch_sameAuction_noConcurrentModification() throws Exception {
            ...
            assertThat(errors.get())
                    .as("Không có exception khi watch/unwatch đồng thời").isZero();
        ...
        @Order(2)
        @Timeout(TIMEOUT_SECONDS)
        @DisplayName("C3b: isOnline() đọc đồng thời khi authenticate/deauthenticate đang xảy ra — không NPE")
        void isOnline_concurrentRead_noNPE() throws Exception {
            ...
            assertThat(errors.get())
                    .as("Không NPE khi đọc isOnline đồng thời với deauthenticate").isZero();
        }
    }
}
