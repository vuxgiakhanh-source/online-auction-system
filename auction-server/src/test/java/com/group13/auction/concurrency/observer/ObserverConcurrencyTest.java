 ...
 *
 * GAP 4: AuctionService.addObserver() + notify() concurrent
 *   → CopyOnWriteArrayList.contains() + add() không atomic → duplicate risk.
 *   → Verify: không lost observer; duplicate chỉ khi cùng instance.
 *
 * GAP 8: Stress test observer add + notify nhiều rounds.
 * ============================================================================
 */
...
class ObserverConcurrencyTest extends ConcurrencyTestBase {
     ...
     * AuctionService có field `private final SystemAdmin system = SystemAdmin.getInstance()`
     * được khởi tạo tại field-level (trước constructor body) → cần INSTANCE tồn tại
     * trước khi `new AuctionService(...)` được gọi.
     */
    ...
    @Test
    @Order(1)
    @DisplayName("G4-1: 20 threads addObserver() với instance khác nhau — tất cả observers nhận notify()")
    @Timeout(value = 10)
    void concurrentAddObserver_allObserversReceiveNotify() throws InterruptedException {
        ...
        as.notify(auction,
                ...
                buildUser("notifier", 0L), 600_000L);

        assertThat(notifyCount.get())
                .as("Tất cả %d observers phải nhận notify()", N)
                .isEqualTo(N);
    }
    ...
    @Test
    @Order(2)
    @DisplayName("G4-2: [KNOWN RISK] Cùng observer instance add từ 5 threads — duplicate detection không atomic")
    @Timeout(value = 5)
    void sameObserver_concurrentAdd_duplicateRisk_documentedBehavior() throws InterruptedException {
        ...
        as.notify(auction,
                AuctionEvent.AuctionEventType.BID_PLACED,
                buildUser("notifier2", 0L), 600_000L);

        if (notifyCount.get() > 1) {
            log.warn("[G4-2 KNOWN RISK] Observer duplicate: notifyCount={} "
                    + "(contains+add not atomic → fix bằng synchronized block)", notifyCount.get());
        }
    ...
    void stressRounds_addAndNotify_countCorrect() throws InterruptedException {
        for (int round = 0; round < 5; round++) {
            ...
            as.notify(freshAuction,
                    AuctionEvent.AuctionEventType.BID_PLACED,
                    buildUser("n" + round, 0L), 600_000L);

            assertThat(count.get())
                    .as("[Round %d] Phải có đúng %d notifications", round, N)
                    .isEqualTo(N);
