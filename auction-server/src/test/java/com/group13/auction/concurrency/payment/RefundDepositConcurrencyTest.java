 ...
 *
 * Vấn đề:
 *   refundDeposits() gọi walletService.unlockDeposit() cho từng bidder tuần tự.
 *   Nếu 2 threads gọi refundDeposits() đồng thời:
 *     - unlockDeposit() của cùng 1 bidder được gọi 2 lần → deposit unlock 2× → âm.
 *     - WalletService.unlockDeposit() có synchronized(bidder) → deposit không âm,
 *       nhưng availableBalance có thể tăng 2× (unlock 2 lần cùng amount).
 *
 *   forfeitDeposit() cũng có cùng vấn đề nếu gọi 2 lần cho 1 winner.
 *
 * Tests:
 *   R1: refundDeposits() gọi 2 lần đồng thời — mỗi bidder được unlock đúng 1 lần
 *       → availableBalance tăng đúng depositAmount (không double).
 *   R2: refundDeposits() concurrent với nhiều bidders — tổng balance đúng.
 *   R3: forfeitDeposit() concurrent 2 lần cùng 1 winner — deposit chỉ trừ 1 lần.
 *
 * FIX CẦN:
 *   Thêm guard "đã refund chưa" trong refundDeposits() —  ví dụ kiểm tra
...
class RefundDepositsConcurrencyTest extends ConcurrencyTestBase {
    ...
    void refundDeposits_concurrent_eachBidderUnlockedOnce() throws InterruptedException {
        ...
        when(mockBidTransactionDAO.findBiddersByAuction(auction.getId()))
                .thenReturn(bidders);
        ...
        for (NormalUser bidder : bidders) {
            ...
            assertThat(available)
                    .as("Bidder [%s] availableBalance phải = USER_BALANCE sau refund",
                            bidder.getUsername())
    ...
    void refundDeposits_20Bidders_totalBalanceCorrect() throws InterruptedException {
        ...
        for (NormalUser bidder : bidders) {
            assertThat(bidder.getAvailableBalance())
                    .as("Bidder [%s] availableBalance không được vượt USER_BALANCE",
                            bidder.getUsername())
    ...
    @Test
    @Order(3)
    @DisplayName("R3: forfeitDeposit() concurrent 2 lần cùng winner — deposit chỉ trừ 1 lần, không âm")
    @Timeout(value = 5)
    void forfeitDeposit_concurrent_depositDeductedOnce() throws InterruptedException {
        ...
        log.info("[R3] successes={}, failures={}, lockedDeposit={}, balance={}",
                successes.get(), failures.get(),
                winner.getLockedDeposit(), winner.getBalance());
        ...
        if (failures.get() == 0) {
            log.warn("[R3 POTENTIAL BUG] Cả 2 threads forfeit đều success — "
                    + "lockedDeposit sau: {}. Nếu = 0: synchronized OK. "
                    + "Nếu âm: double-forfeit.", winner.getLockedDeposit());
        }
    }
    ...
    void refundDeposits_excludesWinner_correctlyProcessed() {
        ...
        AuctionWinner aw = AuctionWinner.create(winner, auction.getId(),
                STARTING_PRICE, DEPOSIT_AMOUNT, false);
