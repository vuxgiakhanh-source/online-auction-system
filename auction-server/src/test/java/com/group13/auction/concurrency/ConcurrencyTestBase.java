...
/**
 * Base class chứa helper dùng chung cho tất cả concurrency test.
 * Tránh duplicate code giữa 7 file test.
 */
public abstract class ConcurrencyTestBase {
    ...
    protected NormalUser buildUser(String username, long balance) {
        return NormalUser.reconstitute(
                ...
                username, "hashed", username + "@test.com",
    ...
    protected List<NormalUser> buildBidders(int count) {
        ...
        for (int i = 0; i < count; i++) {
            list.add(buildUser("bidder_" + i + "_" + UUID.randomUUID().toString().substring(0, 4),
                    USER_BALANCE));
        }
        return list;
    }
    ...
    protected Auction buildRunningAuction() {
        return buildRunningAuction(STARTING_PRICE, RESERVE_PRICE,
                LocalDateTime.now().plusHours(2));
    }

    protected Auction buildRunningAuction(long startingPrice, long reservePrice,
        ...
        Item item = Electronics.reconstitute(
                ...
                "Item-" + UUID.randomUUID().toString().substring(0, 4), "desc",
                startingPrice, seller, "Brand", 12, "New"
        );
    ...
    /**
     * AuctionObserver rỗng – implement đủ 2 methods để không compile lỗi.
     * Dùng thay cho lambda khi chỉ cần một stub không làm gì.
     */
    protected static AuctionObserver noopObserver() {
        return new AuctionObserver() {
            @Override public void onBidPlaced(AuctionEvent e) {}
            @Override public void onAuctionEnded(AuctionEvent e) {}
        };
    }

    /**
     * Tạo AuctionObserver đếm số lần được gọi (dùng AtomicInteger bên ngoài).
     * Vì AuctionObserver không phải functional interface, helper này tách rõ concern.
     */
    protected static AuctionObserver countingObserver(java.util.concurrent.atomic.AtomicInteger counter) {
        return new AuctionObserver() {
            @Override public void onBidPlaced(AuctionEvent e)  { counter.incrementAndGet(); }
            @Override public void onAuctionEnded(AuctionEvent e) { counter.incrementAndGet(); }
        };
    }

    /**
     * Add SELLER role vào user qua reflection
     * (roles là EnumSet private final trong NormalUser).
     */
