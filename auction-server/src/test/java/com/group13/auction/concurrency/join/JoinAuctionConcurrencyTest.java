 ...
 *
 * <p>THAY ĐỔI SO VỚI BẢN CŨ: - G1-1 / G7-1: BugRoot là joinAuction() thiếu synchronized trên
 * bidder. → Test giờ ENFORCE assert chính xác (times(1)), không chỉ document bug. → Khi
 * BidService.joinAuction() được fix (bọc synchronized(bidder)), hai test này sẽ GREEN. - G6-1 /
 * G6-2: giữ nguyên known-bug assertion (< N) vì fix cần đổi source. - G7-1: delay nhân tạo 50ms
 * được giữ để stress-test race window.
 * ============================================================================
 */
...
class JoinAuctionConcurrencyTest extends ConcurrencyTestBase {
  ...
  @Order(1)
  @DisplayName(
      "G1-1: 10 threads cùng join cùng 1 bidder — lockDeposit() phải gọi đúng 1 lần [ENFORCE]")
  @Timeout(value = 10)
  void concurrentJoin_singleBidder_lockDepositCalledOnce() throws InterruptedException {
    ...
    verify(mockWalletService, times(1)).lockDeposit(eq(bidder), anyLong(), eq(auction.getId()));

    assertThat(bidder.hasJoined(auction.getId())).as("Bidder phải join thành công").isTrue();
  ...
  @Order(6)
  @DisplayName(
      "G7-1: Window race với delay nhân tạo — lockDeposit() phải chỉ gọi 1 lần dù 3 threads race"
          + " [ENFORCE]")
  @Timeout(value = 15)
  void joinWindow_raceCondition_withDelay() throws InterruptedException {
    ...
    if (callCount > 1) {
      log.warn(
          "[G7-1 BUG CONFIRMED] lockDeposit() gọi {} lần thay vì 1! "
              + "Race window giữa hasJoined() và lockDeposit() cần synchronized.",
          callCount);
    }
  ...
  void afterJoin_hasJoined_isTrue_subsequentJoinIgnored() {
    ...
    assertThatCode(() -> bidService.joinAuction(bidder, auction, obs)).doesNotThrowAnyException();
