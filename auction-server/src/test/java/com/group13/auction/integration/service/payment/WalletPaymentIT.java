...
/**
 * ════════════════════════════════════════════════════════════════════
 *  WalletPaymentIT — Integration Tests cho WalletService (Tài chính)
 *  Kỹ thuật: Bottom-up (WalletService + DAO trực tiếp, không mock)
 * ════════════════════════════════════════════════════════════════════
 *
 *  Scope: Kiểm tra tầng WalletService tích hợp với DAO + DB thực.
 *
 *  Bao gồm:
 *  - TC-01: executePaymentToBank() — Rollback nhất quán RAM + DB
 *           BUG RISK: systemBank.receive() gọi trước DB persist → không rollback được
 *           khi DB fail. batchTx ghi financialTransactionDAO trước exception → duplicate.
 *
 *  - TC-06: withdraw() — Không rút vào tiền đang lock (availableBalance boundary)
 *           BUG RISK: getAvailableBalance() = balance - lockedDeposit, nếu logic sai
 *           → user rút vào tiền cọc → lockedDeposit hụt → mất cọc khi thanh toán.
 */
@RequiresDocker
@Testcontainers
...
class WalletPaymentIT extends IntegrationTestBase {
    ...
    class ExecutePaymentToBankRollbackTests {
        ...
        @Test
        @Order(1)
        @DisplayName("TC-01a: Happy path — balance, lockedDeposit, DB nhất quán sau thanh toán thành công")
        void happyPath_balanceAndDB_consistentAfterPayment() {
            ...
            // Assert RAM
            assertAll("RAM nhất quán sau thanh toán thành công",
            ...
            assertAll("DB khớp RAM sau thanh toán thành công",
        ...
        void insufficientBalance_throwsPaymentException_ramAndDbRolledBack() {
            ...
            // RAM phải rollback
            assertAll("RAM rollback sau thất bại",
                () -> assertThat(winner.getBalance())
            ...
            NormalUser fromDB = userDAO.findNormalUserById(winner.getId());
            assertAll("DB phải phản ánh trạng thái rollback",
                () -> assertThat(fromDB.getBalance())
        ...
        @Test
        @Order(3)
        @DisplayName("TC-01c: Concurrent executePaymentToBank — chỉ 1 trong 2 thread thành công, không double-payment")
        void concurrentPayment_onlyOneSucceeds_noDoubleCharge() throws InterruptedException {
            ...
            assertAll("Concurrent payment — không double charge",
                () -> assertThat(successCount.get())
                        .as("Chỉ đúng 1 thread được thanh toán thành công")
                        .isEqualTo(1),
                () -> assertThat(failCount.get())
                        .as("1 thread phải thất bại")
                        .isEqualTo(1),
                () -> assertThat(winner.getBalance())
    ...
    class WithdrawBoundaryTests {
        ...
        void withdraw_exactAvailableAmount_succeeds() {
            ...
            assertAll("Sau khi rút đúng available",
            ...
            assertThat(fromDB.getBalance())
                    .as("DB balance phải = 3M")
                    .isEqualTo(3_000_000L);
        }

        @Test
        @Order(2)
        @DisplayName("TC-06b: Rút vượt available (cố rút vào locked) → IllegalArgumentException, không thay đổi")
        void withdraw_exceedsAvailableBalance_throws_noSideEffect() {
            ...
            assertAll("Không thay đổi sau withdraw thất bại",
        ...
        @Test
        @Order(4)
        @DisplayName("TC-06d: deposit() → withdraw() → deposit() nhiều lần — DB balance nhất quán end-to-end")
