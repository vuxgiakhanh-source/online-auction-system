...
/**
 * ════════════════════════════════════════════════════════════════════
 *  DepositRefundIT — Integration Tests cho luồng hoàn cọc
 *  Kỹ thuật: Bottom-up (PaymentService + WalletService + DAO thực)
 * ════════════════════════════════════════════════════════════════════
 *
 *  Scope: Kiểm tra PaymentService.refundDeposits() — phương thức
 *  hoàn cọc cho tất cả bidder khi phiên bị CANCELED hoặc kết thúc.
 *
 *  TC-02 [CRITICAL]:
 *  BUG RISK: refundDeposits() gọi bidTransactionDAO.findBiddersByAuction()
 *  để lấy danh sách, rồi walletService.unlockDeposit() cho từng người.
 *  depositAmount tính bằng startingPrice * 3/10 thay vì query từ
 *  financial_transactions → số tiền hoàn có thể không khớp số tiền đã lock.
 *  Winner không được hoàn (cọc tính vào finalPrice) — nếu logic sai → mất tiền.
 */
@RequiresDocker
@Testcontainers
...
class DepositRefundIT extends IntegrationTestBase {
    ...
    void threeBidders_allReceiveFullDepositRefund_afterCancel() {
        ...
        for (NormalUser b : List.of(b1, b2, b3)) {
            ...
            BidTransaction tx = BidTransaction.create(
                    b, auction.getId(), 5_500_000L, BidTransaction.BidResult.ACCEPTED);
        ...
        assertAll("Tất cả bidder nhận lại đủ cọc",
    ...
    @Test
    @Order(4)
    @DisplayName("TC-02d: refundDeposits() gọi 2 lần — lần 2 không double-refund (lockedDeposit đã = 0)")
