 ...
 *
 * <p>Sandwich: AuctionService (real) + BidderObserver/SellerObserver (real) + AuctionDAO (mock).
 *
 * <p>API thực tế đã xác nhận: AuctionEvent.getEventType() — KHÔNG phải getType()
 * AuctionEvent.getBidAmount() — KHÔNG phải getAmount() BidderObserver(NormalUser, IRatingService)
 * SellerObserver(NormalUser, IRatingService) AuctionService.getObservers(String auctionId) →
 * List<AuctionObserver>
 */
...
class RealtimeNotificationIntegrationTest {
  ...
  /**
   * FIX: inject mock AuctionWinnerDAO. closeAuction() TH2 gọi saveWinner() — real DAO sẽ fail vì
   * auction không có trong DB (FK constraint) → throw RuntimeException → test TC_TD_01 crash.
   */
  ...
  void setUp() throws Exception {
    ...
    auctionService =
        new AuctionService(
            ratingService, mockAuctionDAO, mockFinancialTransactionDAO, mockAuctionWinnerDAO);
    ...
    when(mockFinancialTransactionDAO.saveTransaction(
            any(com.group13.auction.model.bid.FinancialTransaction.class)))
        .thenReturn(true);
    when(mockFinancialTransactionDAO.findLockedDepositAmount(anyString(), anyString()))
        .thenReturn(0L);
  ...
  class TopDownEventDispatch {
    ...
    void closeAuction_withWinner_dispatchesAuctionEnded() {
      ...
      List<AuctionEventType> types =
          cap.getAllValues().stream().map(AuctionEvent::getEventType).toList();
    ...
    void startAuction_dispatchesAuctionStarted_toAllObservers() {
      ...
      verify(spyB, times(1))
          .onAuctionEnded(argThat(e -> e.getEventType() == AuctionEventType.AUCTION_STARTED));
      verify(spyS, times(1))
          .onAuctionEnded(argThat(e -> e.getEventType() == AuctionEventType.AUCTION_STARTED));
    }
  ...
  class BottomUpObserverBehavior {
    ...
    void bidderObserver_onAuctionEnded_asWinner_noException() {
      ...
      AuctionEvent event =
          new AuctionEvent(AuctionEventType.AUCTION_ENDED, auction, bidder, 1_100_000L);
    ...
    void bidderObserver_onAuctionEnded_asLoser_noException() {
      ...
      BidderObserver obs = new BidderObserver(bidder, ratingService);
      AuctionEvent event =
          new AuctionEvent(AuctionEventType.AUCTION_ENDED, auction, bidder2, 1_100_000L);
    ...
    @Test
    @DisplayName(
        "TC_BU_03: SellerObserver.onAuctionEnded() nhận AUCTION_ENDED — không ném exception")
    ...
    void bidderObserver_onBidPlaced_noException() {
      ...
      BidderObserver obs = new BidderObserver(bidder, ratingService);
      AuctionEvent event =
          new AuctionEvent(AuctionEventType.BID_PLACED, auction, bidder2, 600_000L);
  ...
  class SandwichIntegration {
    ...
    @Test
    @DisplayName(
        "TC_SW_01: closeAuction() → observer.onAuctionEnded() được gọi đúng 1 lần với"
            + " AUCTION_ENDED")
    void closeAuction_callsObserverOnce_withCorrectEvent() {
      ...
      verify(spy, times(1))
          .onAuctionEnded(argThat(e -> e.getEventType() == AuctionEventType.AUCTION_ENDED));
    ...
    @Test
    @DisplayName(
        "TC_SW_04: startAuction() chỉ broadcast tới đúng auctionId — phiên khác không nhận")
  ...
  class BigBangEndToEnd {
    ...
    void auctionEvent_containsCorrectFields() {
      ...
      AuctionEvent event = new AuctionEvent(AuctionEventType.BID_PLACED, auction, bidder, amount);
  ...
  class BoundaryAndError {
    ...
    void addObserver_null_doesNotThrow() {
      ...
      assertThatCode(() -> auctionService.addObserver(auctionId, null)).doesNotThrowAnyException();
      ...
      assertThatCode(
              () ->
                  auctionService.notify(
                      auction, AuctionEventType.AUCTION_EXTENDED, bidder, 0L, null))
   ...
   *
   * <p><b>KHÔNG</b> set instance = null vì AuctionService.notify() gọi {@code
   * AuctionManager.getInstance()} trực tiếp mà không kiểm tra null — nếu null thì NPE sẽ lan sang
   * toàn bộ test chạy sau trong cùng JVM.
   *
   * <p>Thay vào đó: clear 4 collection nội bộ (allAuctions, allUsers, globalObservers,
   * staffObservers) để đạt test isolation mà không phá singleton.
   */
  ...
  private void resetAuctionManagerSingleton() throws Exception {
    ...
    for (String fieldName :
        new String[] {"allAuctions", "allUsers", "globalObservers", "staffObservers"}) {
