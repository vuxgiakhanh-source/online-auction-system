...
class AuctionWinnerDAOIT extends IntegrationTestBase {
  ...
  class SaveWinnerTests {
    ...
    void saveWinner_valid_persistsAndHasPending() {
      ...
      AuctionWinner winner =
          AuctionWinner.create(ctx.winner, ctx.auction.getId(), ctx.finalPrice, ctx.deposit, false);
    ...
    void saveWinner_secondOffer_returnsTrue() {
      ...
      AuctionWinner winner =
          AuctionWinner.create(ctx.winner, ctx.auction.getId(), ctx.finalPrice, ctx.deposit, true);
  ...
  class UpdatePaymentStatusTests {
    ...
    void updatePaymentStatus_fundsHeld_noLongerPending() {
      ...
      boolean updated =
          auctionWinnerDAO.updatePaymentStatus(winner.getId(), PaymentStatus.FUNDS_HELD.name());

      assertThat(updated).as("Phải update thành công FUNDS_HELD").isTrue();
