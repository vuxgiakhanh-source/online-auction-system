 ...
 *
 * Tầng trên (stub): AuctionDAO, AuctionWinnerDAO, UserDAO, FinancialTransactionDAO,
 *                   BidTransactionDAO, SecondChanceOfferDAO
 * Tầng giữa (real): PaymentService ↔ WalletService ↔ AuctionService
 * Tầng dưới (real): SystemBank, Auction, AuctionWinner domain objects
 *
 * Luồng kiểm tra: RUNNING → closeAuction() → FINISHED → completePayment() → PAID
 *
 * Lưu ý về balance:
 *   availableBalance = balance - lockedDeposit
 *   completePayment trừ: remaining = finalPrice - depositPaid từ availableBalance
 *   → winner.balance phải >= remaining + depositPaid để availableBalance >= remaining
 */
...
class PaymentStateTransitionIT {
    ...
    class HappyPath {
        ...
        void systemBank_receives_finalPrice() throws Exception {
            ...
            assertThat(SystemBank.getInstance().getTotalBalance() - bankBefore)
        ...
        void winnerDAO_called_once_with_FUNDS_HELD() {
            paymentService.completePayment(auction);
            verify(auctionWinnerDAO, times(1)).updatePaymentStatus(
    ...
    class FailurePath {
        ...
        void auctionDAO_not_called_with_PAID_on_failure() {
            ...
            verify(auctionDAO, never()).updateAuctionStatus(anyString(),
                    eq(AuctionStatus.PAID.name()));
        }
    }
    ...
    class StateIntegrity {
        ...
        void end_to_end_full_lifecycle() {
            ...
            Auction e2e = TestFixture.runningAuction(TestFixture.normalSeller("e2eSeller"),
                    STARTING_PRICE);
    ...
    private Auction buildFinishedAuctionWithPrice(NormalUser customWinner, long finalPrice) {
        ...
        AuctionWinner aw = AuctionWinner.create(
                customWinner, a.getId(), finalPrice, DEPOSIT, false);
        a.setWinner(aw);
