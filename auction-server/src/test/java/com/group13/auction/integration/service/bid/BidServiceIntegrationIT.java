...
class BidServiceIntegrationIT extends IntegrationTestBase {
  ...
  class JoinAuctionTests {
    ...
    void joinAuction_happyPath_depositLockedAndDbConsistent() {
      ...
      assertAll(
          "DB khớp RAM sau joinAuction",
          () -> assertThat(fromDB.getBalance()).isEqualTo(bidder.getBalance()),
          () -> assertThat(fromDB.getLockedDeposit()).isEqualTo(bidder.getLockedDeposit()));

      // FinancialTransaction DEPOSIT_LOCK phải được ghi
      long lockedInDB =
          financialTransactionDAO.findLockedDepositAmount(bidder.getId(), auction.getId());
    ...
    @Order(3)
    @DisplayName(
        "TC-04c: Seller cố join phiên của chính mình — bị từ chối AuctionBusinessException")
    ...
    void joinAuction_insufficientBalance_rejected_noSideEffect() {
      ...
      assertAll(
          "Không có side-effect khi join thất bại",
  ...
  class PlaceBidTests {
    ...
    @Order(1)
    @DisplayName(
        "TC-05a: Happy path — placeBid() ghi BidTransaction ACCEPTED, DB currentPrice cập nhật")
    ...
    @Order(3)
    @DisplayName(
        "TC-05c: Bid >= reservePrice → BidResult.ACCEPTED (không phải ACCEPTED_RESERVE_NOT_MET)")
    ...
    @Order(5)
    @DisplayName(
        "TC-05e: [CONCURRENT] 10 thread cùng placeBid — chỉ 1 winner, price tăng monotonic")
    void placeBid_concurrent10Threads_onlyOneLeader_priceMonotonic() throws InterruptedException {
      ...
      for (int i = 0; i < threadCount; i++) {
        ...
        bidService.joinAuction(bidders[i], auction, new BidderObserver(bidders[i], null));
      }
      ...
      assertAll(
          "Concurrent bid — price monotonic, leader valid",
          () ->
  ...
  private Auction givenRunningAuction(
    ...
    String itemId = buildItem(seller.getId(), "Item-" + sellerUsername, startingPrice, itemDAO);
    Item item = itemDAO.findItemById(itemId);
    Auction auction =
        Auction.create(
            item,
  ...
  private Auction givenRunningAuctionForSeller(
      NormalUser seller, long startingPrice, long reservePrice) {
