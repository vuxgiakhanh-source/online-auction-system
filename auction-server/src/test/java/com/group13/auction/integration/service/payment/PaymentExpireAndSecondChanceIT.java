...
/**
 * ════════════════════════════════════════════════════════════════════
 *  PaymentExpireAndSecondChanceIT — Integration Tests cho luồng
 *  hết hạn thanh toán và chào hàng lần 2 (Second Chance Offer)
 *  Kỹ thuật: Sandwich (Service giữa + DAO thật + mock-free)
 * ════════════════════════════════════════════════════════════════════
 *
 *  TC-03 [CRITICAL] — expirePayment() full chain:
 *  BUG RISK: expirePayment() nối 5 thành phần:
 *    forfeitDeposit → penalizeLatePayment → autoBanIfNeeded
 *    → status EXPIRED → offerSecondChance → SecondChanceOffer tạo + lưu DB
 *  Mỗi bước fail có thể dừng chuỗi giữa chừng → trạng thái không nhất quán.
 *
 *  TC-07 [HIGH] — SecondChanceOffer lifecycle:
 *  BUG RISK: 3 nhánh ACCEPTED/DECLINED/EXPIRED chưa được test tích hợp.
 *  ACCEPTED phải lockDeposit runnerUp + tạo AuctionWinner mới.
 *  DECLINED/EXPIRED phải cancelAuction ngay.
 */
@RequiresDocker
@Testcontainers
...
class PaymentExpireAndSecondChanceIT extends IntegrationTestBase {
    ...
    class ExpirePaymentFullChainTests {
        ...
        void expirePayment_fullChain_allFourStepsConsistent() {
            ...
            AuctionWinner aw = AuctionWinner.reconstitute(
                    ...
                    java.time.LocalDateTime.now().minusHours(25),
                    winner, auction.getId(), 7_000_000L, deposit,
            ...
            assertAll("expirePayment — 4 bước phải nhất quán",
        ...
        @Test
        @Order(2)
        @DisplayName("TC-03b: expirePayment() với isSecondOffer=true → cancelAuction ngay, không tạo SecondChance mới")
        void expirePayment_secondOfferExpired_cancelAuctionDirectly_noNewSecondChance() {
            ...
            AuctionWinner aw = AuctionWinner.reconstitute(
                    ...
                    java.time.LocalDateTime.now().minusHours(25),
                    java.time.LocalDateTime.now().minusHours(25),
                    runner, auction.getId(), 5_000_000L, deposit,
                    java.time.LocalDateTime.now().minusHours(1), // paymentDeadline đã qua
                    null, null,
                    AuctionWinner.PaymentStatus.PENDING,
        ...
        void expirePayment_notYetExpired_guardClauseActivates_noChanges() {
            ...
            AuctionWinner aw = AuctionWinner.create(
                    winner, auction.getId(), 5_000_000L, deposit, false);
            auction.setWinner(aw);
            ...
            assertAll("Chưa expire — không có thay đổi",
                    () -> assertThat(winner.getRating())
    ...
    class SecondChanceOfferLifecycleTests {
        ...
        void accept_locksRunnerUpDeposit_setsNewWinner() {
            ...
            SecondChanceOffer offer = SecondChanceOffer.create(
                    runner, auction.getId(), offerPrice, depositPaid);

            // Act
            paymentService.acceptSecondChanceOffer(offer, auction);

            assertAll("acceptSecondChanceOffer — side effects",
                    ...
                    () -> assertThat(auction.getWinner())
                            .as("Auction phải có winner mới")
                            .isNotNull(),
        ...
        void decline_offerDeclined_auctionCanceled() {
            ...
            SecondChanceOffer offer = SecondChanceOffer.create(
                    runner, auction.getId(), 6_000_000L, 900_000L);

            paymentService.declineSecondChanceOffer(offer, auction);

            assertAll("declineSecondChanceOffer",
                    () -> assertThat(offer.getStatus())
        ...
        @Test
        @Order(3)
        @DisplayName("TC-07c: acceptSecondChanceOffer() khi offer không còn PENDING → IllegalStateException")
        ...
        @Test
        @Order(4)
        @DisplayName("TC-07d: acceptSecondChanceOffer() khi offer đã hết hạn → status EXPIRED, auction CANCELED")
        void accept_offerExpired_statusExpired_auctionCanceled() {
            ...
            SecondChanceOffer expiredOffer = SecondChanceOffer.reconstitute(
                    java.util.UUID.randomUUID().toString(),
                    java.time.LocalDateTime.now().minusHours(25),
                    java.time.LocalDateTime.now().minusHours(25),
                    runner, auction.getId(), 6_000_000L, 900_000L,
            ...
            assertAll("Expired offer → EXPIRED + CANCELED",
    ...
    private Auction givenRunningAuction(String sellerUsername, long startingPrice, long reservePrice) {
        NormalUser seller = buildUserWithBalance(sellerUsername, 50_000_000L, userDAO);
        ensureSellerRecord(seller.getId());
        String itemId     = UUID.randomUUID().toString();
        itemDAO.addItem(itemId, seller.getId(), "Item " + sellerUsername,
                "test", startingPrice, "ELECTRONICS");
        trackItem(itemId);

        var item    = itemDAO.findItemById(itemId);
        var auction = Auction.create(item,
