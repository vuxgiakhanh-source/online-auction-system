...
class RatingServiceIntegrationIT extends IntegrationTestBase {
  ...
  class PenalizeLatePaymentTests {
    ...
    @Order(1)
    @DisplayName(
        "TC-08a: penalizeLatePayment() — rating giảm 1.0, hasEverBeenPenalized = true, DB nhất"
            + " quán")
    void penalizeLatePayment_reducesRatingAndSetsFlag_dbConsistent() {
      ...
      assertAll(
          "RAM sau penalize",
          () -> assertThat(bidder.getRating()).as("rating phải giảm đúng 1.0").isEqualTo(2.0),
          () ->
    ...
    @Order(2)
    @DisplayName(
        "TC-08b: penalizeLatePayment() khi rating ≤ 1.5 → auto-suspend, DB status = SUSPENDED")
    void penalizeLatePayment_belowSuspendThreshold_autoSuspends() {
      ...
      assertAll(
          "Auto-suspend sau penalize",
          () -> assertThat(bidder.getRating()).as("rating phải = 1.0").isEqualTo(1.0),
          () ->
    ...
    @Order(4)
    @DisplayName(
        "TC-08d: isEligible() sau penalize đủ → false (rating < 2.0 sau 2 lần phạt từ 3.0)")
  ...
  class RestoreSuspendedTests {
    ...
    @Order(1)
    @DisplayName(
        "TC-09a: Sau đúng 3 tháng + 1 ngày → rating tăng, ACTIVE, hasEverBeenRestored = true")
    void restore_afterThreeMonths_ratingIncreasedAndStatusActive() throws Exception {
      ...
      ratingService.checkAndRestoreSuspended(
          bidder, LocalDateTime.now()); // currentTime = now > threshold

      assertAll(
          "Sau restore",
          () -> assertThat(bidder.getRating()).as("rating phải tăng thêm 0.6").isGreaterThan(1.0),
          () ->
    ...
    void restore_calledTwice_secondCallIsNoOp() throws Exception {
      ...
      assertThat(bidder.getRating()).as("Rating không được tăng lần 2").isEqualTo(ratingAfterFirst);
    }
    ...
    void restore_beforeThreeMonths_noSideEffect() throws Exception {
      ...
      assertAll(
          "Không restore khi chưa đủ thời gian",
          () -> assertThat(bidder.getRating()).isEqualTo(ratingBefore),
          () -> assertThat(bidder.getAccountStatus()).isEqualTo(User.AccountStatus.SUSPENDED),
