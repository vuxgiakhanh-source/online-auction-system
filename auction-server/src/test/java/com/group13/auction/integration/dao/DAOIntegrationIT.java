 ...
 * (Bottom-up Strategy) ============================================================
 *
 * <p>CHIẾN LƯỢC: Bottom-up Integration Testing ------------------------------------------ Theo lý
 * thuyết Bottom-up, chúng ta bắt đầu test từ tầng thấp nhất (DatabaseConnection) rồi tích hợp dần
 * lên tầng DAO cao hơn:
 *
 * <p>Tầng 1 → DatabaseConnection (cơ sở hạ tầng DB) Tầng 2 → UserDAO + FinancialTransactionDAO (DAO
 * độc lập) Tầng 3 → ItemDAO + BidTransactionDAO (DAO phụ thuộc UserDAO) Tầng 4 → AuctionDAO (DAO
 * phụ thuộc ItemDAO + UserDAO) Tầng 5 → Tích hợp đa DAO + kiểm tra tính toàn vẹn Transaction
 *
 * <p>CÔNG NGHỆ: - Testcontainers (MySQLContainer): database thực tế, isolated - JUnit 5: test
 * framework - AssertJ: fluent assertions
 *
 * <p>DATA ISOLATION: - @BeforeEach: tạo dữ liệu test mới trước mỗi test - @AfterEach: cleanup toàn
 * bộ dữ liệu đã tạo trong test đó - Không dùng dữ liệu shared giữa các test → mỗi test độc lập hoàn
 * toàn
 *
 * <p>Các class được tích hợp: - {@link DatabaseConnection} (Singleton — reconfigured via HikariCP
 * pool) - {@link UserDAO} - {@link ItemDAO} - {@link AuctionDAO} - {@link BidTransactionDAO} -
...
class DAOIntegrationIT {
  ...
  /**
   * Cleanup sau mỗi test — xóa theo thứ tự FK an toàn: bid_transactions → financial_transactions →
   * user_auction_activity → auctions → items → users
   */
  @AfterEach
      ...
      try (Connection conn = DatabaseConnection.getInstance().getConnection()) {
        ...
        for (String table : requiredTables) {
          try (ResultSet rs = meta.getTables(null, null, table, new String[] {"TABLE"})) {
            assertThat(rs.next()).as("Bảng '%s' phải tồn tại trong schema", table).isTrue();
          }
        }
      }
    }
  ...
  class UserDAOTests {
    ...
    void registerUser_createsUserAndReturnsUuid() {
      String userId =
          userDAO.registerUser("bidder_alice", hashPassword("pass1234"), "alice@omnibid.vn");
    ...
    void registerUser_duplicateUsername_returnsNull() {
      String userId1 =
          userDAO.registerUser("bidder_bob", hashPassword("pass1234"), "bob@omnibid.vn");
      createdUserIds.add(userId1);

      // Cùng username, email khác
      String userId2 =
          userDAO.registerUser("bidder_bob", hashPassword("pass5678"), "bob2@omnibid.vn");
    ...
    void findNormalUserById_reconstitutesUser() {
      String userId =
          userDAO.registerUser("bidder_carol", hashPassword("pass1234"), "carol@omnibid.vn");
      createdUserIds.add(userId);
    ...
    void addBalance_incrementsBalanceCorrectly() {
      String userId =
          userDAO.registerUser("bidder_dave", hashPassword("pass1234"), "dave@omnibid.vn");
      createdUserIds.add(userId);
    ...
    void updateBalances_updatesBothFields() {
      String userId =
          userDAO.registerUser("bidder_eve", hashPassword("pass1234"), "eve@omnibid.vn");
      createdUserIds.add(userId);
    ...
    void updateAccountStatus_changesStatusInDB() {
      String userId =
          userDAO.registerUser("bidder_frank", hashPassword("pass1234"), "frank@omnibid.vn");
      createdUserIds.add(userId);
    ...
    void saveAndFindUserAuctionActivity_roundTrip() {
      // Tạo seller + seller record + item + auction thật vì user_auction_activity có FK →
      // auctions(id)
      String userId =
          userDAO.registerUser("bidder_grace", hashPassword("pass1234"), "grace@omnibid.vn");
      createdUserIds.add(userId);
    ...
    @Order(9)
    @DisplayName(
        "saveUserAuctionActivity() WATCHING không overwrite JOINED (regression #join-watch-bug)")
    void saveUserAuctionActivity_watchingDoesNotOverwriteJoined() {
      // Arrange — tạo user + auction thật (FK constraint)
      String userId =
          userDAO.registerUser("bidder_henry", hashPassword("pass1234"), "henry@omnibid.vn");
      createdUserIds.add(userId);
    ...
    void updateAccountStatus_supportsAllStatuses(String status) {
      ...
      assertThat(result).isTrue();
      NormalUser user = userDAO.findNormalUserById(userId);
      assertThat(user.getAccountStatus().name())
  ...
  class FinancialTransactionDAOTests {
    ...
    private String createRealAuction(String sellerUsername) {
      String sellerId =
          userDAO.registerUser(sellerUsername, hashPassword("pass"), sellerUsername + "@test.vn");
      createdUserIds.add(sellerId);
      ensureSellerRecord(sellerId);
      String itemId = UUID.randomUUID().toString();
      itemDAO.addItem(
          itemId, sellerId, "FinTx Item " + sellerUsername, "desc", 1_000_000L, "ELECTRONICS");
      createdItemIds.add(itemId);
      Item item = itemDAO.findItemById(itemId);
      Auction auction =
          Auction.create(
    ...
    void saveTransaction_depositLock_persistsCorrectly() {
      ...
      String userId = userDAO.registerUser("wallet_user1", hashPassword("pass"), "wallet1@test.vn");
      createdUserIds.add(userId);
      String auctionId = createRealAuction("wallet_seller1");

      FinancialTransaction tx =
          FinancialTransaction.create(
              userId, "SYSTEM_LOCKED", 500_000L, TransactionType.DEPOSIT_LOCK, auctionId);
    ...
    void findLockedDepositAmount_sumIsCorrect() {
      String userId = userDAO.registerUser("wallet_user2", hashPassword("pass"), "wallet2@test.vn");
      createdUserIds.add(userId);
    ...
    void findLockedDepositAmount_noneFound_returnsZero() {
      long total =
          financialTransactionDAO.findLockedDepositAmount(
    ...
    void saveTransaction_allTransactionTypes_persist() {
      String userId = userDAO.registerUser("wallet_user3", hashPassword("pass"), "wallet3@test.vn");
      createdUserIds.add(userId);
      ...
      for (TransactionType type : types) {
        FinancialTransaction tx =
            FinancialTransaction.create(userId, "SYSTEM_BANK", 100_000L, type, auctionId);
        boolean saved = financialTransactionDAO.saveTransaction(tx);
  ...
  class ItemDAOIntegrationTests {
    ...
    void addItem_withValidSeller_persistsItem() {
      String sellerId =
          userDAO.registerUser("seller_henry", hashPassword("pass"), "henry@omnibid.vn");
      createdUserIds.add(sellerId);
      ensureSellerRecord(sellerId);
    ...
    void findItemById_reconstitutesElectronics() {
      String sellerId =
          userDAO.registerUser("seller_irene", hashPassword("pass"), "irene@omnibid.vn");
      createdUserIds.add(sellerId);
      ensureSellerRecord(sellerId);

      String itemId = UUID.randomUUID().toString();
      itemDAO.addItem(itemId, sellerId, "MacBook Air M2", "Brand new", 25_000_000L, "ELECTRONICS");
      createdItemIds.add(itemId);
    ...
    void findItemsBySellerId_returnsOnlySellerItems() {
      String seller1Id =
          userDAO.registerUser("seller_jack", hashPassword("pass"), "jack@omnibid.vn");
      String seller2Id =
          userDAO.registerUser("seller_kate", hashPassword("pass"), "kate@omnibid.vn");
      ...
      assertThat(seller1Items)
          .hasSize(2)
          .extracting(Item::getId)
          .containsExactlyInAnyOrder(item1Id, item2Id);
      assertThat(seller2Items).hasSize(1).extracting(Item::getId).containsExactly(item3Id);
    }
    ...
    void addItem_withImageUrls_persistsAndRecovers() {
      String sellerId = userDAO.registerUser("seller_leo", hashPassword("pass"), "leo@omnibid.vn");
      createdUserIds.add(sellerId);
      ensureSellerRecord(sellerId);

      List<String> imgs = List.of("/uploads/items/img-a.jpg", "/uploads/items/img-b.jpg");

      String itemId = UUID.randomUUID().toString();
      boolean saved =
          itemDAO.addItem(
              itemId, sellerId, "Tranh sơn dầu", "Tác phẩm mới", 5_000_000L, "ART", imgs);
      createdItemIds.add(itemId);
      ...
      assertThat(found).isNotNull();
      assertThat(found.getImageUrls()).hasSize(2).containsExactlyElementsOf(imgs);
    }
    ...
    void addItem_noImageUrls_returnsEmptyList() {
      String sellerId = userDAO.registerUser("seller_mia", hashPassword("pass"), "mia@omnibid.vn");
      createdUserIds.add(sellerId);
      ensureSellerRecord(sellerId);
  ...
  class AuctionDAOIntegrationTests {
    ...
    /** Helper: tạo seller + item + auction đầy đủ trong DB, track IDs để cleanup. */
    private Auction prepareFullAuction(
        String sellerUsername, String itemName, long startingPrice, long reservePrice) {
      ...
      String itemId = UUID.randomUUID().toString();
      itemDAO.addItem(
          itemId, sellerId, itemName, "Integration test item", startingPrice, "ELECTRONICS");
      createdItemIds.add(itemId);
    ...
    void createAuction_persistsWithOpenStatus() {
      Auction auction =
          prepareFullAuction("seller_liam", "Sony WH-1000XM5", 3_000_000L, 4_000_000L);
    ...
    void updateAuctionStatus_openToRunning_updatesDB() {
      Auction auction = prepareFullAuction("seller_mia", "Dell XPS 15", 15_000_000L, 18_000_000L);
    ...
    void updateHighestPrice_updatesPriceAndLeader() {
      Auction auction = prepareFullAuction("seller_noah", "LG C2 OLED", 10_000_000L, 12_000_000L);

      // Tạo bidder
      String bidderId =
          userDAO.registerUser("bidder_olivia", hashPassword("pass"), "olivia@omnibid.vn");
      createdUserIds.add(bidderId);

      boolean updated = auctionDAO.updateHighestPrice(auction.getId(), 11_000_000L, bidderId);
    ...
    @Order(3)
    @DisplayName(
        "updateHighestPrice() với giá THẤP HƠN current_price → no-op (stale-write protection)")
    void updateHighestPrice_stalePriceLower_isNoOp() {
      ...
      Auction auction =
          prepareFullAuction("seller_stale", "Stale Write Item", 8_000_000L, 10_000_000L);

      String bidderId =
          userDAO.registerUser("bidder_stale", hashPassword("pass"), "stale@omnibid.vn");
      createdUserIds.add(bidderId);

      // Bid hợp lệ: nâng giá lên 11_000_000
      boolean firstUpdate = auctionDAO.updateHighestPrice(auction.getId(), 11_000_000L, bidderId);
      assertThat(firstUpdate).isTrue();

      // Stale-write: giá cũ 9_000_000 (thấp hơn 11_000_000 hiện tại) → phải là no-op
      boolean staleUpdate = auctionDAO.updateHighestPrice(auction.getId(), 9_000_000L, bidderId);
    ...
    @Order(3)
    @DisplayName(
        "updateHighestPrice() với giá BẰNG current_price → no-op (không overwrite bằng cùng giá)")
    void updateHighestPrice_samePriceEqualCurrent_isNoOp() {
      Auction auction =
          prepareFullAuction("seller_equal", "Equal Price Item", 5_000_000L, 6_000_000L);

      String bidderId =
          userDAO.registerUser("bidder_equal", hashPassword("pass"), "equal@omnibid.vn");
      createdUserIds.add(bidderId);
      ...
      boolean sameUpdate = auctionDAO.updateHighestPrice(auction.getId(), 7_000_000L, bidderId);
    ...
    void updateAuctionResult_finishedState_persistsCorrectly() {
      Auction auction = prepareFullAuction("seller_peter", "ASUS ROG", 20_000_000L, 22_000_000L);

      String bidderId =
          userDAO.registerUser("bidder_quinn", hashPassword("pass"), "quinn@omnibid.vn");
      createdUserIds.add(bidderId);
    ...
    void updateEndTime_antiSniping_updatesDB() {
      Auction auction = prepareFullAuction("seller_rachel", "Kindle Oasis", 2_000_000L, 2_500_000L);
      ...
      assertThat(queryEndTime(auction.getId())).isAfterOrEqualTo(auction.getEndTime());
    }
    ...
    void findUnfinishedAuctionIds_onlyActiveStatuses() {
      String sellerId = userDAO.registerUser("seller_sam", hashPassword("pass"), "sam@omnibid.vn");
      createdUserIds.add(sellerId);
      userDAO.addBalance(sellerId, 50_000_000L);
  ...
  class BidTransactionDAOIntegrationTests {
    ...
    void saveTransaction_accepted_persistsBid() {
      ...
      BidTransaction tx =
          BidTransaction.create(ctx.bidder, ctx.auctionId, 2_500_000L, BidResult.ACCEPTED);
    ...
    void saveTransaction_rejected_persistsForAuditTrail() {
      ...
      BidTransaction tx =
          BidTransaction.create(ctx.bidder, ctx.auctionId, 100L, BidResult.REJECTED);
    ...
    void findBiddersByAuction_excludesRejectedBids() {
      ...
      String bidder2Id =
          userDAO.registerUser("bid_bidder3b", hashPassword("pass"), "bidder3b@omnibid.vn");
      createdUserIds.add(bidder2Id);
      NormalUser bidder2 = userDAO.findNormalUserById(bidder2Id);

      BidTransaction acceptedTx =
          BidTransaction.create(ctx.bidder, ctx.auctionId, 2_200_000L, BidResult.ACCEPTED);
      BidTransaction rejectedTx =
          BidTransaction.create(bidder2, ctx.auctionId, 100L, BidResult.REJECTED);
    ...
    void findByAuctionId_returnsSortedBidHistory() {
      ...
      BidTransaction tx2 =
          BidTransaction.create(ctx.bidder, ctx.auctionId, 2_500_000L, BidResult.ACCEPTED);
      BidTransaction tx3 =
          BidTransaction.create(ctx.bidder, ctx.auctionId, 2_800_000L, BidResult.ACCEPTED);
      ...
      assertThat(history)
          .extracting(BidTransaction::getAmount)
  ...
  class TransactionIntegrityTests {
     ...
     *
     * <p>Kiểm tra: trừ balance + tăng lockedBalance + lưu FinancialTransaction phải nhất quán với
     * nhau sau khi thành công.
     *
     * <p>Đây là simulation của WalletService.lockDeposit() ở tầng DAO: ta thực hiện thủ công trong
     * 1 Connection + 1 Transaction để đảm bảo atomicity.
     */
    @Test
    @Order(1)
    ...
    void lockDeposit_atomic_allOrNothing() throws Exception {
      // Setup
      String userId =
          userDAO.registerUser("atom_bidder1", hashPassword("pass"), "atom1@omnibid.vn");
      createdUserIds.add(userId);
      userDAO.addBalance(userId, 10_000_000L);
      ensureSellerRecord(userId);
      String auctionId = UUID.randomUUID().toString();
      insertDummyAuctionRow(
          auctionId, userId); // FK: financial_transactions.auction_id → auctions.id
      ...
      assertAll(
          "Atomicity — balance, locked, finTx phải nhất quán",
          () ->
     ...
     *
     * <p>Kiểm tra: nếu bước lưu FinancialTransaction bị lỗi (ví dụ: vi phạm constraint), thì
     * balance trong bảng users phải được rollback — không mất tiền của user.
     */
    @Test
    @Order(2)
    @DisplayName(
        "TC-ATOM-02: Rollback khi lưu FinancialTransaction thất bại — balance không thay đổi")
    void lockDeposit_rollback_onFinancialTxFailure() throws Exception {
      String userId =
          userDAO.registerUser("atom_bidder2", hashPassword("pass"), "atom2@omnibid.vn");
      createdUserIds.add(userId);
      userDAO.addBalance(userId, 10_000_000L);
      ensureSellerRecord(userId);
      String auctionId = UUID.randomUUID().toString();
      ...
      assertThatThrownBy(() -> executeAtomicDepositLock_withFailure(userId, auctionId, 3_000_000L))
     ...
     *
     * <p>Kiểm tra: 2 thread đồng thời lockDeposit cho cùng 1 user không làm mất tiền (lost update).
     */
    @Test
    @Order(3)
    ...
    void concurrentLockDeposit_noLostUpdate() throws Exception {
      String userId =
          userDAO.registerUser("atom_bidder3", hashPassword("pass"), "atom3@omnibid.vn");
      createdUserIds.add(userId);
      userDAO.addBalance(userId, 10_000_000L);
      ensureSellerRecord(userId);
     ...
     *
     * <p>Kiểm tra: balance được cộng lại + lockedDeposit giảm + FinancialTx lưu đúng.
     */
    @Test
    @Order(4)
    ...
    void unlockDeposit_atomic_restoresBalanceCorrectly() throws Exception {
      String userId =
          userDAO.registerUser("atom_bidder4", hashPassword("pass"), "atom4@omnibid.vn");
      createdUserIds.add(userId);
      userDAO.addBalance(userId, 10_000_000L);
      ensureSellerRecord(userId);
      String auctionId = UUID.randomUUID().toString();
      ...
      NormalUser user = userDAO.findNormalUserById(userId);
      assertAll(
          "unlockDeposit — phải hoàn về trạng thái ban đầu",
          () ->
              assertThat(user.getBalance())
     ...
     *
     * <p>Kiểm tra: nếu user chỉ có 5tr nhưng cố lock 8tr, thao tác phải bị reject và balance không
     * thay đổi.
     */
    @Test
    @Order(5)
    ...
    void lockDeposit_exceedsAvailableBalance_rejected() throws Exception {
      String userId =
          userDAO.registerUser("atom_bidder5", hashPassword("pass"), "atom5@omnibid.vn");
      createdUserIds.add(userId);
      userDAO.addBalance(userId, 5_000_000L);
      // auctionId không cần tồn tại — test throw trước khi chạm DB
      String auctionId = UUID.randomUUID().toString();

      assertThatThrownBy(() -> executeAtomicDepositLock(userId, auctionId, 8_000_000L))
     ...
     *
     * <p>Simulate: bidder thắng phiên, thanh toán phần còn lại. Kiểm tra: balance giảm đúng,
     * FinancialTransaction ghi đủ các bước.
     */
    @Test
    @Order(6)
    @DisplayName(
        "TC-ATOM-06: Multi-step payment flow — balance và FinancialTx nhất quán end-to-end")
    void paymentFlow_multiStep_allTransactionsConsistent() throws Exception {
      String winnerId =
          userDAO.registerUser("atom_winner6", hashPassword("pass"), "winner6@omnibid.vn");
  ...
  class CrossDAOIntegrityTests {
    ...
    void fullBidFlow_dataConsistencyAcrossAllTables() {
      // === Setup ===
      String sellerId =
          userDAO.registerUser("crossdao_seller", hashPassword("pass"), "crossseller@omnibid.vn");
      String bidderId =
          userDAO.registerUser("crossdao_bidder", hashPassword("pass"), "crossbidder@omnibid.vn");
      ...
      String itemId = UUID.randomUUID().toString();
      itemDAO.addItem(itemId, sellerId, "Cross DAO Test Item", "d", 5_000_000L, "ELECTRONICS");
      createdItemIds.add(itemId);
      ...
      userDAO.updateBalances(bidderId, 20_000_000L - deposit, deposit);

      FinancialTransaction lockTx =
          FinancialTransaction.create(
              bidderId, "SYSTEM_LOCKED", deposit, TransactionType.DEPOSIT_LOCK, auction.getId());
      ...
      BidTransaction bidTx =
          BidTransaction.create(bidder, auction.getId(), bidAmount, BidResult.ACCEPTED);
      ...
      long depositInDB = financialTransactionDAO.findLockedDepositAmount(bidderId, auction.getId());
      ...
      assertAll(
          ...
          () ->
              assertThat(bidderFromDB.getLockedDeposit())
                  ...
                  .isEqualTo(deposit),
          () -> assertThat(bidHistory).hasSize(1).as("BidTransaction phải được ghi 1 lần"),
          () ->
    ...
    @Order(2)
    @DisplayName(
        "Auction kết thúc không có winner: updateAuctionResult(CANCELED) — không tạo AuctionWinner")
    void closeAuction_noWinner_statusCanceled() {
      String sellerId =
          userDAO.registerUser("crossdao_seller2", hashPassword("pass"), "crossseller2@omnibid.vn");
      createdUserIds.add(sellerId);
      ensureSellerRecord(sellerId);
    ...
    void dataCleanupVerification_noLeakedData() {
      ...
      String tempUserId =
          userDAO.registerUser("cleanup_check_user", hashPassword("pass"), "cleanup@omnibid.vn");
   ...
   */
  private String executeAtomicDepositLock(String userId, String auctionId, long depositAmount)
      throws Exception {
    FinancialTransaction tx =
        FinancialTransaction.create(
            userId, "SYSTEM_LOCKED", depositAmount, TransactionType.DEPOSIT_LOCK, auctionId);
      ...
      try {
        ...
        if (available < depositAmount) {
          ...
          throw new IllegalStateException(
              "Số dư khả dụng không đủ. Khả dụng: " + available + ", Yêu cầu: " + depositAmount);
        }
        // Dùng arithmetic SQL: balance = balance - ? tránh lost update
        try (PreparedStatement ps =
            conn.prepareStatement(
                "UPDATE users SET balance = balance - ?, locked_balance = locked_balance + ? WHERE"
                    + " id = ?")) {
  ...
  /** Cố tình simulate lỗi ở bước lưu FinancialTransaction để kiểm tra rollback của TC-ATOM-02. */
  private void executeAtomicDepositLock_withFailure(
      String userId, String auctionId, long depositAmount) throws Exception {
    NormalUser user = userDAO.findNormalUserById(userId);
    ...
    try (Connection conn = DatabaseConnection.getInstance().getConnection()) {
      ...
      try {
        ...
        try (PreparedStatement ps =
            conn.prepareStatement("UPDATE users SET balance = ? WHERE id = ?")) {
  ...
  /**
   * Thực thi unlockDeposit trong 1 DB Transaction (atomicity). Bao gồm: 1. Cập nhật balance +
   * lockedBalance (cộng lại balance) 2. Lưu FinancialTransaction kiểu DEPOSIT_UNLOCK
   *
   * @return ID của FinancialTransaction đã lưu
   */
  private String executeAtomicDepositUnlock(String userId, String auctionId, long depositAmount)
      throws Exception {
    FinancialTransaction tx =
        FinancialTransaction.create(
            "SYSTEM_LOCKED", userId, depositAmount, TransactionType.DEPOSIT_UNLOCK, auctionId);
  ...
  /**
   * Simulate thanh toán cuối: trừ phần remaining + giải phóng deposit. Balance = balance -
   * (finalPrice - depositPaid) - depositPaid = balance - finalPrice
   *
   * @return ID của FinancialTransaction payment
   */
  private String executeAtomicPayment(
      String userId, String auctionId, long finalPrice, long depositPaid) throws Exception {
    long remaining = finalPrice - depositPaid;

    FinancialTransaction paymentTx =
        FinancialTransaction.create(
            userId, "SYSTEM_BANK", finalPrice, TransactionType.PAYMENT_FROM_WINNER, auctionId);
  ...
  /**
   * Đảm bảo user có bản ghi APPROVED trong bảng sellers. items.seller_id FK → sellers(user_id),
   * không phải users(id). Dùng INSERT IGNORE để idempotent.
   */
  ...
  /**
   * Insert bản ghi auction tối thiểu vào DB để phục vụ FK constraint trong
   * financial_transactions.auction_id → auctions(id). Dùng trong TransactionIntegrityTests khi test
   * không cần auction đầy đủ.
   */
  ...
  private void deleteByIds(Connection conn, String table, String column, List<String> ids)
      throws SQLException {
    if (ids.isEmpty()) {
      return;
    }
  ...
  private FullAuctionContext createFullAuctionContext(
      String sellerUsername, String bidderUsername) {
    String sellerId =
        userDAO.registerUser(sellerUsername, hashPassword("pass"), sellerUsername + "@omnibid.vn");
    String bidderId =
        userDAO.registerUser(bidderUsername, hashPassword("pass"), bidderUsername + "@omnibid.vn");
    createdUserIds.addAll(List.of(sellerId, bidderId));
    ...
    String itemId = UUID.randomUUID().toString();
    itemDAO.addItem(
        itemId, sellerId, "Test Item " + sellerUsername, "d", 2_000_000L, "ELECTRONICS");
    createdItemIds.add(itemId);

    Item item = itemDAO.findItemById(itemId);
    Auction auction =
        Auction.create(
            item, LocalDateTime.now().plusMinutes(1), LocalDateTime.now().plusHours(1), 3_000_000L);
    auctionDAO.createAuction(auction);
    createdAuctionIds.add(auction.getId());
   ...
   * records/classes // =========================================================================
   *
   * <p>/** Context object giữ thông tin auction + bidder cho các test.
   */
