 ...
 *
 * <p>Controller/service phía client chỉ nên gọi lớp này thay vì gọi trực tiếp
 * {@link AuctionWebSocketClient}. Cách tách này giữ nguyên code teammate đã viết,
 * đồng thời gom toàn bộ request API của client vào một điểm ổn định.
 */
public final class ClientNetworkFacade {
     ...
     *
     * <p>Ứng dụng JavaFX bình thường chỉ nên dùng một server URI trong một lần chạy.
     * Nếu cần đổi server URI sau khi đã tạo instance, hãy gọi {@link #shutdown()} trước.
     *
     ...
     *
     * <p>Callback chạy trên thread WebSocket. Nếu cập nhật JavaFX UI, controller/service
     * cần tự bọc bằng {@code Platform.runLater(...)}.
     *
     * @param packet packet cần gửi
