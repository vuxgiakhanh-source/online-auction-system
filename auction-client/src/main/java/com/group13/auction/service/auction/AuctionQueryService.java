...
public final class AuctionQueryService {
   ...
   *
   * <p>Khi {@code keyword} rỗng, service dùng API danh sách auction. Khi có keyword, service dùng
   * API {@code SEARCH_ITEMS}. Server hiện đã hỗ trợ {@code scopeFilter} cho cả hai luồng, nên
   * client chỉ cần gửi scope tương ứng với lựa chọn trên UI. Riêng trạng thái khi đang search vẫn
   * được lọc thêm ở client vì {@code ItemSearchRequestDTO} chưa có {@code statusFilter}.
   *
