...
public enum Route {
  ...
  MY_ORDERS(ViewPath.MY_ORDERS_VIEW),
  QUALITY_REPORT(ViewPath.QUALITY_REPORT_VIEW),
  MY_QUALITY_REPORTS(ViewPath.QUALITY_REPORT_LIST_VIEW),
  SELLER_QUALITY_REPORTS(ViewPath.QUALITY_REPORT_LIST_VIEW),
  ...
  ADMIN_STAFF_MANAGEMENT(ViewPath.ADMIN_STAFF_MANAGEMENT_VIEW),
  ADMIN_SYSTEM_BANK(ViewPath.ADMIN_SYSTEM_BANK_VIEW);
  ...
  public static Optional<Route> fromFxmlPath(String fxmlPath) {
    ...
    return Arrays.stream(values()).filter(route -> route.fxmlPath.equals(fxmlPath)).findFirst();
  }
